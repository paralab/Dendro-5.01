...
template <typename T>
void io::varToRawData(const ot::Mesh* pMesh, const T** vars,
                      unsigned int numVars, const char** varNames,
                      const char* fPrefix) {
    char fName[256];
    ...
    for (unsigned int v = 0; v < numVars; v++) {
        for (unsigned int ele = eleLocalBegin; ele < eleLocalEnd; ele++) {
            ...
            skipIndex =
                (1u << (m_uiMaxDepth - (level + MAXDEAPTH_LEVEL_DIFF + 1))) - 1;

            for (unsigned int k = 0; k < nz; k++)
                for (unsigned int j = 0; j < ny; j++)
                    for (unsigned int i = 0; i < nx; i++) {
                        regGridBuffer[(rzI + k) * (Ny * Nx) + (ryI + j) * (Nx) +
                                      (rxI + i)] =
                            nodalValues[k * (ny * nx) + j * (ny) + i];
                        /* for(unsigned int sk=0;sk<skipIndex;sk++)
                             for(unsigned int sy=0;sy<skipIndex;sy++)
                                 for(unsigned int si=0;si<skipIndex;si++)
                                 {
                                    regGridBuffer[(rzI+sk) *(Ny*Nx) +
                           (ryI+sy)*(Nx) + (rxI+si) ]= nodalValues[ k*(ny*nx) +
                           j*(ny) + i];
                                 }*/
                    }
        }
...
template <typename T>
void io::dump_array(const T* const in, unsigned int sz, const char* fPrefix,
                    MPI_Comm comm) {
    int rank, npes;
