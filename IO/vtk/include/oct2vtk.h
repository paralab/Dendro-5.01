...
static int vtk_write_compressed(FILE *vtkfile, char *numeric_data,
                                size_t byte_length) {
    ...
    compression_header[2] =
        (uint32_t)(lastsize > 0 || byte_length == 0 ? lastsize : blocksize);
    for (iz = 3; iz < header_entries; ++iz) {
        compression_header[iz] = 0;
    }
    base64_init_encodestate(&encode_state);
    base_length = base64_encode_block((char *)compression_header, header_size,
...
static int vtk_write_binary(FILE *vtkfile, char *numeric_data,
                            size_t byte_length) {
    ...
    base_data   = (char *)calloc(code_length,
                                 sizeof(char));  // (code_length*sizeof(char));

    base64_init_encodestate(&encode_state);
    base_length = base64_encode_block((char *)&int_header, sizeof(int_header),
                                      base_data, &encode_state);
    assert(base_length < code_length);
    base_data[base_length] = '\0';
    (void)fwrite(base_data, 1, base_length, vtkfile);
...
/**
 *@breif Writes the given mesh to a binary vtu (in xml format) file (Note that
 *this will write only the coarse octree. ).
 * @param [in] pMesh: input mesh
 * @param [in] fPrefix: vtk file prefix
 * @param [in] numVars: Number of vertex point variables.
 * @param [in] varNames: list of variable names
...
/**
 * @brief Writes the given mesh to a binary vtu (in xml format) file (Note that
 * this will write only the coarse octree. ). (Only the slice of it. )
 *
...
/**
 * @brief computes the wavelet coefficients for specified varIDs and write them
 * and write them along with the mesh.
 ...
 *
 */
void waveletsToVTU(ot::Mesh *mesh, const double **zippedVec,
                   const double **unzippedVec, const unsigned int *varIds,
                   const unsigned int numVars, const char *fileName);
