...
// #define MESH_RANGE 1<<30    // points in the mesh will be generated randomly
// between 0 and MESH_RANGE
...
class Mpi_datatype<Node_Type> {
    /*
       ...
       MPI_Datatype * dptr) { for(int i = 0; i < (*len); i++) { ot::TreeNode
       first = (static_cast<ot::TreeNode*>(in))[i]; ot::TreeNode second =
       ...
       (static_cast<ot::TreeNode*>(inout))[i];
                    (static_cast<ot::TreeNode*>(inout))[i] = ( ( first > second
       )? first : second );
                }//end for
            }//end function
