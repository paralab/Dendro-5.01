...
MeshGPU* MeshGPU::alloc_mesh_on_device(const ot::Mesh* pMesh) {
    ...
    if (!pMesh->isActive()) return dptr_mesh;
    ...
    if (m2b_c.size() > 0)
        this->m_unzip_grid[1] =
            std::max(1u, *std::max_element(m2b_c.begin(), m2b_c.end()));

    this->m_max_send_ele = 1u;
    if (se_c.size() > 0)
        this->m_max_send_ele =
            std::max(1u, *std::max_element(se_c.begin(), se_c.end()));

    this->m_max_recv_ele = 1u;
    if (re_c.size() > 0)
        this->m_max_recv_ele =
            std::max(1u, *std::max_element(re_c.begin(), re_c.end()));

    this->m_max_send_cg = 1u;
    if (s_cg_c.size() > 0)
        this->m_max_send_cg =
            std::max(1u, *std::max_element(s_cg_c.begin(), s_cg_c.end()));

    this->m_max_recv_cg = 1u;
    if (r_cg_c.size() > 0)
        this->m_max_recv_cg =
            std::max(1u, *std::max_element(r_cg_c.begin(), r_cg_c.end()));
    ...
    for (unsigned int i = 0; i < blk_list.size(); i++)
        if ((blk_list[i].getLocalElementEnd() -
             blk_list[i].getLocalElementBegin()) > m_zip_grid[0])
            this->m_zip_grid[0] = (blk_list[i].getLocalElementEnd() -
                                   blk_list[i].getLocalElementBegin());

    this->m_e2b_unzip_map = GPUDevice::device_malloc<DEVICE_UINT>(m2b.size());
    this->m_e2b_unzip_map_count =
        GPUDevice::device_malloc<DEVICE_UINT>(m2b_c.size());
    this->m_e2b_unzip_map_offset =
        GPUDevice::device_malloc<DEVICE_UINT>(m2b_f.size());
    ...
    GPUDevice::host_to_device_async<DEVICE_UINT, cudaStream_t>(
        m2b.data(), this->m_e2b_unzip_map, m2b.size());
    GPUDevice::host_to_device_async<DEVICE_UINT, cudaStream_t>(
        m2b_c.data(), this->m_e2b_unzip_map_count, m2b_c.size());
    GPUDevice::host_to_device_async<DEVICE_UINT, cudaStream_t>(
        m2b_f.data(), this->m_e2b_unzip_map_offset, m2b_f.size());
    GPUDevice::host_to_device_async<DEVICE_UINT, cudaStream_t>(
        e2e.data(), this->m_e2e, e2e.size());
    GPUDevice::host_to_device_async<DEVICE_UINT, cudaStream_t>(
        en_cg.data(), this->m_e2n_cg, en_cg.size());
    GPUDevice::host_to_device_async<DEVICE_UINT, cudaStream_t>(
        en_dg.data(), this->m_e2n_dg, en_dg.size());

    GPUDevice::host_to_device_async<DEVICE_UINT, cudaStream_t>(
        se_sm.data(), this->m_elem_send_sm, se_sm.size());
    GPUDevice::host_to_device_async<DEVICE_UINT, cudaStream_t>(
        re_sm.data(), this->m_elem_recv_sm, re_sm.size());
    GPUDevice::host_to_device_async<DEVICE_UINT, cudaStream_t>(
        se_c.data(), this->m_elem_send_c, se_c.size());
    GPUDevice::host_to_device_async<DEVICE_UINT, cudaStream_t>(
        se_f.data(), this->m_elem_send_f, se_f.size());
    GPUDevice::host_to_device_async<DEVICE_UINT, cudaStream_t>(
        re_c.data(), this->m_elem_recv_c, re_c.size());
    GPUDevice::host_to_device_async<DEVICE_UINT, cudaStream_t>(
        re_f.data(), this->m_elem_recv_f, re_f.size());

    GPUDevice::host_to_device_async<DEVICE_UINT, cudaStream_t>(
        s_cg_sm.data(), this->m_cg_send_sm, s_cg_sm.size());
    GPUDevice::host_to_device_async<DEVICE_UINT, cudaStream_t>(
        r_cg_sm.data(), this->m_cg_recv_sm, r_cg_sm.size());
    GPUDevice::host_to_device_async<DEVICE_UINT, cudaStream_t>(
        s_cg_c.data(), this->m_cg_send_c, s_cg_c.size());
    GPUDevice::host_to_device_async<DEVICE_UINT, cudaStream_t>(
        s_cg_f.data(), this->m_cg_send_f, s_cg_f.size());
    GPUDevice::host_to_device_async<DEVICE_UINT, cudaStream_t>(
        r_cg_c.data(), this->m_cg_recv_c, r_cg_c.size());
    GPUDevice::host_to_device_async<DEVICE_UINT, cudaStream_t>(
        r_cg_f.data(), this->m_cg_recv_f, r_cg_f.size());

    BlockGPU3D* blk_buffer =
        GPUDevice::host_malloc<BlockGPU3D>(blk_list.size());
    TNode3d* tn_buffer = GPUDevice::host_malloc<TNode3d>(pNodes.size());
    HangingInfo* hanging_info =
        GPUDevice::host_malloc<HangingInfo>(pNodes.size());
    ...
    for (unsigned int ele = 0; ele < pNodes.size(); ele++) {
        for (unsigned int dir = 0; dir < NUM_FACES; dir++) {
            ...
            if (pMesh->isFaceHanging(ele, dir, cnum)) {
                ...
                // hanging_info[ele].get_fcnum(dir)!=cnum)
                //     printf("dire %d  hflag %d fcnum %d \n",dir,
                //     hanging_info[ele].hflag, hanging_info[ele].fcnum);
            }
        }
        ...
        for (unsigned int dir = NUM_FACES; dir < NUM_FACES + NUM_EDGES; dir++) {
            ...
            if (pMesh->isEdgeHanging(ele, dir, cnum)) {
                ...
                // if(hanging_info[ele].is_hanging(dir)!=true &&
                // hanging_info[ele].get_ecnum(dir)!=cnum)
                //     printf("dire %d  hflag %d ecnum %d \n",dir,
                //     hanging_info[ele].hflag, hanging_info[ele].ecnum);
            }
        }
    }
    ...
    GPUDevice::host_to_device_async<HangingInfo, cudaStream_t>(
        hanging_info, this->m_hinfo, pNodes.size());
    ...
    for (unsigned int i = 0; i < blk_list.size(); i++) {
        ...
        const Point oct_coord =
            Point((double)blk_list[i].getBlockNode().minX(),
                  (double)blk_list[i].getBlockNode().minY(),
                  (double)blk_list[i].getBlockNode().minZ());
    ...
    GPUDevice::host_to_device_async<BlockGPU3D, cudaStream_t>(
        blk_buffer, this->m_blk_list, blk_list.size());

    for (unsigned int i = 0; i < pNodes.size(); i++)
        tn_buffer[i] = TNode3d(pNodes[i]);

    GPUDevice::host_to_device_async<TNode3d, cudaStream_t>(
        tn_buffer, this->m_all_elements, pNodes.size());

    const RefElement* refel = pMesh->getReferenceElement();
    cudaMemcpyToSymbolAsync(refel_1d, (const DEVICE_REAL*)refel->getIMTChild0(),
                            sizeof(DEVICE_REAL) * (ep + 1) * (ep + 1), 0);
    cudaMemcpyToSymbolAsync(refel_1d, (const DEVICE_REAL*)refel->getIMTChild1(),
                            sizeof(DEVICE_REAL) * (ep + 1) * (ep + 1),
                            sizeof(DEVICE_REAL) * (ep + 1) * (ep + 1));
