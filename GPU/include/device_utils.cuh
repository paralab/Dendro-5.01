...
template <typename T>
void alloc_mpi_ctx(const ot::Mesh* pMesh,
                   std::vector<ot::AsyncExchangeContex>& ctx_list, int dof,
                   int async_k) {
...

    {
        ...
        for (unsigned int i = 0; i < async_k; i++) {
            ...
            if (sendBSz)
                ctx_list[i].getSendBuffer(
                    GPUDevice::device_malloc<T>(batch_sz * sendBSz));

            if (recvBSz)
                ctx_list[i].getRecvBuffer(
                    GPUDevice::device_malloc<T>(batch_sz * recvBSz));
...
template <typename T>
void dealloc_mpi_ctx(const ot::Mesh* pMesh,
                     std::vector<ot::AsyncExchangeContex>& ctx_list, int dof,
                     int async_k) {
    if (pMesh->getMPICommSizeGlobal() == 1 || !pMesh->isActive()) return;
    ...
    for (unsigned int i = 0; i < ctx_list.size(); i++) {
        ...
        if (sendBSz) GPUDevice::device_free<T>((T*)ctx_list[i].getSendBuffer());

        if (recvBSz) GPUDevice::device_free<T>((T*)ctx_list[i].getRecvBuffer());
