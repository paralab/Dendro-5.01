...
DEVICE_FUNC void radiative_bc(DEVICE_REAL* f_rhs, const DEVICE_REAL* f,
                              const DEVICE_REAL* dxf, const DEVICE_REAL* dyf,
                              const DEVICE_REAL* dzf, DEVICE_REAL f_falloff,
                              DEVICE_REAL f_asymptotic, const BlockGPU3D* blk,
                              DEVICE_UINT blk_id) {
...

    {
        ...
        const DEVICE_UINT k =
            GPUDevice::block_id_x() * GPUDevice::block_dim_y() +
        ...
        if (bflag & (1u << OCT_DIR_LEFT)) {
            if (i == pw) {
                f_rhs[globalIdx] =
                    -inv_r * (x * dxf[globalIdx] + y * dyf[globalIdx] +
                              z * dzf[globalIdx] +
...
template <unsigned int pw>
DEVICE_FUNC void radiative_bc_blk(DEVICE_REAL* f_rhs, const DEVICE_REAL* f,
                                  const DEVICE_REAL* dxf,
                                  const DEVICE_REAL* dyf,
                                  const DEVICE_REAL* dzf, DEVICE_REAL f_falloff,
                                  DEVICE_REAL f_asymptotic,
                                  const BlockGPU3D* blk, DEVICE_UINT blk_id) {
    const DEVICE_UINT BLK_ID    = blk_id;
    ...
    if (!(si < (actual_nx - pw) && sj < (actual_ny - pw))) return;
    ...
    if (bflag & (1u << OCT_DIR_BACK)) {
        ...
        if (sk == pw) {
            ...
            f_rhs[globalIdx] =
                -inv_r *
                (x * dxf[globalIdx] + y * dyf[globalIdx] + z * dzf[globalIdx] +
                 f_falloff * (f[globalIdx] - f_asymptotic));
        }
    }
...
DEVICE_FUNC void radiative_bc_pt(DEVICE_REAL* __restrict__ f_rhs, DEVICE_REAL f,
                                 DEVICE_REAL dxf, DEVICE_REAL dyf,
                                 DEVICE_REAL dzf, DEVICE_REAL f_falloff,
                                 DEVICE_REAL f_asymptotic,
                                 const BlockGPU3D* __restrict__ const blk) {
