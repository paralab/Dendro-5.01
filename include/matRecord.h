...
/**
 * @brief Matrix record class to be used while setting values in the global
 * matrix. This is based from the old dendro MatRecord class, where now it
 * supports  both petsc and non petsc matrices.
 *
...
class MatRecord {
   private:
    /**@brief local index of the node coresponding to the local row of the
     * matrix */
    unsigned int m_uiRowID;
    /**@brief local index of the node corresponding to the local column of the
     * matrix */
    unsigned int m_uiColID;
    /**@brief The degree of freedom (0-based) of the node corresponding to  the
     * row of the matrix.*/
   ...
   public:
     ...
     */
    MatRecord() {
        m_uiRowID = m_uiColID = m_uiRowDim = m_uiColDim =
    ...
    MatRecord(unsigned int rowID, unsigned int colID, unsigned int rowDim,
    ...
    bool operator==(MatRecord const &other) const {
        return ((m_uiRowID == other.getRowID()) &&
                (m_uiColID == other.getColID()) &&
                (m_uiVal == other.getMatVal()) &&
                (m_uiRowDim == other.getRowDim()) &&
                (m_uiColDim == other.getColDim()));
    ...
    friend std::ostream &operator<<(std::ostream &os, MatRecord const &re) {
        return (os << " row : " << re.getRowID() << " col: " << re.getColID()
                   << " rdim: " << re.getRowDim() << " cdim: " << re.getColDim()
                   << "val: " << re.getMatVal());
    }
};
