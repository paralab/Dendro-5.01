...
class MeshGPU {
   ...
   public:
    ...
    void unzip_dg(const ot::Mesh* const pMesh, const MeshGPU* const dptr_mesh,
                  const T* const dptr_in, T* const dptr_out,
                  unsigned int dof = 1, stream s = (stream)0);

    template <typename T, typename stream>
    void zip_dg(const ot::Mesh* const pMesh, const MeshGPU* const dptr_mesh,
                const T* const dptr_in, T* const dptr_out, unsigned int dof = 1,
                stream s = (stream)0);

    template <typename T, typename stream>
    void unzip_cg(const ot::Mesh* const pMesh, const MeshGPU* const dptr_mesh,
                  const T* const dptr_in, T* const dptr_out,
                  unsigned int dof = 1, stream s = (stream)0);

    template <typename T, typename stream>
    void zip_cg(const ot::Mesh* const pMesh, const MeshGPU* const dptr_mesh,
                const T* const dptr_in, T* const dptr_out, unsigned int dof = 1,
                stream s = (stream)0);

    template <typename T, typename stream>
    void read_from_ghost_dg_begin(ot::AsyncExchangeContex& ctx_host,
                                  ot::AsyncExchangeContex& ctx_device,
                                  const ot::Mesh* const pMesh,
                                  const MeshGPU* const dptr_mesh, T* dptr_vec,
                                  unsigned int dof = 1, stream s = (stream)0);

    template <typename T, typename stream>
    void read_from_ghost_dg_end(ot::AsyncExchangeContex& ctx_host,
                                ot::AsyncExchangeContex& ctx_device,
                                const ot::Mesh* const pMesh,
                                const MeshGPU* const dptr_mesh, T* dptr_vec,
                                unsigned int dof = 1, stream s = (stream)0);
    ...
    template <typename T, typename stream>
    void read_from_ghost_cg_begin(ot::AsyncExchangeContex& ctx_host,
                                  ot::AsyncExchangeContex& ctx_device,
                                  const ot::Mesh* const pMesh,
                                  const MeshGPU* const dptr_mesh, T* dptr_vec,
                                  unsigned int dof = 1, stream s = (stream)0);
    ...
    template <typename T, typename stream>
    void read_from_ghost_cg_end(ot::AsyncExchangeContex& ctx_host,
                                ot::AsyncExchangeContex& ctx_device,
                                const ot::Mesh* const pMesh,
                                const MeshGPU* const dptr_mesh, T* dptr_vec,
                                unsigned int dof = 1, stream s = (stream)0);
