...
class Key : public ot::TreeNode {
   protected:
    ...
    std::vector<unsigned int>
        m_uiStencilIndexWithDirection;  // first 3 bits contains the direction
                                        // (OCT_DIR_LEFT,OCT_DIR_RIGHT etc) and
                                        // the other 5 bit contains the stencil
                                        // index.  Hence k_s max is 31.
   ...
   public:
    ...
    Key(unsigned int px, unsigned int py, unsigned int pz, unsigned int plevel,
        unsigned int pDim,
        unsigned int
            pMaxDepth) /*: ot::TreeNode(px,py,pz,plevel,pDim,pMaxDepth)*/
    {
        m_uiX            = px;
        m_uiY            = py;
        m_uiZ            = pz;
    ...
    {
        // std::cout<<"shared_ptr_ copy begin0 : "<<*this<< " =
        // "<<node<<std::endl; std::cout<<"shared_ptr_ copy begin1 : "<<*this<<"
        // owner Size: "<<m_uiOwnerList.get()->size()<< " = "<<node<<" node
        // ownerSize : "<<node.m_uiOwnerList.get()->size()<<std::endl;
        m_uiX            = node.getX();
        m_uiY            = node.getY();
        m_uiZ            = node.getZ();
        m_uiLevel        = node.getFlag();
    ...
    inline void addStencilIndexAndDirection(unsigned int index,
    ...
    inline std::vector<unsigned int>* getStencilIndexDirectionList() {
