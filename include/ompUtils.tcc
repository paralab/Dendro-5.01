...
#pragma omp parallel for
    for (int i = 0; i < p; i++) {
        for (int j = 0; j < n; j++) {
            ...
            split[indx]     = A_[indx1];
            split_size[indx] =
                indx1 + (std::lower_bound(B_, B_last, split[indx], comp) - B_);

            indx1 = (indx * N2) / (p * n);
            indx += p * n;
            split[indx] = B_[indx1];
            split_size[indx] =
                indx1 + (std::lower_bound(A_, A_last, split[indx], comp) - A_);
        }
    }
...
#pragma omp parallel for
    for (int i = 1; i < p; i++) {
        ...
        j = (std::lower_bound(&split_size[p * n], &split_size[p * n * 2],
                              req_size, std::less<_DiffType>()) -
             &split_size[p * n]) +
            p * n;
...
#pragma omp parallel for
    ...
    for (int j = 1; j < p; j = j * 2) {
        for (int i = 0; i < p; i = i + 2 * j) {
            if (i + j < p) {
                omp_par::merge(A_ + split[i], A_ + split[i + j],
                               A_ + split[i + j],
                               A_ + split[(i + 2 * j <= p ? i + 2 * j : p)],
                               B_ + split[i], p, comp);
            } else {
#pragma omp parallel for
...
void omp_par::scan(T* A, T* B, I cnt) {
    int p = omp_get_max_threads();
    if (cnt < 100 * p) {
        for (I i = 1; i < cnt; i++) B[i] = B[i - 1] + A[i - 1];
        return;
    }
...
#pragma omp parallel for
    for (int i = 0; i < p; i++) {
        ...
        if (i == p - 1) end = cnt;
        if (i != 0) B[start] = 0;
        for (I j = start + 1; j < end; j++) B[j] = B[j - 1] + A[j - 1];
    }
...
#pragma omp parallel for
    for (int i = 1; i < p; i++) {
        ...
        if (i == p - 1) end = cnt;
        T sum_ = sum[i];
        for (I j = start; j < end; j++) B[j] += sum_;
    }
    delete[] sum;
}
