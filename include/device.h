...
class Device {
   public:
    ...
    static inline void check_error(T result) {
        return LDevice::template check_error<T>(result);
    }

    static inline void check_last_error() {
        return LDevice::template check_last_error();
    }
    ...
    template <typename T>
    static void host_to_device(const T* const hptr, T* const dptr,
                               unsigned int n) {
    ...
    template <typename T>
    static void device_to_host(T* const hptr, const T* const dptr,
                               unsigned int n) {
        return LDevice::template device_to_host<T>(hptr, dptr, n);
    }

    template <typename T, typename device_stream>
    static void host_to_device_async(const T* const hptr, T* const dptr,
                                     unsigned int n,
                                     device_stream s = (device_stream)0) {
        return LDevice::template host_to_device_async<T, device_stream>(
            hptr, dptr, n, s);
    }

    template <typename T, typename device_stream>
    static void device_to_host_async(T* const hptr, const T* const dptr,
                                     unsigned int n,
                                     device_stream s = (device_stream)0) {
        return LDevice::template device_to_host_async<T, device_stream>(
            hptr, dptr, n, s);
    }
    ...
    static void stream_synchronize(device_stream s = (device_stream)0) {
        return LDevice::template stream_synchronize<device_stream>(s);
    }
};
...
class DeviceCUDA {
   public:
    ...
    static void host_to_device(const T* const hptr, T* const dptr,
                               unsigned int n) {
        check_error(
            cudaMemcpy(dptr, hptr, sizeof(T) * n, cudaMemcpyHostToDevice));
        return;
    }
    ...
    static void device_to_host(T* const hptr, const T* const dptr,
                               unsigned int n) {
        check_error(
            cudaMemcpy(hptr, dptr, sizeof(T) * n, cudaMemcpyDeviceToHost));
        return;
    }
    ...
    static void host_to_device_async(const T* const hptr, T* const dptr,
                                     unsigned int n,
                                     device_stream s = (device_stream)0) {
        check_error(cudaMemcpyAsync(dptr, hptr, sizeof(T) * n,
    ...
    static void device_to_host_async(T* const hptr, const T* const dptr,
                                     unsigned int n,
                                     device_stream s = (device_stream)0) {
        check_error(cudaMemcpyAsync(hptr, dptr, sizeof(T) * n,
...
template <typename T>
GLOBAL_FUNC void axpy_cu(int n, T a, const T* __restrict__ const in,
                         T* __restrict__ const out) {
    const int t_id   = threadIdx.x + blockDim.x * blockIdx.x;
    const int stride = blockDim.x * gridDim.x;
    for (int i = t_id; i < n; i += stride) out[i] += a * in[i];
}

template <typename T>
GLOBAL_FUNC void axpy_cu_2d(unsigned int lb, unsigned int le,
                            unsigned int szpdof, T a,
                            const T* __restrict__ const in,
                            T* __restrict__ const out) {
    const int t_id   = threadIdx.x + blockDim.x * blockIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
static inline int cuda_mpi_aware_init() {
    ...
    if ((local_rank_str = getenv("OMPI_COMM_WORLD_LOCAL_RANK")) != NULL) {
        ...
        std::cout
            << "local rank extraction through env. vars failed for cuda init";
        exit(0);
    }
