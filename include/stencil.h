 ...
 *@brief Contains the implementation of the structure stencil which is generic
 *enough to specify,a given stencil.
 */
//
...
struct Stencil {
   private:
    ...
    /** offset location of the stencil , which denotes the 0th point
     * coefficient. */
   ...
   public:
    ...
    Stencil(T* coeff, int n, StencilDirection pDir) {
        assert(n == length);
        for (unsigned int k = 0; k < length; k++) m_uiS[k] = coeff[k];
    ...
    inline StencilDirection getStencilDirection() const {
