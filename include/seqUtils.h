...
/**
  @brief Finds the index of the smallest upper bound of the search key in the
  array.
  ...
  */
template <typename T>
int UpperBound(unsigned int nelem, const T* arr, unsigned int startIdx,
               const T& key);

/**
  @brief Finds the index of the greatest lower bound of the search key in the
  array. The implementation uses a simple modification of the binary search
  algorithm.
  @author Rahul Sampath
  @param arr A sorted array
  @param key The search key
  @param retIdx The index of the position of the last element in the array <=
  key
  @param leftIdx If this is not NULL, then the search will be limited to
  elements at positions >= *leftIdx
  @param rightIdx if this is not NULL, then the search will be limited to
  elements at positions <= *rightIdx
  @return 'true' if the search was successful
  */
template <typename T>
bool maxLowerBound(const std::vector<T>& arr, const T& key,
                   unsigned int& retIdx, unsigned int* leftIdx,
