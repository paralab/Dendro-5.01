 ...
 * @breif Contains SFC based sorting functionality (finalized) for Morton and
 * Hilbert Curve.
...
std::vector<double>
    stats;  // contains the last run of the SFC::parSort::SFC_TreeSort.
 ...
 *
 * (2^0 bit location) TS_REMOVE_DUPLICATES : if selected ensures that the output
 * of the algorithm is sorted and unique. (2^1 bit location) TS_CONSTRUCT_OCTREE
 * : if selected ensures that the output of the algorithm is sorted and
 * completed octree (2^2 bit location) TS_BALANCE_OCTREE    : if selected
 * ensures that the output of the algorithm is sorted completed and balanced.
 *
 *
 * */
...
#define TS_BALANCE_OCTREE \
    4  // which ensures that balance octree cannote be called without construct
       // octree function.
...
struct BucketInfo {
    ...
    BucketInfo(unsigned char p_rot_id, unsigned char p_lev, DendroIntL p_begin,
               DendroIntL p_end) {
        rot_id = p_rot_id;
        lev    = p_lev;
        begin  = p_begin;
...
struct OctreeComp {
    ...
    inline bool operator()(const T& first, const T& other) const {  //$
        ...
        /*        return (((first.getLevel()!=other.getLevel()) &&
           (first.getLevel()>other.getLevel()))
                       || ((first.getLevel()==other.getLevel()) &&
           ((first.getX() != other.getX())) && (first.getX() < other.getX()))
                       || ((first.getLevel()==other.getLevel()) &&(first.getX()
           == other.getX()) && (first.getY() != other.getY()) && (first.getY() <
           other.getY()))
                       || ((first.getLevel()==other.getLevel()) && (first.getX()
           == other.getX()) && (first.getY() == other.getY())  && (first.getZ()
           < other.getZ())));*/

        /*return (((first.getLevel()==other.getLevel()) ||
           (first.getLevel()<other.getLevel()))
                && ((first.getLevel()!=other.getLevel()) || ((first.getX() ==
           other.getX())) || (first.getX() > other.getX()))
                && ((first.getLevel()!=other.getLevel()) || (first.getX() !=
           other.getX()) || (first.getY() == other.getY()) || (first.getY() >
           other.getY()))
                && ((first.getLevel()!=other.getLevel()) || (first.getX() !=
        ...
        a2 = (((uint128_t)other.getLevel()) << 96u) |
             (((uint128_t)other.getX()) << 64u) |
             (((uint128_t)other.getY()) << 32u) | (((uint128_t)other.getZ()));
        return a1 < a2;

        /*
                if(first.getLevel()!=other.getLevel())
                {
                  if(state!=(first.getLevel()>other.getLevel())) std::cout<<"
           lev first"<<first<<" other: "<<other<<std::endl; return
           (first.getLevel()>other.getLevel()); }else
                {
                  if(first.getX()!=other.getX())
                  {
                    if(state!=(first.getX()<other.getX())) std::cout<<" x
           first"<<first<<" other: "<<other<<std::endl; return
           first.getX()<other.getX(); }else
                  {
                     if(first.getY()!=other.getY())
                     {
                       if(state!=(first.getY()<other.getY())) std::cout<<" y
           first"<<first<<" other: "<<other<<std::endl; return
           first.getY()<other.getY(); }else
                     {
                       if(state!=(first.getZ()<other.getZ())) std::cout<<" z
           first"<<first<<" other: "<<other<<std::endl; return
           first.getZ()<other.getZ();
                     }
                  }
                }*/
    }
...
struct OctreeHash {
    inline uint128_t operator()(const T& first) const {
        /*        uint64_t answer1 = 0;
                answer1 |= splitBy3(node.getX()) | splitBy3(node.getY()) << 1 |
           splitBy3(node.getZ()) << 2; return answer1;*/
        ...
        return a1;
        // return
        // (((uint64_t)h4<<48u)|((uint64_t)h3<<32u)|((uint64_t)h2<<16u)|((uint64_t)h1));
    }
};
...
/**
 * @author Milinda Fernando
 * @breif Bottom up construction of the auxilary octants which will be needed in
 * the balancing stage.
 * * Assumes that ,
 ...
 * */
template <typename T>
void SFC_treeSort(T* pNodes, DendroIntL n, std::vector<T>& pOutSorted,
                  std::vector<T>& pOutConstruct, std::vector<T>& pOutBalanced,
                  unsigned int pMaxDepthBit, unsigned int pMaxDepth, T& parent,
                  unsigned int rot_id, unsigned int k, unsigned int options);

/**
 * @author Milinda Fernando
 * @breif Sequential Bucketing function which will be needed in adaptive load
 * balancing and parallel tree sort implmentation.
 * */

template <typename T>
inline void SFC_bucketing(T* pNodes, int lev, unsigned int maxDepth,
 ...
 * @param[in] pMaxDepthBit initial value should be max depth of the tree. this
 *is automatically decrease as we traverse deper in the tree.
...
template <typename T>
void SFC_treeSortLocalOptimal(T* pNodes, DendroIntL n,
                              unsigned int pMaxDepthBit, unsigned int pMaxDepth,
                              T& parent, unsigned int rot_id, bool minimum,
                              T& optimal);
...
inline void SFC_bottomUpBalOctantCreation(std::vector<T>& pNodes) {  //$
    ...
    std::pair<typename std::set<T, OctreeComp<T>>::iterator, bool>
        hint[neighbourCount];
    ...
    /* std::unordered_set<T,OctreeHash<T>> auxOct(pNodes.begin(),pNodes.end());
     std::pair<typename std::unordered_set<T,OctreeHash<T>>::iterator,bool>
     hint;*/
    ...
    for (unsigned int w = 0; w < pNodes.size(); ++w) {
        ...
        if (hintParent.second) {
            ...
            // std::cout<<"aux Size: "<<auxOct.size()<<" pNodes size:
...
void SFC_treeSort(T* pNodes, DendroIntL n, std::vector<T>& pOutSorted,
    ...
    for (DendroIntL i = 0; i < n; ++i) {
        cnum = (lev < pNodes[i].getLevel())
                   ? 1 + ((((pNodes[i].getZ() >> pMaxDepthBit) & 1u) << 2u) |
                          (((pNodes[i].getY() >> pMaxDepthBit) & 1u) << 1u) |
                          ((pNodes[i].getX() >> pMaxDepthBit) & 1u))
                   : 0;
        count[cnum + 1]++;
    }
    ...
    if (live > 0) {
        ...
        for (DendroIntL i = 0; i < n; ++i) {
            cnum =
                (lev < unsorted[live].getLevel())
                    ? ((((unsorted[live].getZ() >> pMaxDepthBit) & 1u) << 2u) |
            ...
            (loc[cnum] == n) ? unsorted[live] = pNodes[loc[cnum] - 1]
    ...
    if ((options == TS_SORT_ONLY) || (options == TS_REMOVE_DUPLICATES)) {
        if (pMaxDepthBit > 0) {
            ...
            for (unsigned int i = 1; i < (NUM_CHILDREN + 1); i++) {
                ...
                (i > 1)
                    ? cnum_prev =
                          ((rotations[ROTATION_OFFSET * rot_id + i - 2] - '0') +
                           2)
                    : cnum_prev = 1;
                ...
                if (numElements > k) {
                    ...
                    if (numElements)
                        SFC_treeSort(pNodes + count[cnum_prev], numElements,
                                     pOutSorted, pOutConstruct, pOutBalanced,
                                     (pMaxDepthBit), pMaxDepth, temp, rotation,
                                     k, options);
                }
            }
        }
        ...
        if (pMaxDepthBit > MAXDEAPTH_LEVEL_DIFF) {
            ...
            for (unsigned int i = 1; i < (NUM_CHILDREN + 1); i++) {
                ...
                if (numElements > k) {
                    ...
                    SFC_treeSort(pNodes + count[cnum_prev], numElements,
                                 pOutSorted, pOutConstruct, pOutBalanced,
                                 (pMaxDepthBit), pMaxDepth, temp, rotation, k,
                                 options);
                    ...
                    if (options & TS_BALANCE_OCTREE) {
                        // generate all the neighbours and add them to unordered
                        // map. use pOutBalaced to push balanced octree.
    ...
    if (lev == 0) {
        // !!!! Note: Please note that all the code here executed only once. In
        // the final stage of the recursion.
        ...
        if ((options & TS_REMOVE_DUPLICATES)) {
            ...
            // Note: This is executed only once. In the final stage of the
            // recursion. Do the remove duplicates here.
            if (n >= 1) {
                std::vector<T> tmp(n);
                T* tmpPtr            = (&(*(tmp.begin())));
                ...
                for (unsigned int i = 1; i < n; i++) {
                    if (/*(!tmpPtr[tmpSize-1].isAncestor(pNodes[i])) &*/ (
                        tmpPtr[tmpSize - 1] !=
                        pNodes[i])) {  // It is efficient to do this rather than
                                       // marking all the elements in sorting.
                                       // (Which will cause a performance
                                       // degradation. )
                ...
                // Assumes that we have removed all the duplicates after the
                // first iteration.
        ...
        if (options & TS_BALANCE_OCTREE) {
            ...
            // std::cout<<"balOCt: before Aux octants:
            // "<<pOutBalanced.size()<<std::endl;
#ifdef PROFILE_TREE_SORT
            t1 = std::chrono::high_resolution_clock::now();  // MPI_Wtime();
#endif
            /*int rank;
            ...
            // std::cout<<"bal with aux octants:
            // "<<pOutBalanced.size()<<std::endl;
            SFC::seqSort::SFC_treeSort(
                &(*(pOutBalanced.begin())), pOutBalanced.size(), tmpSorted,
                tmpConstruct, tmpBalanced, pMaxDepth, pMaxDepth, root, 0, k, 2);
...
inline void SFC_bucketing(T* pNodes, int lev, unsigned int maxDepth,
    ...
    // std::cout<<rank<<" Bucketing : begin: "<<begin<<" end: "<<end<<" lev:
    // "<<lev<<" rotaion: "<<(int)rot_id<<std::endl;
    ...
    live--;
    // if(live <0) std::cout<<"live count overflow. " <<std::endl; // Note: This
    // could be a potential bug.
    for (DendroIntL i = begin; i < end; ++i) {
        // cnum = (lev < unsorted[live].getLevel()) ? (((((unsorted[live].getZ()
        // & (1u << mid_bit)) >> mid_bit) << 2u) | (((unsorted[live].getY() &
        // (1u << mid_bit)) >> mid_bit) << 1u) | ((unsorted[live].getX() & (1u
        // << mid_bit)) >> mid_bit)))+ 1: 0 ;
        cnum = (lev < unsorted[live].getLevel())
                   ? ((((unsorted[live].getZ() >> mid_bit) & 1u) << 2u) |
                      (((unsorted[live].getY() >> mid_bit) & 1u) << 1u) |
                      ((unsorted[live].getX() >> mid_bit) & 1u)) +
                         1
                   : 0;
        if (loc[cnum] < count[cnum + 1]) {
            pNodes[loc[cnum]++] = unsorted[live];
        ...
        /*if(live<0)
        {
            std::cout<<"begin: "<<begin<<" end: "<<end<<" i: "<<i<<" loc[0]:
        "<<loc[0]<<" live : "<<live<<std::endl;
        }*/
    }
...
void SFC_treeSortLocalOptimal(T* pNodes, DendroIntL n,
    ...
    if (n == 0 && minimum) {
        ...
        optimal =
            T(((1u << m_uiMaxDepth) - 1), ((1u << m_uiMaxDepth) - 1),
              ((1u << m_uiMaxDepth) - 1), m_uiMaxDepth, m_uiDim, m_uiMaxDepth);
        return;
    }
    ...
    if ((pMaxDepthBit) && ((nNodeEnd - nNodeBegin) > 1)) {
        ...
        if (splitterNodes[hindex] != splitterNodes[hindexN])
            SFC_treeSortLocalOptimal(
                pNodes + splitterNodes[hindex],
                (splitterNodes[hindexN] - splitterNodes[hindex]),
                (pMaxDepthBit - 1), pMaxDepth, parent, index, minimum, optimal);
        else {
            // std::cout<<"Calling Seq: sort at Lev: "<<lev<<"size :
            // "<<n<<std::endl;
            std::vector<T> tmpVec;
            SFC::seqSort::SFC_treeSort(pNodes, n, tmpVec, tmpVec, tmpVec,
                                       pMaxDepth, pMaxDepth, parent,
...
void SFC_treeSortLocalOptimal(T* pNodes, DendroIntL n,
    ...
    for (unsigned int i = 0; i < NUM_CHILDREN; i++) {
        ...
        (min_b == (NUM_CHILDREN - 1))
            ? hindexN = min_b + 1
            : hindexN =
                  (rotations[2 * NUM_CHILDREN * rot_id + min_b + 1] - '0');
    ...
    for (unsigned int i = 0; i < (NUM_CHILDREN); i++) {
        ...
        (max_b == (NUM_CHILDREN - 1))
            ? hindexN = max_b + 1
            : hindexN =
                  (rotations[2 * NUM_CHILDREN * rot_id + max_b + 1] - '0');
        if (splitterNodes[hindex] != splitterNodes[hindexN]) break;
        max_b--;
    }
 ...
 * */
template <typename T>
void SFC_treeSort(std::vector<T>& pNodes, std::vector<T>& pOutSorted,
                  std::vector<T>& pOutConstruct, std::vector<T>& pOutBalanced,
                  double loadFlexibility, unsigned int pMaxDepth, T& parent,
                  unsigned int rot_id, unsigned int k, unsigned int options,
                  unsigned int sf_k, MPI_Comm pcomm);

/* template <typename T>
 void SFC_PartitionW(std::vector<T>&pNodes,double loadFlexibility, unsigned int
...
inline void SFC_SplitterFix(std::vector<T>& pNodes, unsigned int pMaxDepth,
    ...
    if (npes > NUM_NPES_THRESHOLD) {
        ...
        unsigned int firstSplitLevel =
            std::ceil(binOp::fastLog2(a) / (double)dim);
        ...
        // maintain the splitters and buckets for splitting for further
        // splitting.
        std::vector<DendroIntL> bucketCounts;
        std::vector<BucketInfo<T>>
            bucketInfo;  // Stores the buckets info of the buckets where the
                         // initial buckets was splitted.
        ...
        while (numLeafBuckets < totalNumBuckets) {
            ...
            SFC::seqSort::SFC_bucketing(&(*(pNodes.begin())), tmp.lev,
                                        pMaxDepth, tmp.rot_id, tmp.begin,
            ...
            for (int i = 0; i < NUM_CHILDREN; i++) {
                ...
                if (i == (NUM_CHILDREN - 1))
                    ...
                    hindexN =
                        (rotations[2 * NUM_CHILDREN * tmp.rot_id + i + 1] -
                         '0');
                assert(spliterstemp[hindex] <= spliterstemp[hindexN]);
                index = HILBERT_TABLE[NUM_CHILDREN * tmp.rot_id + hindex];

                BucketInfo<T> child(index, (tmp.lev + 1), spliterstemp[hindex],
                                    spliterstemp[hindexN]);
                nodeStack.push_back(child);

                if (tmp.lev == (firstSplitLevel - 1)) {
                    BucketInfo<T> bucket(index, (tmp.lev + 1),
                                         spliterstemp[hindex],
                                         spliterstemp[hindexN]);
                    bucketCounts.push_back(
                        (spliterstemp[hindexN] - spliterstemp[hindex]));
        ...
        par::Mpi_Allreduce<DendroIntL>(&(*(bucketCounts.begin())),
                                       &(*(bucketCounts_g.begin())),
        ...
        for (int k = 1; k < bucketCounts_g.size(); k++) {
            bucketCounts_gScan[k] =
                bucketCounts_gScan[k - 1] + bucketCounts_g[k];
        }

#ifdef DEBUG_TREE_SORT
        if (!rank) {
            for (int i = 0; i < totalNumBuckets; i++) {
                // std::cout << "Bucket count G : " << i << " : " <<
                // bucketCounts_g[i] << std::endl;
                std::cout << "Bucket initial count scan G : " << i << " : "
        ...
        for (int i = 0; i < a - 1; i++) {
            ...
            DendroIntL toleranceLoadBalance =
                ((i + 1) * globalSz / a - i * globalSz / a) * loadFlexibility;

            unsigned int loc =
                (std::lower_bound(bucketCounts_gScan.begin(),
                                  bucketCounts_gScan.end(), idealLoadBalance) -
                 bucketCounts_gScan.begin());
            // std::cout<<rank<<" Searching: "<<idealLoadBalance<<"found:
            // "<<loc<<std::endl;

            if (abs(bucketCounts_gScan[loc] - idealLoadBalance) >
                toleranceLoadBalance) {
                if (splitBucketIndex.empty() || splitBucketIndex.back() != loc)
                    splitBucketIndex.push_back(loc);
                /*if(!rank)
                  std::cout<<"Bucket index :  "<<loc << " Needs a split
                  "<<std::endl;*/
            } else {
        ...
        for (int i = 0; i < splitBucketIndex.size() - 1; i++) {
            assert(pNodes[bucketSplitter[splitBucketIndex[i]]] <
                   pNodes[bucketSplitter[splitBucketIndex[i + 1]]]);
        }
#endif
        ...
        while (!splitBucketIndex.empty()) {
            ...
            if (bucketInfo[splitBucketIndex[0]].lev < pMaxDepth) {
                BucketInfo<T> tmp;
                // unsigned int numSplitBuckets = NUM_CHILDREN *
                // splitBucketIndex.size();

#ifdef DEBUG_TREE_SORT
                if (!rank)
                    for (int i = 0; i < splitBucketIndex.size(); i++)
                        std::cout << "Splitter Bucket Index: " << i << "  : "
...
#ifdef DEBUG_TREE_SORT
                    if (!rank)
                        std::cout << "Splitting Bucket index "
                                  << splitBucketIndex[k] << std::endl;
#endif

                    tmp = bucketInfo[splitBucketIndex[k]];
                    SFC::seqSort::SFC_bucketing(
                        &(*(pNodes.begin())), tmp.lev, pMaxDepth, tmp.rot_id,
                        tmp.begin, tmp.end, splitterTemp);

                    for (int i = 0; i < NUM_CHILDREN; i++) {
                        hindex = (rotations[2 * NUM_CHILDREN * tmp.rot_id + i] -
                                  '0');
                        if (i == (NUM_CHILDREN - 1))
                            hindexN = i + 1;
                        else
                            hindexN = (rotations[2 * NUM_CHILDREN * tmp.rot_id +
                                                 i + 1] -
                                       '0');

                        // newBucketCounts[NUM_CHILDREN * k + i] =
                        // (splitterTemp[hindexN] - splitterTemp[hindex]);
                        newBucketCounts.push_back(
                            (splitterTemp[hindexN] - splitterTemp[hindex]));

                        index =
                            HILBERT_TABLE[NUM_CHILDREN * tmp.rot_id + hindex];
                        BucketInfo<T> bucket(index, (tmp.lev + 1),
                                             splitterTemp[hindex],
                                             splitterTemp[hindexN]);
                        //                          newBucketInfo[NUM_CHILDREN *
                        //                          k + i] = bucket;
                        //                          newBucketSplitters[NUM_CHILDREN
                        //                          * k + i] =
                        //                          splitterTemp[hindex];
                        newBucketInfo.push_back(bucket);
                        newBucketSplitters.push_back(splitterTemp[hindex]);
#ifdef DEBUG_TREE_SORT
                        assert(pNodes[splitterTemp[hindex]] <=
                               pNodes[splitterTemp[hindexN]]);
#endif
                    }
                }

                newBucketCounts_g.resize(newBucketCounts.size());
                par::Mpi_Allreduce(&(*(newBucketCounts.begin())),
                                   &(*(newBucketCounts_g.begin())),
                                   newBucketCounts.size(), MPI_SUM, comm);
                // MPI_Allreduce(&newBucketCounts[0], &newBucketCounts_g[0],
                // newBucketCounts.size(), MPI_LONG_LONG, MPI_SUM, comm);

#ifdef DEBUG_TREE_SORT
                for (int i = 0; i < newBucketSplitters.size() - 1; i++) {
                    assert(pNodes[newBucketSplitters[i]] <=
                           pNodes[newBucketSplitters[i + 1]]);
                }
#endif
                ...
                for (int k = 0; k < splitBucketIndex.size(); k++) {
                    ...
                    (k == 0) ? bucketBegin = 0
                             : bucketBegin = splitBucketIndex[k - 1] + 1;
                    ...
                    tmp = bucketInfo[splitBucketIndex[k]];
                    for (int i = 0; i < NUM_CHILDREN; i++) {
                        bucketCount_gMerge.push_back(
                            newBucketCounts_g[NUM_CHILDREN * k + i]);
                        bucketInfoMerge.push_back(
                            newBucketInfo[NUM_CHILDREN * k + i]);
                        bucketSplitterMerge.push_back(
                            newBucketSplitters[NUM_CHILDREN * k + i]);
                    }

                    if (k == (splitBucketIndex.size() - 1)) {
                        for (int i = splitBucketIndex[k] + 1;
                             i < bucketCounts_g.size(); i++) {
                            bucketCount_gMerge.push_back(bucketCounts_g[i]);
                            bucketInfoMerge.push_back(bucketInfo[i]);
                            bucketSplitterMerge.push_back(bucketSplitter[i]);
                        }
                ...
                for (int i = 0; i < bucketSplitter.size() - 2; i++) {
                    std::cout << "Bucket Splitter : " << bucketSplitter[i]
                              << std::endl;
                    assert(bucketSplitter[i + 1] != (pNodes.size()) &&
                           pNodes[bucketSplitter[i]] <=
                               pNodes[bucketSplitter[i + 1]]);
                }
#endif
                ...
                for (unsigned int i = 0; i < a - 1; i++) {
                    idealLoadBalance +=
                        ((i + 1) * globalSz / a - i * globalSz / a);
                    DendroIntL toleranceLoadBalance =
                        (((i + 1) * globalSz / a - i * globalSz / a) *
                         loadFlexibility);
                    unsigned int loc =
                        (std::lower_bound(bucketCounts_gScan.begin(),
                                          bucketCounts_gScan.end(),
                                          idealLoadBalance) -
                         bucketCounts_gScan.begin());

                    if (abs(bucketCounts_gScan[loc] - idealLoadBalance) >
                        toleranceLoadBalance) {
                        if (splitBucketIndex.empty() ||
                            splitBucketIndex.back() != loc)
                            splitBucketIndex.push_back(loc);
                ...
                for (unsigned int i = 0; i < a - 1; i++) {
                    idealLoadBalance +=
                        ((i + 1) * globalSz / a - i * globalSz / a);
                    // DendroIntL toleranceLoadBalance = ((i + 1) * globalSz /
                    // npes - i * globalSz / npes) * loadFlexibility;
                    unsigned int loc =
                        (std::lower_bound(bucketCounts_gScan.begin(),
                                          bucketCounts_gScan.end(),
                                          idealLoadBalance) -
        ...
        for (unsigned int k = 0; k < p_mod_a; k++) blockCounts[k] = (b + 1);
        ...
        for (unsigned int k = 0; k < a; k++) {
            if ((rank >= blockOffset[k]) &&
                rank < (blockOffset[k] + blockCounts[k])) {
                (blockOffset[k] != 0) ? blk_id = rank % blockOffset[k]
                                      : blk_id = rank;
            }
        }
        ...
        /* if(!rank)
             for(unsigned int k=0;k<a;k++)
                 std::cout<<" bck count: "<<k<<" val: "<<blockCounts[k]<<"
           offsets: "<<blockOffset[k]<<std::endl;*/
        ...
        (rank < (b + 1) * p_mod_a)
            ? owner_blk = rank / (b + 1)
            : owner_blk = (((rank - (b + 1) * p_mod_a) / b) + p_mod_a);
        ...
        /*if(rank==4)
            for(unsigned int p=0;p<a;p++)
                std::cout<<" sendCnt["<<p<<" ]: "<<sendCnt[p]<<" spliter:
           ["<<p<<" ] : value: "<<localSplitterTmp[p]<<std::endl;*/
        ...
        for (unsigned int k = 0; k < a; k++) {
            ...
            if ((rank < (b + 1) * p_mod_a) && blockCounts[k] == (b + 1)) {
                ...
                if (k > 0)
                    send_offset[(blockOffset[k] + blk_id)] =
                        localSplitterTmp[k - 1];
                send_count[(blockOffset[k] + blk_id)] = sendCnt[k];
            } else {
                assert((blockOffset[k] + (blk_id % b)) < npes);
                if (k > 0)
                    send_offset[(blockOffset[k] + (blk_id % b))] =
                        localSplitterTmp[k - 1];
                send_count[(blockOffset[k] + (blk_id % b))] = sendCnt[k];
            }
        }

        par::Mpi_Alltoall(&(*(send_count.begin())), &(*(recv_count.begin())), 1,
                          comm);

        recv_offset[0] = 0;
        omp_par::scan(&(*(recv_count.begin())), &(*(recv_offset.begin())),
                      npes);
        ...
        par::Mpi_Alltoallv_sparse(
            &(*(pNodes.begin())), (int*)&(*(send_count.begin())),
            (int*)&(*(send_offset.begin())), &(*(recvBuf.begin())),
            (int*)&(*(recv_count.begin())), (int*)&(*(recv_offset.begin())),
            comm);
        (owner_blk > 0)
            ? recvBuf.insert(recvBuf.end(),
                             pNodes.begin() + localSplitterTmp[owner_blk - 1],
                             pNodes.begin() + localSplitterTmp[owner_blk - 1] +
                                 sendCnt[owner_blk])
            : recvBuf.insert(recvBuf.end(), pNodes.begin(),
                             pNodes.begin() + sendCnt[owner_blk]);
...
void SFC_treeSort(std::vector<T>& pNodes, std::vector<T>& pOutSorted,
    ...
    if (npes == 1) {
        ...
        SFC::seqSort::SFC_treeSort(
            &(*(pNodes.begin())), pNodes.size(), pOutSorted, pOutConstruct,
            pOutBalanced, pMaxDepth, pMaxDepth, parent, rot_id, k, options);
        return;
    }
    ...
    if (npes > NUM_NPES_THRESHOLD) {
        SF_Stages =
            std::ceil((binOp::fastLog2(npes) / (double)binOp::fastLog2(sf_k))) -
            1;
        ...
        for (int i = 0; i < SF_Stages; i++) {
            ...
            // if(!rank) std::cout<<"sf_stage: "<<i<<"of "<<SF_Stages<<" sf_comm
            // size "<<sf_npes<<std::endl;
            ...
            if (sf_npes > NUM_NPES_THRESHOLD) {
                ...
                SFC_SplitterFix(pNodes, pMaxDepth, loadFlexibility, sf_k,
                                SF_comm, &sf_comms[i]);
...
#ifdef PROFILE_TREE_SORT
    splitter_fix_all = std::chrono::duration_cast<std::chrono::milliseconds>(
    ...
    unsigned int firstSplitLevel =
        std::ceil(binOp::fastLog2(npes) / (double)(dim));
    unsigned int totalNumBuckets = 1u << (dim * firstSplitLevel);
    DendroIntL localSz           = pNodes.size();
    DendroIntL globalSz          = 0;
    MPI_Allreduce(&localSz, &globalSz, 1, MPI_LONG_LONG, MPI_SUM, comm);
    // if(!rank) std::cout<<"First Split Level : "<<firstSplitLevel<<" Total
    // number of buckets: "<<totalNumBuckets <<std::endl; if(!rank)
    ...
    std::vector<DendroIntL> bucketCounts;
    std::vector<BucketInfo<T>>
        bucketInfo;  // Stores the buckets info of the buckets where the initial
                     // buckets was splitted.
    std::vector<DendroIntL> bucketSplitter;
    ...
    unsigned int begin_loc      = 0;
    /*
     * We need to split the array into buckets until it is we number of buckets
     * is slightly higher than the number of processors.
     */
    ...
    while (numLeafBuckets < totalNumBuckets) {
        ...
        SFC::seqSort::SFC_bucketing(&(*(pNodes.begin())), tmp.lev, pMaxDepth,
                                    tmp.rot_id, tmp.begin, tmp.end,
                                    spliterstemp);
        ...
        for (int i = 0; i < NUM_CHILDREN; i++) {
            ...
            if (i == (NUM_CHILDREN - 1))
                ...
                hindexN =
                    (rotations[2 * NUM_CHILDREN * tmp.rot_id + i + 1] - '0');
            assert(spliterstemp[hindex] <= spliterstemp[hindexN]);
            index = HILBERT_TABLE[NUM_CHILDREN * tmp.rot_id + hindex];
            ...
            if (tmp.lev == (firstSplitLevel - 1)) {
                BucketInfo<T> bucket(index, (tmp.lev + 1), spliterstemp[hindex],
                                     spliterstemp[hindexN]);
                bucketCounts.push_back(
                    (spliterstemp[hindexN] - spliterstemp[hindex]));
                bucketSplitter.push_back(spliterstemp[hindex]);
    ...
    // #ifdef DEBUG_TREE_SORT
    // std::cout<<rank<<" Initial Splitter Calculation ended
    // "<<numLeafBuckets<<std::endl; #endif
    ...
    /* if(!rank)
         std::cout<<"All Reduction Time for : "<<npes<<"  :
       "<<allReduceTime<<std::endl;*/

    // MPI_Allreduce(&bucketCounts[0], &bucketCounts_g[0], bucketCounts.size(),
    // MPI_LONG_LONG, MPI_SUM, comm); std::cout<<"All to all ended.
    // "<<rank<<std::endl;
    ...
    for (int i = 0; i < npes - 1; i++) {
        ...
        DendroIntL toleranceLoadBalance =
            ((i + 1) * globalSz / npes - i * globalSz / npes) * loadFlexibility;
    ...
    while (!splitBucketIndex.empty()) {
        ...
        if (bucketInfo[splitBucketIndex[0]].lev < pMaxDepth) {
            ...
            for (int k = 0; k < splitBucketIndex.size(); k++) {
                ...
                if (!rank)
                    std::cout << "Splitting Bucket index "
                              << splitBucketIndex[k] << "begin: " << tmp.begin
                              << " end: " << tmp.end
                ...
                for (int i = 0; i < NUM_CHILDREN; i++) {
                    hindex =
                        (rotations[2 * NUM_CHILDREN * tmp.rot_id + i] - '0');
                    if (i == (NUM_CHILDREN - 1))
                        hindexN = i + 1;
                    else
                        hindexN =
            ...
            for (unsigned int i = 0; i < npes - 1; i++) {
                idealLoadBalance +=
                    ((i + 1) * globalSz / npes - i * globalSz / npes);
                DendroIntL toleranceLoadBalance =
                    (((i + 1) * globalSz / npes - i * globalSz / npes) *
                     loadFlexibility);
                unsigned int loc = (std::lower_bound(bucketCounts_gScan.begin(),
                                                     bucketCounts_gScan.end(),
                                                     idealLoadBalance) -
                                    bucketCounts_gScan.begin());
            ...
            for (unsigned int i = 0; i < npes - 1; i++) {
                idealLoadBalance +=
                    ((i + 1) * globalSz / npes - i * globalSz / npes);
                // DendroIntL toleranceLoadBalance = ((i + 1) * globalSz / npes
                // - i * globalSz / npes) * loadFlexibility;
                unsigned int loc = (std::lower_bound(bucketCounts_gScan.begin(),
                                                     bucketCounts_gScan.end(),
                                                     idealLoadBalance) -
                                    bucketCounts_gScan.begin());
    ...
    if (!rank) {
        for (int i = 0; i < npes; i++)
            std::cout << "Rank " << rank << " Local Splitter: " << i << ": "
...
#ifdef PROFILE_TREE_SORT
    splitter_time = std::chrono::duration_cast<std::chrono::milliseconds>(
                        std::chrono::high_resolution_clock::now() - t2)
                        .count();
    // MPI_Barrier(pcomm);
    t2 = std::chrono::high_resolution_clock::now();  // MPI_Wtime();
...
#ifdef DEBUG_TREE_SORT
    /*if (!rank)*/ std::cout << rank << " : send = " << sendCounts[0] << ", "
                             << sendCounts[1] << std::endl;
    /*if (!rank)*/ std::cout << rank << " : recv = " << recvCounts[0] << ", "
                             << recvCounts[1] << std::endl;

/* if (!rank) std::cout << rank << " : send offset  = " << sendDispl[0] << ", "
<< sendDispl[1] << std::endl; if (!rank) std::cout << rank << " : recv offset  =
" << recvDispl[0] << ", " << recvDispl[1] << std::endl;*/
#endif
    ...
    par::Mpi_Alltoallv_sparse(&pNodes[0], sendCounts, sendDispl, &pNodesRecv[0],
                              recvCounts, recvDispl, comm);
    // MPI_Request req;
...
#ifdef PROFILE_TREE_SORT
    all2all2_time = std::chrono::duration_cast<std::chrono::milliseconds>(
                        std::chrono::high_resolution_clock::now() - t2)
                        .count();
    // MPI_Barrier(pcomm);
#endif
...
// std::cout<<"Rank: "<<rank<<"executing local Sort for size:
// "<<pNodes.size()<<std::endl;
#ifdef PROFILE_TREE_SORT
    // MPI_Barrier(pcomm);
    t2 = std::chrono::high_resolution_clock::now();  // MPI_Wtime();
#endif
    SFC::seqSort::SFC_treeSort(&(*(pNodes.begin())), pNodes.size(), pOutSorted,
                               pOutConstruct, pOutBalanced, pMaxDepth,
                               pMaxDepth, parent, rot_id, k, options);

#ifdef PROFILE_TREE_SORT
    localSort_time = std::chrono::duration_cast<std::chrono::milliseconds>(
                         std::chrono::high_resolution_clock::now() - t2)
                         .count();  // MPI_Wtime()-t2;
    // MPI_Barrier(pcomm);
#endif
    ...
    if (options & TS_REMOVE_DUPLICATES) {
        ...
        MPI_Comm new_comm;
        // very quick and dirty solution -- assert that tmpVec is non-emply at
        // every processor (repetetive calls to splitComm2way exhaust MPI
        // resources)
        ...
        if (!pOutSorted.empty()) {
            ...
            par::Mpi_Sendrecv<T, T>(
                &end, 1, ((new_rank < (new_size - 1)) ? (new_rank + 1) : 0), 1,
                &endRecv, 1, ((new_rank > 0) ? (new_rank - 1) : (new_size - 1)),
                1, new_comm, &status);

            // Remove endRecv if it exists (There can be no more than one copy
            // of this)
            if (new_rank) {
                typename std::vector<T>::iterator Iter =
                    std::find(pOutSorted.begin(), pOutSorted.end(), endRecv);
            ...
            while (count < pOutSorted.size() & state_global) {
                ...
                par::Mpi_Sendrecv<T, T>(
                    &begin, 1,
                    ((new_rank > 0) ? (new_rank - 1) : (new_size - 1)), 1,
                    &beginRecv, 1,
                    ((new_rank < (new_size - 1)) ? (new_rank + 1) : 0), 1,
                    new_comm, &status);

                /*if(beginRecv.isAncestor(end))
                {
                   std::cout<<"for rank: "<<new_rank<<" beginRecv:
                "<<beginRecv<<" is ancestor to : "<<end<<std::endl;
                }*/
                ...
                count++;
                MPI_Allreduce(&state, &state_global, 1, MPI_CXX_BOOL, MPI_LOR,
                              new_comm);
            }
...
// MPI_Barrier(pcomm);
...
#ifdef PROFILE_TREE_SORT
    total_rd = std::chrono::duration_cast<std::chrono::milliseconds>(
    ...
    if (!rank_g) {
        ...
        // std::cout<<"Rank: "<<rank_g<<"splitter_fix_all: "<<stat_property[0]<<
    ...
    par::Mpi_Reduce(&remove_duplicates_seq, &stat_property[0], 1, MPI_MIN, 0,
                    pcomm);
    par::Mpi_Reduce(&remove_duplicates_seq, &stat_property[1], 1, MPI_SUM, 0,
                    pcomm);
    par::Mpi_Reduce(&remove_duplicates_seq, &stat_property[2], 1, MPI_MAX, 0,
                    pcomm);
    ...
    par::Mpi_Reduce(&remove_duplicates_par, &stat_property[0], 1, MPI_MIN, 0,
                    pcomm);
    par::Mpi_Reduce(&remove_duplicates_par, &stat_property[1], 1, MPI_SUM, 0,
                    pcomm);
    par::Mpi_Reduce(&remove_duplicates_par, &stat_property[2], 1, MPI_MAX, 0,
                    pcomm);
    ...
    for (int i = 0; i < SF_Stages; i++) {
        ...
        par::Mpi_Reduce(&sf_all2all[i], &stat_property[0], 1, MPI_MIN, 0,
                        pcomm);
        par::Mpi_Reduce(&sf_all2all[i], &stat_property[1], 1, MPI_SUM, 0,
                        pcomm);
        par::Mpi_Reduce(&sf_all2all[i], &stat_property[2], 1, MPI_MAX, 0,
                        pcomm);
        ...
        par::Mpi_Reduce(&sf_splitters[i], &stat_property[0], 1, MPI_MIN, 0,
                        pcomm);
        par::Mpi_Reduce(&sf_splitters[i], &stat_property[1], 1, MPI_SUM, 0,
                        pcomm);
        par::Mpi_Reduce(&sf_splitters[i], &stat_property[2], 1, MPI_MAX, 0,
                        pcomm);
...
/*std::cout<<"rank: "<<rank<<" Remove Duplicates"<<std::endl;*/
        ...
        if (options & TS_CONSTRUCT_OCTREE) {
            ...
            SFC::parSort::SFC_treeSort(pOutConstruct, tmpSorted, tmpConstructed,
                                       tmpBalanced, loadFlexibility, pMaxDepth,
                                       root, rot_id, k, 1, sf_k, pcomm);
            ...
            par::Mpi_Reduce(&construction_time, &stat_property[0], 1, MPI_MIN,
                            0, pcomm);
            par::Mpi_Reduce(&construction_time, &stat_property[1], 1, MPI_SUM,
                            0, pcomm);
            par::Mpi_Reduce(&construction_time, &stat_property[2], 1, MPI_MAX,
                            0, pcomm);
...
// if(!rank) std::cout<<"Executing balancing"<<std::endl;
            ...
            SFC::parSort::SFC_treeSort(pOutBalanced, tmpSorted, tmpConstructed,
                                       tmpBalanced, loadFlexibility, pMaxDepth,
                                       root, rot_id, k, 1, sf_k, pcomm);
            ...
            par::Mpi_Reduce(&balancing_time, &stat_property[0], 1, MPI_MIN, 0,
                            pcomm);
            par::Mpi_Reduce(&balancing_time, &stat_property[1], 1, MPI_SUM, 0,
                            pcomm);
            par::Mpi_Reduce(&balancing_time, &stat_property[2], 1, MPI_MAX, 0,
                            pcomm);
