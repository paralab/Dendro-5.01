...
double rsNormLp(const ot::Mesh* pMesh, const T* v1, unsigned int p) {
    ...
    for (unsigned int ele = pMesh->getElementLocalBegin();
        ...
        pMesh->octCoordToDomainCoord(
            Point(pNodes[ele].minX(), pNodes[ele].minY(), pNodes[ele].minZ()),
            pmin);
        pMesh->octCoordToDomainCoord(
            Point(pNodes[ele].maxX(), pNodes[ele].maxY(), pNodes[ele].maxZ()),
            pmax);
        ...
        for (unsigned int k = 0; k < DD; k++)
            for (unsigned int j = 0; j < DD; j++)
                for (unsigned int i = 0; i < DD; i++) {
                    double cell_avg =
                        (nodalVals[(k) * (DD + 1) * (DD + 1) + (j) * (DD + 1) +
                                   (i)] +
                         nodalVals[(k) * (DD + 1) * (DD + 1) + (j) * (DD + 1) +
                                   (i + 1)] +
                         nodalVals[(k) * (DD + 1) * (DD + 1) +
                                   (j + 1) * (DD + 1) + (i)] +
                         nodalVals[(k) * (DD + 1) * (DD + 1) +
                                   (j + 1) * (DD + 1) + (i + 1)] +
                         nodalVals[(k + 1) * (DD + 1) * (DD + 1) +
                                   (j) * (DD + 1) + (i)] +
                         nodalVals[(k + 1) * (DD + 1) * (DD + 1) +
                                   (j) * (DD + 1) + (i + 1)] +
                         nodalVals[(k + 1) * (DD + 1) * (DD + 1) +
    ...
    double d_volume =
        (dmax.x() - dmin.x()) * (dmax.y() - dmin.y()) * (dmax.z() - dmin.z());
 ...
 */
template <typename T>
double rsNormLp(const ot::Mesh* pMesh, const T* v1, const T* v2,
                unsigned int p) {
    if (!pMesh->isActive()) return -1.0;
    ...
    for (unsigned int e = pMesh->getElementLocalBegin();
        ...
        pMesh->octCoordToDomainCoord(
            Point(pNodes[e].minX(), pNodes[e].minY(), pNodes[e].minZ()), pmin);
        pMesh->octCoordToDomainCoord(
            Point(pNodes[e].maxX(), pNodes[e].maxY(), pNodes[e].maxZ()), pmax);
        ...
        for (unsigned int k = 0; k < DD; k++)
            for (unsigned int j = 0; j < DD; j++)
                for (unsigned int i = 0; i < DD; i++) {
                    double cell_avg1 = (eleVec1[(k) * (DD + 1) * (DD + 1) +
                                                (j) * (DD + 1) + (i)] +
                                        eleVec1[(k) * (DD + 1) * (DD + 1) +
                                                (j) * (DD + 1) + (i + 1)] +
                                        eleVec1[(k) * (DD + 1) * (DD + 1) +
                    ...
                    double cell_avg2 = (eleVec2[(k) * (DD + 1) * (DD + 1) +
                                                (j) * (DD + 1) + (i)] +
                                        eleVec2[(k) * (DD + 1) * (DD + 1) +
                                                (j) * (DD + 1) + (i + 1)] +
                                        eleVec2[(k) * (DD + 1) * (DD + 1) +
 ...
 */
template <typename T>
T vecMin(const ot::Mesh* pMesh, const T* v1, ot::VEC_TYPE vtype,
         bool isGhosted = true) {
    if (!(pMesh->isActive())) return NAN;
    ...
    if (vtype == ot::VEC_TYPE::CG_NODAL) {
        if (isGhosted) offset = pMesh->getNodeLocalBegin();
        ...
        if (isGhosted)
            offset =
                pMesh->getElementLocalBegin() * pMesh->getNumNodesPerElement();

        nsz = pMesh->getNumLocalMeshElements() * pMesh->getNumNodesPerElement();
        val = vecMin((T*)(v1 + offset), nsz, aComm);

    } else if (vtype == ot::VEC_TYPE::ELEMENTAL) {
        if (isGhosted) offset = pMesh->getElementLocalBegin();

        nsz = pMesh->getNumLocalMeshElements();
        val = vecMin((T*)(v1 + offset), nsz, aComm);

    } else {
        std::cout << "rank: " << arank << " invalid vec type to the norm call"
                  << __LINE__ << std::endl;
        val = NAN;
    }
 ...
 */
template <typename T>
T vecMax(const ot::Mesh* pMesh, const T* v1, ot::VEC_TYPE vtype,
         bool isGhosted = true) {
    if (!(pMesh->isActive())) return NAN;
 ...
 */
template <typename T>
T normL2(const ot::Mesh* pMesh, const T* v1, ot::VEC_TYPE vtype,
         bool isGhosted = true) {
    if (!(pMesh->isActive())) return NAN;
 ...
 */
template <typename T>
T normLInfty(const ot::Mesh* pMesh, const T* v1, ot::VEC_TYPE vtype,
             bool isGhosted = true) {
    if (!(pMesh->isActive())) return NAN;
