 ...
 * @brief Class to maintain launching different solvers in different MPI
...
class Launcher {
   ...
   public:
    Launcher(MPI_Comm comm, unsigned int nodes_per_job,
             unsigned int cores_per_node) {
        m_uiCommGlobal   = comm;
    ...
    int alloc_sub_communicator(unsigned int njobs) {
        ...
        if (npes_g != m_uiNumJobs * m_uiNodesPerJob * m_uiCoresPerNode) {
            if (!rank_g)
                std::cout << "GLOBAL CORES : " << npes_g
                          << " not enough to run " << m_uiNumJobs
                          << " jobs with " << m_uiNodesPerJob << " nodes with "
                          << m_uiCoresPerNode << " cores per node" << std::endl;
    ...
    inline const MPI_Comm* get_sub_communicator() const {
        return m_uiCommLocal;
    }

    /**@brief get the global communicator*/
    inline MPI_Comm get_global_communicator() const { return m_uiCommGlobal; }

    inline unsigned int get_comm_split_index() const {
        return m_uiLocalSplitIndex;
    }
};
