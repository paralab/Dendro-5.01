 ...
 * */
template <typename T>
void copyArrayToDeviceAsync(const T* in, T* __deviceptr, unsigned int numElems,
 ...
 * */
template <typename T>
void copy2DCudaArrayToDeviceAsync(const T** in, T** __devicePtr,
                                  unsigned int sz1, unsigned int sz2,
                                  const cudaStream_t stream);

template <typename T>
void copyArrayToHostAsync(T* host_ptr, const T* __deviceptr,
                          unsigned int numElems, const cudaStream_t stream);
...
/***
 * computes the how dendro blocks (octree blocks result in from unzip) to the
 * gpu/
 * @param[in] blkList: list of dendro data blocks
 * @param[in] numBlocks: number of data blocks,
 * @param[out] blockMap: (blockMap[2*blocDim.x] , blockMap[2*blocDim.x+1]) begin
 * & end of data block that is going to be process by the gpu block
 * */
void computeDendroBlockToGPUMap(const ot::Block* blkList,
                                unsigned int numBlocks, unsigned int*& blockMap,
                                dim3& gridDim);
 ...
 * */
template <typename T>
void copy2DArrayToHost(T** host_ptr, const T** __device_ptr, unsigned int sz1,
                       unsigned int sz2);
...
void copyArrayToDeviceAsync(const T* in, T* __deviceptr, unsigned int numElems,
                            const cudaStream_t stream) {
    cudaMemcpyAsync(__deviceptr, in, sizeof(T) * numElems,
...
void copyArrayToHostAsync(T* host_ptr, const T* __deviceptr,
                          unsigned int numElems, const cudaStream_t stream) {
    cudaMemcpyAsync(host_ptr, __deviceptr, sizeof(T) * numElems,
                    cudaMemcpyDeviceToHost, stream);
    CUDA_CHECK_ERROR();
}

template <typename T>
void copyArrayToHost(T* host_ptr, const T* __device_ptr,
                     unsigned int numElems) {
    cudaMemcpy(host_ptr, __device_ptr, sizeof(T) * numElems,
...
void copy2DArrayToHost(T** host_ptr, const T** __device_ptr, unsigned int sz1,
    ...
    for (unsigned int i = 0; i < sz1; i++) {
        cudaMemcpy(host_ptr[i], tmp2D[i], sizeof(T) * sz2,
                   cudaMemcpyDeviceToHost);
        CUDA_CHECK_ERROR();
    }
