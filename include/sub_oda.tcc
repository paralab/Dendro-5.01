...
inline void subDA::init<ot::DA_FLAGS::WRITABLE>() {
    ...
    while ((binOp::getBit(octFlags[m_uip_sub2DA_ElemMap[count]],
                          ODA_W_DEPENDENT_FLAG_BIT) != 1) &&
           (binOp::getBit(octFlags[m_uip_sub2DA_ElemMap[count]],
                          ODA_INDEPENDENT_FLAG_BIT) != 1) &&
           (count < m_uiLoopInfo.indexEnd))
        count++;

    if ((binOp::getBit(octFlags[m_uip_sub2DA_ElemMap[count]],
                       ODA_W_DEPENDENT_FLAG_BIT) != 1) &&
        (binOp::getBit(octFlags[m_uip_sub2DA_ElemMap[count]],
                       ODA_INDEPENDENT_FLAG_BIT) != 1))
        count++;
...
inline void subDA::next<ot::DA_FLAGS::WRITABLE>() {
    ...
    while (((m_uiLoopInfo.currentIndex) < m_uiLoopInfo.indexEnd) &&
           (binOp::getBit(
                octFlags[m_uip_sub2DA_ElemMap[m_uiLoopInfo.currentIndex]],
                ODA_W_DEPENDENT_FLAG_BIT) != 1) &&
           (binOp::getBit(
                octFlags[m_uip_sub2DA_ElemMap[m_uiLoopInfo.currentIndex]],
                ODA_INDEPENDENT_FLAG_BIT) != 1))
...
inline unsigned int subDA::curr() { return m_uiLoopInfo.currentIndex; }

template <typename T>
int subDA::createVector(T*& local, bool isElemental, bool isGhosted,
...
template <typename T>
int subDA::createVector(std::vector<T>& local, bool isElemental, bool isGhosted,
                        unsigned int dof) const {
    if (!(m_uiIsActive)) {
        local.clear();
...
template <typename T>
void subDA::nodalVecToGhostedNodal(const T* in, T*& out, bool isAllocated,
                                   unsigned int dof) const {
    ...
    for (unsigned int var = 0; var < dof; var++) {
        std::memcpy((out + var * m_uiTotalNodalSz + m_uiNodeLocalBegin),
                    (in + var * m_uiLocalNodalSz),
                    sizeof(T) * (m_uiLocalNodalSz));
    }
}

template <typename T>
void subDA::ghostedNodalToNodalVec(const T* gVec, T*& local, bool isAllocated,
                                   unsigned int dof) const {
    if (!(m_uiIsActive)) return;
    ...
    for (unsigned int var = 0; var < dof; var++)
        std::memcpy((local + var * m_uiLocalNodalSz),
                    (gVec + (var * m_uiTotalNodalSz) + m_uiNodeLocalBegin),
                    sizeof(T) * (m_uiLocalNodalSz));
}

template <typename T>
void subDA::readFromGhostBegin(T* vec, unsigned int dof) {
    if (m_da->getNpesAll() == 1) return;
    ...
    if (m_uiIsActive) {
        const unsigned int sendBSz = m_uiSendOffsets[m_uiNpesActive - 1] +
                                     m_uiSendCounts[m_uiNpesActive - 1];
        const unsigned int recvBSz = m_uiRecvOffsets[m_uiNpesActive - 1] +
                                     m_uiRecvCounts[m_uiNpesActive - 1];
        ...
        if (recvBSz) {
            ...
            for (unsigned int recv_p = 0; recv_p < m_uiRecvProcList.size();
                ...
                par::Mpi_Irecv((recvB + dof * m_uiRecvOffsets[proc_id]),
                               dof * m_uiRecvCounts[proc_id], proc_id,
                               m_uiCommTag, m_uiCommActive, req);
        ...
        if (sendBSz) {
            ...
            for (unsigned int send_p = 0; send_p < m_uiSendProcList.size();
                ...
                for (unsigned int var = 0; var < dof; var++) {
                    for (unsigned int k = m_uiSendOffsets[proc_id];
                         k <
                         (m_uiSendOffsets[proc_id] + m_uiSendCounts[proc_id]);
                         k++) {
                        sendB[dof * (m_uiSendOffsets[proc_id]) +
                              (var * m_uiSendCounts[proc_id]) +
                              (k - m_uiSendOffsets[proc_id])] =
                            (vec +
                             var * m_uiTotalNodalSz)[m_uiSendScatterMap[k]];
                    }
                }
            }
            ...
            for (unsigned int send_p = 0; send_p < m_uiSendProcList.size();
                ...
                proc_id          = m_uiSendProcList[send_p];
                MPI_Request* req = new MPI_Request();
                par::Mpi_Isend(sendB + dof * m_uiSendOffsets[proc_id],
                               dof * m_uiSendCounts[proc_id], proc_id,
                               m_uiCommTag, m_uiCommActive, req);
                ctx.getRequestList().push_back(req);
            }
        }
    ...
    if (m_uiIsActive) {
        ...
        if (recvBSz && m_uiTotalNodalSz) {
            ...
            for (unsigned int recv_p = 0; recv_p < m_uiRecvProcList.size();
                ...
                for (unsigned int var = 0; var < dof; var++) {
                    for (unsigned int k = m_uiRecvOffsets[proc_id];
                         k <
                         (m_uiRecvOffsets[proc_id] + m_uiRecvCounts[proc_id]);
                         k++) {
                        (vec + var * m_uiTotalNodalSz)[m_uiRecvScatterMap[k]] =
                            recvB[dof * (m_uiRecvOffsets[proc_id]) +
                                  (var * m_uiRecvCounts[proc_id]) +
                                  (k - m_uiRecvOffsets[proc_id])];
                    }
                }
            }
        }
...
// template <typename T>
// void subDA::getElementNodalValues(const T*in, T* eleVecOut,unsigned int
// eleID,unsigned int dof) const
// {
//     //assert((eleID>=m_uiMesh->getElementLocalBegin()) &&
//     (eleID<m_uiMesh->getElementLocalEnd())); const unsigned int
//     nPe=m_da->getNumNodesPerElement(); for(unsigned int var=0;var<dof;var++)
...
// template <typename T>
// void subDA::eleVecToVecAccumilation(T*out, const T* eleVecIn,unsigned int
// eleID,unsigned int dof) const
// {
...
// template <typename T>
// void subDA::setVectorByFunction(T*
// local,std::function<void(T,T,T,T*)>func,bool isElemental, bool isGhosted,
// unsigned int dof) const
// {
...
//         for(unsigned int
//         node=m_uiMesh->getNodeLocalBegin();node<m_uiMesh->getNodeLocalEnd();node++)
//         {
//             lookup=cgToDg[node];
...
//         for(unsigned int
//         ele=m_uiMesh->getElementLocalBegin();ele<m_uiMesh->getElementLocalEnd();ele++)
//         {
...
// template <typename T>
// void subDA::setVectorByScalar(T* local,const T* value,bool isElemental, bool
// isGhosted, unsigned int dof) const
// {
...
template <typename T>
T* subDA::getVecPointerToDof(T* in, unsigned int dofInex, bool isElemental,
...
template <typename T>
void subDA::copyVector(T* dest, const T* source, bool isElemental,
                       bool isGhosted) const {
    if (!(m_uiIsActive)) return;
...
template <typename T>
void subDA::copyVectors(T* dest, const T* source, bool isElemental,
...
// template<typename T>
// int subDA::getFaceNeighborValues(unsigned int eleID, const T* in, T* out, T*
// coords, unsigned int * neighID, unsigned int face, NeighbourLevel & level)
// const
// {
...
//     int
//     state=m_uiMesh->getFaceNeighborValues(eleID,in,out,coords,neighID,face,level);
...
template <typename T>
void subDA::petscSetVectorByFunction(Vec& local,
...
template <typename T>
void subDA::petscSetVectorByScalar(Vec& local, const T* value, bool isElemental,
                                   bool isGhosted, unsigned int dof) const {
    PetscScalar* arry = NULL;
    VecGetArray(local, &arry);
