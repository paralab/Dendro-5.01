...
T normLInfty(T* vec1, T* vec2, unsigned int n) {
    ...
    for (unsigned int i = 1; i < n; i++) {
        if ((std::isnan(vec1[i])) || (std::isnan(vec2[i]))) return NAN;
...
T dot(const T* v1, const T* v2, const unsigned int n) {
    T ans = 0;
    for (unsigned int i = 0; i < n; i++) ans += (v1[i] * v2[i]);
...
void mul(const T alpha, const T* v, const unsigned int n, T* out) {
    for (unsigned int i = 0; i < n; i++) out[i] = alpha * v[i];
}

template <typename T>
T add(const T* v1, const T* v2, const unsigned int n, T* out) {
    for (unsigned int i = 0; i < n; i++) out[i] = v1[i] + v2[i];
}
...
template <typename T>
void kron(const T* M1, const T* M2, T* out, unsigned int r1, unsigned int c1,
