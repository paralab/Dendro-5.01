...
/**
 * @brief subDA based on the DA, where to perform computations sub domain of the
 * main domain.
 *
 */
class subDA {
   ...
   public:
     ...
     */
    subDA(DA* da, std::function<double(double, double, double)> fx_retain,
          double* gSize);
    ...
    * */
    ...
    inline const RefElement* getReferenceElement() const {
        return m_da->getReferenceElement();
    }
    ...
    inline unsigned int getPreAndPostGhostNodeSize() const {
        return (m_uiNumPreGhostNodes + m_uiNumPostGhostNodes);
    }

    /**@brief: get the number of pre and post ghost elements*/
    unsigned int getGhostedElementSize() const {
        return (m_uiNumPreGhostElements + m_uiNumPostGhostElements);
    }
    ...
    */
    ...
    inline unsigned int getLevel(unsigned int ele) const {
        return m_da->getLevel(m_uip_sub2DA_ElemMap[ele]);
    }
    ...
    inline ot::TreeNode getOctant(unsigned int ele) const {
        return m_da->getOctant(m_uip_sub2DA_ElemMap[ele]);
    }
     ...
     * */
    template <typename T>
    void intergridTransfer(const T* varIn, T*& varOut, const ot::DA* newDA,
                           bool isElemental = false, bool isGhosted = false,
                           unsigned int dof = 1);
    ...
    */
     ...
     */
    template <typename T>
    void petscIntergridTransfer(const Vec& varIn, Vec& varOut,
                                const ot::DA* newDA, bool isElemental = false,
                                bool isGhosted = false, unsigned int dof = 1);
