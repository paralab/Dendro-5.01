...
template <typename T>
DA::DA(std::function<void(T, T, T, T*)> func, unsigned int dofSz, MPI_Comm comm,
       unsigned int order, double interp_tol, unsigned int grainSz,
       double sfc_tol, SM_TYPE smType) {
    int rank, npes;
    ...
    for (unsigned int i = 0; i < numInterpSz; i++) varIndex[i] = i;

    function2Octree(func, dofSz, (unsigned int*)varIndex, numInterpSz, tmpNodes,
                    m_uiMaxDepth, interp_tol, order, comm);
    double t2 = MPI_Wtime();
    ...
    shrinkOrExpandOctree(tmpNodes, sfc_tol, DENDRO_DEFAULT_SF_K, isActive,
                         commActive, comm);
    ...
    if (isActive) {
        ...
        SFC::parSort::SFC_treeSort(tmpNodes, tmpVec, tmpVec, tmpVec, sfc_tol,
                                   m_uiMaxDepth, root, ROOT_ROTATION, 1,
                                   TS_REMOVE_DUPLICATES, DENDRO_DEFAULT_SF_K,
                                   commActive);
        ...
        SFC::parSort::SFC_treeSort(tmpNodes, tmpVec, tmpVec, tmpVec, sfc_tol,
                                   m_uiMaxDepth, root, ROOT_ROTATION, 1,
                                   TS_CONSTRUCT_OCTREE, DENDRO_DEFAULT_SF_K,
                                   commActive);
        std::swap(tmpNodes, tmpVec);
        tmpVec.clear();
        ...
        SFC::parSort::SFC_treeSort(tmpNodes, balOct, balOct, balOct, sfc_tol,
                                   m_uiMaxDepth, root, ROOT_ROTATION, 1,
                                   TS_BALANCE_OCTREE, DENDRO_DEFAULT_SF_K,
                                   commActive);
        tmpNodes.clear();
    ...
    // generates the mesh.
    m_uiMesh =
        new ot::Mesh(balOct, 1, order, comm, false, smType, grainSz, sfc_tol);
    balOct.clear();
    ...
    ot::computeLocalToGlobalNodalMap(m_uiMesh, m_uiLocalToGlobalNodalMap,
                                     m_uiGlobalNodeSz, m_uiNodalOffset);

#ifdef DEBUG_ODA
    for (init<DA_FLAGS::INDEPENDENT>(); curr() < end<DA_FLAGS::INDEPENDENT>();
         next<DA_FLAGS::INDEPENDENT>())
        independent.push_back(allElements[curr()]);

    for (init<DA_FLAGS::W_DEPENDENT>(); curr() < end<DA_FLAGS::W_DEPENDENT>();
         next<DA_FLAGS::W_DEPENDENT>())
        w_dependent.push_back(allElements[curr()]);

    for (init<DA_FLAGS::W_BOUNDARY>(); curr() < end<DA_FLAGS::W_BOUNDARY>();
         next<DA_FLAGS::W_BOUNDARY>())
        w_bdy.push_back(
            allElements[curr()]);  // std::cout<<"loop ele: "<<curr()<<" :
                                   // "<<allElements[curr()]<<std::endl;//

    io::vtk::oct2vtu(&(*(independent.begin())), independent.size(),
                     "independent", comm);
    io::vtk::oct2vtu(&(*(w_dependent.begin())), w_dependent.size(),
                     "w_dependent", comm);
...
inline void DA::init<ot::DA_FLAGS::WRITABLE>() {
    ...
    while ((binOp::getBit(m_uiOctantFlags[count], ODA_W_DEPENDENT_FLAG_BIT) !=
            1) &&
           (binOp::getBit(m_uiOctantFlags[count], ODA_INDEPENDENT_FLAG_BIT) !=
            1) &&
           (count < m_uiLoopInfo.indexEnd))
        count++;

    if ((binOp::getBit(m_uiOctantFlags[count], ODA_W_DEPENDENT_FLAG_BIT) !=
         1) &&
        (binOp::getBit(m_uiOctantFlags[count], ODA_INDEPENDENT_FLAG_BIT) != 1))
        count++;
...
inline void DA::init<ot::DA_FLAGS::INDEPENDENT>() {
    ...
    unsigned int count      = m_uiLoopInfo.indexBegin;
    while ((binOp::getBit(m_uiOctantFlags[count], ODA_INDEPENDENT_FLAG_BIT) !=
            1) &&
           (count < m_uiLoopInfo.indexEnd))
        count++;
...
inline void DA::init<ot::DA_FLAGS::W_BOUNDARY>() {
    ...
    unsigned int count      = m_uiLoopInfo.indexBegin;
    while (
        (binOp::getBit(m_uiOctantFlags[count], ODA_W_BOUNDARY_FLAG_BIT) != 1) &&
        (count < m_uiLoopInfo.indexEnd))
        count++;
...
inline void DA::next<ot::DA_FLAGS::WRITABLE>() {
    m_uiLoopInfo.currentIndex++;
    if (m_uiLoopInfo.currentIndex >= m_uiLoopInfo.indexEnd) return;

    while (((m_uiLoopInfo.currentIndex) < m_uiLoopInfo.indexEnd) &&
           (binOp::getBit(m_uiOctantFlags[m_uiLoopInfo.currentIndex],
                          ODA_W_DEPENDENT_FLAG_BIT) != 1) &&
           (binOp::getBit(m_uiOctantFlags[m_uiLoopInfo.currentIndex],
                          ODA_INDEPENDENT_FLAG_BIT) != 1))
        m_uiLoopInfo.currentIndex++;

    if ((binOp::getBit(m_uiOctantFlags[m_uiLoopInfo.currentIndex],
                       ODA_W_DEPENDENT_FLAG_BIT) != 1) &&
        (binOp::getBit(m_uiOctantFlags[m_uiLoopInfo.currentIndex],
                       ODA_INDEPENDENT_FLAG_BIT) != 1))
        m_uiLoopInfo.currentIndex++;
...
inline unsigned int DA::curr() { return m_uiLoopInfo.currentIndex; }

template <typename T>
int DA::createVector(T*& local, bool isElemental, bool isGhosted,
...
template <typename T>
int DA::createVector(std::vector<T>& local, bool isElemental, bool isGhosted,
                     unsigned int dof) const {
    if (!(m_uiMesh->isActive())) {
        local.clear();
...
template <typename T>
void DA::nodalVecToGhostedNodal(const T* in, T*& out, bool isAllocated,
                                unsigned int dof) const {
    if (!(m_uiMesh->isActive())) return;

    if (!isAllocated) createVector<T>(out, false, true, dof);

    for (unsigned int var = 0; var < dof; var++) {
        std::memcpy(
            (out + var * m_uiTotalNodalSz + m_uiMesh->getNodeLocalBegin()),
            (in + var * m_uiLocalNodalSz), sizeof(T) * (m_uiLocalNodalSz));
    }
}

template <typename T>
void DA::ghostedNodalToNodalVec(const T* gVec, T*& local, bool isAllocated,
                                unsigned int dof) const {
    if (!(m_uiMesh->isActive())) return;

    if (!isAllocated) createVector(local, false, false, dof);

    for (unsigned int var = 0; var < dof; var++)
        std::memcpy(
            (local + var * m_uiLocalNodalSz),
            (gVec + (var * m_uiTotalNodalSz) + m_uiMesh->getNodeLocalBegin()),
            sizeof(T) * (m_uiLocalNodalSz));
}

template <typename T>
void DA::readFromGhostBegin(T* vec, unsigned int dof) {
    if (m_uiMesh->getMPICommSizeGlobal() == 1) return;
                ...
                for (unsigned int var = 0; var < dof; var++) {
                    for (unsigned int k = nodeSendOffset[proc_id];
                        ...
                        sendB[dof * (nodeSendOffset[proc_id]) +
                            ...
                            (vec + var * m_uiTotalNodalSz)[sendNodeSM[k]];
                    }
                }
            }
            ...
            for (unsigned int recv_p = 0; recv_p < recvProcList.size();
                ...
                for (unsigned int var = 0; var < dof; var++) {
                    for (unsigned int k = nodeRecvOffset[proc_id];
                         ...
                         k++) {
                        (vec + var * m_uiTotalNodalSz)[recvNodeSM[k]] =
...
template <typename T>
void DA::getElementNodalValues(const T* in, T* eleVecOut, unsigned int eleID,
                               unsigned int dof) const {
    // assert((eleID>=m_uiMesh->getElementLocalBegin()) &&
    // (eleID<m_uiMesh->getElementLocalEnd()));
    const unsigned int nPe = m_uiMesh->getNumNodesPerElement();

    for (unsigned int var = 0; var < dof; var++)
        m_uiMesh->getElementNodalValues(&in[var * m_uiTotalNodalSz],
                                        &eleVecOut[var * nPe], eleID);
}

template <typename T>
void DA::eleVecToVecAccumilation(T* out, const T* eleVecIn, unsigned int eleID,
                                 unsigned int dof) const {
    const unsigned int nPe = m_uiMesh->getNumNodesPerElement();

    for (unsigned int var = 0; var < dof; var++)
        m_uiMesh->computeElementalContribution(
            &eleVecIn[var * nPe], &out[var * m_uiTotalNodalSz], eleID);
}

template <typename T>
void DA::setVectorByFunction(T* local, std::function<void(T, T, T, T*)> func,
...
template <typename T>
void DA::setVectorByScalar(T* local, const T* value, bool isElemental,
...
template <typename T>
T* DA::getVecPointerToDof(T* in, unsigned int dofInex, bool isElemental,
                          bool isGhosted) const {
    if (!(m_uiMesh->isActive())) return NULL;
...
template <typename T>
void DA::vecTopvtu(T* local, const char* fPrefix, char** nodalVarNames,
                   bool isElemental, bool isGhosted, unsigned int dof) {
    T* _local = NULL;
    std::vector<std::string> _nodalVarNames;
    bool isDeafaultVarNames = false;
    if (nodalVarNames == NULL) isDeafaultVarNames = true;
    ...
    io::vtk::mesh2vtuFine(m_uiMesh, fPrefix, 0, NULL, NULL, dof,
                          (const char**)nodalVarNames,
                          (const double**)nodalVec);

    // release mem, only if the input vec is ghosted.
    if (!isGhosted) destroyVector(_local);
...
template <typename T>
void DA::copyVector(T* dest, const T* source, bool isElemental,
                    bool isGhosted) const {
    if (!(m_uiMesh->isActive())) return;
...
template <typename T>
void DA::copyVectors(T* dest, const T* source, bool isElemental, bool isGhosted,
                     unsigned int dof) const {
    if (!(m_uiMesh->isActive())) return;
...
template <typename T>
void DA::intergridTransfer(const T* varIn, T*& varOut, const ot::DA* newDA,
                           bool isElemental, bool isGhosted, unsigned int dof,
    ...
    for (unsigned int var = 0; var < dof; var++) vIn[var] = NULL;

    if (!isGhosted) {
        for (unsigned int var = 0; var < dof; var++) {
            // allocation happens inside the function
            this->nodalVecToGhostedNodal(varIn + var * nLocalNodes_old,
                                         vIn[var], false, 1);
        }
    ...
    if (!isGhosted) {
        ...
        for (unsigned int var = 0; var < dof; var++)
            std::memcpy(varOut + var * zipSzNew, vIn[var],
                        sizeof(T) * zipSzNew);
    }

    for (unsigned int var = 0; var < dof; var++) delete[] vIn[var];

    delete[] vIn;
}

template <typename T>
int DA::getFaceNeighborValues(unsigned int eleID, const T* in, T* out,
                              T* coords, unsigned int* neighID,
                              unsigned int face, NeighbourLevel& level,
                              unsigned int dof) const {
    if (!m_uiMesh->isActive()) return false;
    ...
    for (unsigned int v = 0; v < dof; v++)
        state = m_uiMesh->getFaceNeighborValues(eleID, in + v * totalNodeSz,
                                                out + v * outSz, coords,
                                                neighID, face, level);
...
template <typename T>
void DA::petscSetVectorByFunction(Vec& local,
...
template <typename T>
void DA::petscSetVectorByScalar(Vec& local, const T* value, bool isElemental,
                                bool isGhosted, unsigned int dof) const {
    PetscScalar* arry = NULL;
    VecGetArray(local, &arry);
...
template <typename T>
void DA::petscIntergridTransfer(const Vec& varIn, Vec& varOut,
                                const ot::DA* newDA, bool isElemental,
    ...
    if (!isGhosted) {
        for (unsigned int var = 0; var < dof; var++) {
            // allocation happens inside the function
            this->nodalVecToGhostedNodal(inArray + var * nLocalNodes_old,
                                         vIn[var], false, 1);
        }
        ...
        for (unsigned int var = 0; var < dof; var++) {
            this->createVector(vIn[var], false, true, 1);
            std::memcpy(vIn[var], inArray + var * zipSzOld,
                        sizeof(T) * zipSzOld);
        }
    }
    ...
    if (newDA->isActive()) {
        if (!isGhosted) {
            ...
            for (unsigned int var = 0; var < dof; var++) {
                ...
                newDA->ghostedNodalToNodalVec((const T*)vIn[var], tmpPtr, true,
                                              1);
            }
            ...
            VecGetArray(varOut, &outArray);
            for (unsigned int var = 0; var < dof; var++)
                std::memcpy(outArray + var * zipSzNew, vIn[var],
                            sizeof(T) * zipSzNew);
            VecRestoreArray(varOut, &outArray);
        }
    }
