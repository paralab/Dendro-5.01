/**
  @file oda.h
  @brief The class that manages the octree mesh that support FEM computations.
  Note that this file is a refactored version from Dendro4 which is changed to
  support Dendro-5.0.  Currently we use mesh based FEM computations but in
  future we will move towards the mesh - free FEM computation methods.

  Distributed Array (DA) methods currently uses ot::Mesh class to provide
  interface to write FEM loops.
 ...
 * |------- pre ghost nodes----- |-----------local
 * nodes---------------|--------post ghost nodes--------|
 *
 *
 *
 * */
...
/**
 * @brief loop flags,
 * ALL : Loop over all the elements (note here we loop only on the local + level
 * 1 ghost elements). WRITABLE: Loop over INDEPENDENT U W_DEPENDENT INDEPENDENT
 * : Loop over all the local elements which DOES NOT point to any ghost node
 * region. W_DEPENDENT : Loop over all local elements which has AT LEAST ONE
 * node which point to ghost node region W_BOUNDARY : Loop over all the local
 * elements which is on the domain boundary.
 *
 ...
 * MESH_FREE - denotes the MatVec computation without using mesh data
 * structures.
 * */
...
class DA {
   ...
   public:
     ...
     * @param [in] in : input octree, need to be 2:1 balanced unique sorted
     * octree.
     ...
     * */
    DA(std::vector<ot::TreeNode>& balOct, MPI_Comm comm, unsigned int order,
       unsigned int grainSz = 100, double sfc_tol = 0.3,
     ...
     * @param[in] interp_tol: interpolation tolerance for func approximation/
     * capturing
     ...
     * */
    template <typename T>
    DA(std::function<void(T, T, T, T*)> func, unsigned int dofSz, MPI_Comm comm,
       unsigned int order, double interp_tol, unsigned int grainSz = 100,
       double sfc_tol = 0.3, SM_TYPE smType = SM_TYPE::FEM_CG);
     ...
     */
    DA(const Point* pts, unsigned int numPts, Point pt_min, Point pt_max,
       MPI_Comm comm, unsigned int order, unsigned int grainSz = 100,
       double sfc_tol = 0.3, SM_TYPE smType = SM_TYPE::FEM_CG);
    ...
    inline unsigned int getPreNodalSz() const {
        return m_uiMesh->getNumPreMeshNodes();
    }

    /**@brief returns the post nodal size*/
    inline unsigned int getPostNodalSz() const {
        return m_uiMesh->getNumPostMeshNodes();
    }

    /**@brief returns the total nodal size (this includes the ghosted region as
     * well.)*/
    ...
    /**@brief returns the local elemental size (includes the ghost elements as
     * well)*/
    ...
    inline unsigned int getNumNodesPerElement() const {
        return m_uiMesh->getNumNodesPerElement();
    }

    /**@brief get element order*/
    inline unsigned int getElementOrder() const {
        return m_uiMesh->getElementOrder();
    }

    /**@brief: returns the global MPI communicator*/
    inline MPI_Comm getGlobalComm() const {
        return m_uiMesh->getMPIGlobalCommunicator();
    }

    /**@brief: returns the get nodal offset values. */
    inline const std::vector<DendroIntL>& getNodalOffsets() const {
        return m_uiNodalOffset;
    }

    /**@brief: returns node local to node global map*/
    inline const std::vector<DendroIntL>& getNodeLocalToGlobalMap() const {
        return m_uiLocalToGlobalNodalMap;
    }
    ...
    inline const std::vector<unsigned int> getOctFlags() const {
    ...
    inline unsigned int getNpesAll() const {
        return m_uiMesh->getMPICommSizeGlobal();
    };
    ...
    * @param[in/out] coords: computed coords (note: assumes memory is allocated
    allocated)
    * coords are stored by p0,p1,p2... each pi \in R^dim where pi are ordered in
    along x axis then y axis and  then z axis
    ...
    * */
    ...
    unsigned int getOctantWeight(unsigned int ele) const {
        return m_uiMesh->getAllElements()[ele].getWeight();
    }

    /**@brief get a constant pointer for the reference element*/
    inline const RefElement* getReferenceElement() const {
        return m_uiMesh->getReferenceElement();
    }

    /**@brief: get the number of local elements. */
    inline unsigned int getElementSize() const {
        return m_uiMesh->getNumLocalMeshElements();
    }

    /**@brief: get the number of pre-ghost element size*/
    inline unsigned int getPreGhostElementSize() const {
        return m_uiMesh->getNumPreGhostElements();
    }

    /**@brief: get the number of post-ghost element size*/
    inline unsigned int getPostGhostElementSize() const {
        return m_uiMesh->getNumPostGhostElements();
    }

    /**@brief: get the number of pre and post ghost elements*/
    inline unsigned int getPreAndPostGhostNodeSize() const {
        return (m_uiMesh->getNumPreMeshNodes() +
                m_uiMesh->getNumPostMeshNodes());
    }

    /**@brief number of layer 1 ghost elements. */
    inline unsigned int getNumLayer1GhostEleSz() const {
        return m_uiMesh->getLevel1GhostElementIndices().size();
    }
     ...
     * @param[in/out] ownerrank for i^th node, -1 if not found. (Needs to be
     * allocated, outsize for size n);
     */
     ...
     * @param [in] isElemental: True if creating a elemental vector (cell data
     * vector) false for a nodal vector
     * @param [in] isGhosted: True will allocate ghost nodal values as well,
     * false will only allocate memory for local nodes.
     * @param [in] dof: degrees of freedoms
     * */
    template <typename T>
    int createVector(T*& local, bool isElemental = false,
                     bool isGhosted = false, unsigned int dof = 1) const;
     ...
     * */
    template <typename T>
    int createVector(std::vector<T>& local, bool isElemental = false,
                     bool isGhosted = false, unsigned int dof = 1) const;
     ...
     */
    template <typename T>
    void writeToGhostsEnd(T* vec, DA_FLAGS::WriteMode mode,
                          unsigned int dof = 1);
     ...
     * */
    template <typename T>
    void nodalVecToGhostedNodal(const T* in, T*& out, bool isAllocated = false,
    ...
    template <typename T>
    void ghostedNodalToNodalVec(const T* gVec, T*& local,
     ...
     * @param[in/out] eleVec: allocated eleVec (size: numNodesPerElement*dof)
     * after the function call will have the interpolated values.
     * @param[in] eleID: element ID
     * @param[in] dof: degrees of freedoms
     * */
    template <typename T>
    void getElementNodalValues(const T* in, T* eleVecOut, unsigned int eleID,
                               unsigned int dof = 1) const;
     ...
     * */
    template <typename T>
    void eleVecToVecAccumilation(T* out, const T* eleVecIn, unsigned int eleID,
                                 unsigned int dof = 1) const;

    /**
     * @brief Computes the octree writable boundary nodes.
     * @param[in/out]: bdyIndex: boundary index nodal locations (only writable
     * boundary locations)
     * @param[in] isGhosted: change the indices to the ghosted array.
     * */
    void getOctreeBoundaryNodeIndices(std::vector<unsigned int>& bdyIndex,
     ...
     * @param nodes   Indices into the nodes of the given element. Should be
     allocated by the user prior to calling.
       @return -1 for error
    */
    int getNodeIndices(DendroIntL* nodeIdx, unsigned int ele,
    ...
    /**
     * @brief initialize a variable vector to a function depends on spatial
     * coords.
     ...
     * */
    template <typename T>
    void setVectorByFunction(T* local, std::function<void(T, T, T, T*)> func,
                             bool isElemental = false, bool isGhosted = false,
                             unsigned int dof = 1) const;
     ...
     * coords.
     * @param[in/out] local: allocated vector, initialized vector
     * @param[in] value: user specified scalar values (size should be the  dof
     * size)
     * @param [in] isElemental: True if creating a elemental vector (cell data
     * vector) false for a nodal vector
     * @param [in] isGhosted: True will allocate ghost nodal values as well,
     ...
     * */
    template <typename T>
    void setVectorByScalar(T* local, const T* value, bool isElemental = false,
                           bool isGhosted = false, unsigned int dof = 1) const;
     ...
     * */
    template <typename T>
    void vecTopvtu(T* local, const char* fPrefix, char** nodalVarNames = NULL,
                   bool isElemental = false, bool isGhosted = false,
                   unsigned int dof = 1);
     ...
     * */
    inline unsigned int getLevel(unsigned int ele) const {
        return m_uiMesh->getAllElements()[ele].getLevel();
    }
     ...
     * @param[in] ele: elementID of the current octant
     * */
    inline ot::TreeNode getOctant(unsigned int ele) const {
        return m_uiMesh->getAllElements()[ele];
    }

    /**
     * @brief returns a pointer to a dof index,
     * @param [in] in: input vector pointer
     * @param [in] dofInex: dof index which is the pointer is needed, should be
     * less than dof, value the vector created.
     * @param [in] isElemental: true if this is an elemental vector/ false
     * otherwise
     * @param [in] isGhosted: true if this is a ghosted vector
     * @return pointer to dofIndex.
     * */
    template <typename T>
    T* getVecPointerToDof(T* in, unsigned int dofInex, bool isElemental = false,
                          bool isGhosted = false) const;

    /**
     * @brief copy vecotor to sorce to destination, assumes the same number of
     * dof.
     ...
     * */
    template <typename T>
    void copyVectors(T* dest, const T* source, bool isElemental = false,
                     bool isGhosted = false, unsigned int dof = 1) const;
     ...
     * */
    template <typename T>
    void copyVector(T* dest, const T* source, bool isElemental = false,
                    bool isGhosted = false) const;

    /**
     * @brief: Performs remesh based on the DA_FLAGS::Refine, which specifies no
     * change, refine or coarsen.
     * @param[in] flags: refinement flags.
     * @param[in] sz: size of the array flags (needs to be size of the local
     * elements)
     * @param[in] grainSz: rougly the number of octants per core you need when
     * you create the new da.
     * @param[in] ld_tol: load imbalance tolerance.
     * @param[in] sfK: splitter fix factor. better to be power of two. increase
     * the value to 128 when running on > 64,000 cores
     * @return: Specifies the new grid, with new DA.
     */
    ot::DA* remesh(const DA_FLAGS::Refine* flags, unsigned int sz,
                   unsigned int grainSz = 100, double ld_bal = 0.3,
                   unsigned int sfK                               = 2,
                   unsigned int (*getWeight)(const ot::TreeNode*) = NULL) const;
     ...
     */
    ot::DA* repartition(
        unsigned int grainSz = 100, double ld_bal = 0.3, unsigned int sfK = 2,
        unsigned int (*getWeight)(const ot::TreeNode*) = NULL) const;

    /**
     * @brief performs grid transfer operations after the remesh.
     * @param[in] varIn: variable defined by oldDA
     * @param[out] varOut: variable defined by newDA. interpolate varOut from
     * varIn. (Note: varOut allocated inside the function, no need to allocate
     * outside)
    ...
    void intergridTransfer(
        const T* varIn, T*& varOut, const ot::DA* newDA,
        bool isElemental = false, bool isGhosted = false, unsigned int dof = 1,
    ...
    PetscErrorCode petscCreateVector(Vec& local, bool isElemental,
                                     bool isGhosted, unsigned int dof) const;
     ...
     */
    PetscErrorCode createMatrix(Mat& M, MatType mtype,
                                unsigned int dof = 1) const;
     ...
     * @param[in] dof: degrees of freedoms
     * */
    PetscErrorCode petscNodalVecToGhostedNodal(const Vec& in, Vec& out,
                                               bool isAllocated = false,
                                               unsigned int dof = 1) const;
    ...
    PetscErrorCode petscGhostedNodalToNodalVec(const Vec& gVec, Vec& local,
                                               bool isAllocated = false,
                                               unsigned int dof = 1) const;

    /**
     * @brief Initiate the ghost nodal value exchange
     * @param[in] vec: vector in need to perform ghost exchange (Need be ghosted
     * vector)
     ...
     * */
    template <typename T>
    void petscSetVectorByFunction(Vec& local,
     ...
     * */
    template <typename T>
    void petscSetVectorByScalar(Vec& local, const T* value,
                                bool isElemental = false,
                                bool isGhosted   = false,
                                unsigned int dof = 1) const;
     ...
     * @param [in] dof: degrees of freedoms
     * */
    void petscVecTopvtu(const Vec& local, const char* fPrefix,
                        char** nodalVarNames = NULL, bool isElemental = false,
                        bool isGhosted = false, unsigned int dof = 1);

    /**
     * @brief a wrapper for setting values into the Matrix.  This internally
     calls PETSc's MatSetValues() function.
     * Call PETSc's MatAssembly routines to assemble the matrix after setting
     the values. It would be more efficient to set values in chunks by calling
     this function multiple times with different sets of values instead of a
     single call at the end of the loop. One can use the size of 'records' to
     determine the number of such chunks. Calls to this function with the
     INSERT_VALUES and ADD_VALUES options cannot be mixed without intervening
     calls to PETSc's MatAssembly routines.
    ...
    */
    PetscErrorCode petscSetValuesInMatrix(Mat mat,
                                          std::vector<ot::MatRecord>& records,
                                          unsigned int dof,
                                          InsertMode mode) const;

    /**
     * @brief: Dendro 5 vectors with multiple dof values are stored in
     * 00000000,11111111 for multiple dof values, if the user wants to perform
     * matrix based computations (i.e. not use matrix free option), this will
     * reorder the vector valules 01,01,01,...
     * @param [in,out] v1: input and out put vector,
     * @param [in] dof: number of dof.
     */
    PetscErrorCode petscChangeVecToMatBased(Vec& v1, bool isElemental,
                                            bool isGhosted,
                                            unsigned int dof = 1) const;
     ...
     * @param [in] dof: number of dof.
     */
    PetscErrorCode petscChangeVecToMatFree(Vec& v1, bool isElemental,
                                           bool isGhosted,
                                           unsigned int dof = 1) const;

    /**
     * @brief performs grid transfer operations after the remesh with Petsc
     * Vectors.
     * @param[in] varIn: variable defined by oldDA
     * @param[out] varOut: variable defined by newDA. interpolate varOut from
     * varIn. (Note: varOut allocated inside the function, no need to allocate
     ...
     */
    template <typename T>
    void petscIntergridTransfer(
        const Vec& varIn, Vec& varOut, const ot::DA* newDA,
        bool isElemental = false, bool isGhosted = false, unsigned int dof = 1,
        INTERGRID_TRANSFER_MODE mode = INTERGRID_TRANSFER_MODE::INJECTION);
