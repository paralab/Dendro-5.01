 ...
 *@brief This is modified to perform matrix-free iterative methods for solving
 *linear systems that generated from the Galerkin-Discretization (FEM).
 */
//
    ...
    if (mesh->isActive()) {
        ...
        if (status != 0) {
            for (unsigned int i = 1; i <= max_iter; i++) {
                ...
                alpha = (dot(r0 + nodeLocalBegin, r0 + nodeLocalBegin,
                             local_dof, activeComm) /
                         dot(p + nodeLocalBegin, Ap + nodeLocalBegin, local_dof,
                             activeComm));
                par::Mpi_Bcast(&alpha, 1, 0, activeComm);

                // if(!activeRank) std::cout<<"<r_0,r_0> :
                // "<<dot(r0+nodeLocalBegin,r0+nodeLocalBegin,local_dof,activeComm)<<"
                // <p,Ap>:
                // "<<dot(p+nodeLocalBegin,Ap+nodeLocalBegin,local_dof,activeComm)<<"
                // alpha "<<alpha<<std::endl;

                /*Real normAx=normL2(Ax+nodeLocalBegin,local_dof,activeComm);
                Real normx=normL2(x+nodeLocalBegin,local_dof,activeComm);
                if(!activeRank)std::cout<<"l2(Ax): "<<normAx<<" l2(x):
                "<<normx<<std::endl;*/

                // if(!activeRank) std::cout<<"rank: " <<activeRank<<" alpha:
                // "<<alpha<<std::endl; std::cout<<"alpha: "<<alpha<<std::endl;
                ...
                beta = (dot(r1 + nodeLocalBegin, r1 + nodeLocalBegin, local_dof,
                            activeComm) /
                        dot(r0 + nodeLocalBegin, r0 + nodeLocalBegin, local_dof,
                            activeComm));
                ...
                const char* varNames[] = {"U"};
                const double* var[]    = {x};
                io::vtk::mesh2vtuFine(mesh, fPrefix, 0, NULL, NULL, 1, varNames,
                                      var);
            }
