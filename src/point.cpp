...
Point::Point(int newx, int newy, int newz)
    : _x(static_cast<double>(newx)),
      _y(static_cast<double>(newy)),
      _z(static_cast<double>(newz)) {}
...
Point Point::operator-() const { return Point(-_x, -_y, -_z); }
...
Point Point::TransMatMultiply(double *transMat, Point inPoint) {
    ...
    outPoint._x = transMat[0] * inPoint._x + transMat[4] * inPoint._y +
                  transMat[8] * inPoint._z + transMat[12];
    outPoint._y = transMat[1] * inPoint._x + transMat[5] * inPoint._y +
                  transMat[9] * inPoint._z + transMat[13];
