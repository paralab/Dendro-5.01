...
TreeNode::TreeNode(const unsigned int x, const unsigned int y,
                   const unsigned int z, const unsigned int lev,
                   const unsigned int dim, const unsigned int maxDepth) {
    // m_uiMaxDepth = maxDepth;
    m_uiX = x;
    if (dim > 1) {
...
std::ostream& operator<<(std::ostream& os, TreeNode const& other) {
    return (os << other.getX() << " " << other.getY() << " " << other.getZ()
               << " " << other.getLevel());
}  // end fn.
...
bool TreeNode::isBoundaryOctant(const TreeNode& block, int type,
...
int TreeNode::addChildren(std::vector<ot::TreeNode>& children) const {
    ...
    TreeNode tmpNode0(1, m_uiX, m_uiY, m_uiZ,
                      ((m_uiLevel & ot::TreeNode::MAX_LEVEL) + 1), m_uiDim,
                      m_uiMaxDepth);
    children[childrenSz + 0] = tmpNode0;

    TreeNode tmpNode1(1, (m_uiX + len), m_uiY, m_uiZ,
                      ((m_uiLevel & ot::TreeNode::MAX_LEVEL) + 1), m_uiDim,
                      m_uiMaxDepth);
    children[childrenSz + 1] = tmpNode1;

    if (dim >= 2) {
        TreeNode tmpNode2(1, m_uiX, (m_uiY + len), m_uiZ,
                          ((m_uiLevel & ot::TreeNode::MAX_LEVEL) + 1), m_uiDim,
                          m_uiMaxDepth);
        children[childrenSz + 2] = tmpNode2;

        TreeNode tmpNode3(1, (m_uiX + len), (m_uiY + len), m_uiZ,
                          ((m_uiLevel & ot::TreeNode::MAX_LEVEL) + 1), m_uiDim,
                          m_uiMaxDepth);
    ...
    if (dim == 3) {
        TreeNode tmpNode4(1, m_uiX, m_uiY, (m_uiZ + len),
                          ((m_uiLevel & ot::TreeNode::MAX_LEVEL) + 1), m_uiDim,
                          m_uiMaxDepth);
        children[childrenSz + 4] = tmpNode4;

        TreeNode tmpNode5(1, (m_uiX + len), m_uiY, (m_uiZ + len),
                          ((m_uiLevel & ot::TreeNode::MAX_LEVEL) + 1), m_uiDim,
                          m_uiMaxDepth);
        children[childrenSz + 5] = tmpNode5;

        TreeNode tmpNode6(1, m_uiX, (m_uiY + len), (m_uiZ + len),
                          ((m_uiLevel & ot::TreeNode::MAX_LEVEL) + 1), m_uiDim,
                          m_uiMaxDepth);
        children[childrenSz + 6] = tmpNode6;

        TreeNode tmpNode7(1, (m_uiX + len), (m_uiY + len), (m_uiZ + len),
                          ((m_uiLevel & ot::TreeNode::MAX_LEVEL) + 1), m_uiDim,
                          m_uiMaxDepth);
...
TreeNode TreeNode::getDFD() const {
    ...
    keys[0] = (ot::TreeNode(this->minX(), this->minY(), this->minZ(),
                            m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
    keys[1] = (ot::TreeNode((this->maxX() - 1), this->minY(), this->minZ(),
                            m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
    keys[2] = (ot::TreeNode(this->minX(), (this->maxY() - 1), this->minZ(),
                            m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
    keys[3] = (ot::TreeNode((this->maxX() - 1), (this->maxY() - 1),
                            this->minZ(), m_uiMaxDepth, m_uiDim, m_uiMaxDepth));

    if (m_uiDim == 3) {
        keys[4] = (ot::TreeNode(this->minX(), this->minY(), (this->maxZ() - 1),
                                m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        keys[5] =
            (ot::TreeNode((this->maxX() - 1), this->minY(), (this->maxZ() - 1),
                          m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        keys[6] =
            (ot::TreeNode(this->minX(), (this->maxY() - 1), (this->maxZ() - 1),
                          m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        keys[7] = (ot::TreeNode((this->maxX() - 1), (this->maxY() - 1),
                                (this->maxZ() - 1), m_uiMaxDepth, m_uiDim,
                                m_uiMaxDepth));
    }

    std::vector<ot::TreeNode> tmpNodes;
    ot::TreeNode rootNode(0, 0, 0, 0, m_uiDim, m_uiMaxDepth);
    SFC::seqSort::SFC_treeSort(&(*(keys.begin())), keys.size(), tmpNodes,
                               tmpNodes, tmpNodes, m_uiMaxDepth, m_uiMaxDepth,
