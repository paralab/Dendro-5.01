...
ot::subDA::subDA(ot::DA* da,
    ...
    const unsigned int allEleSize = m_da->getLocalElemSz() +
                                    m_da->getPreGhostElementSize() +
                                    m_da->getPostGhostElementSize();
    const unsigned int allNodeSize =
        m_da->getLocalNodalSz() + m_da->getPreAndPostGhostNodeSize();
    ...
    if (!m_da->isActive()) {
        ...
        for (m_da->init<ot::DA_FLAGS::WRITABLE>();
             m_da->curr() < m_da->end<ot::DA_FLAGS::WRITABLE>();
             m_da->next<ot::DA_FLAGS::WRITABLE>()) {
            lev = m_da->getLevel(m_da->curr());
            for (unsigned int k = 0; k < (eleOrder + 1); k++)
                for (unsigned int j = 0; j < (eleOrder + 1); j++)
                    for (unsigned int i = 0; i < (eleOrder + 1); i++) {
                        nodeLookUp_DG =
                            e2n_dg[(m_da->curr()) * nPe +
                                   k * (eleOrder + 1) * (eleOrder + 1) +
                                   j * (eleOrder + 1) + i];
                        _mesh->dg2eijk(nodeLookUp_DG, ownerID, ii_x, jj_y,
                                       kk_z);
                        const unsigned int len = 1u << (m_uiMaxDepth - lev);
                        coords[m_uiDim * (k * (eleOrder + 1) * (eleOrder + 1) +
                                          j * (eleOrder + 1) + i) +
                               0] = xFac * (pNodes[ownerID].getX() +
                                            ii_x * (len / (eleOrder)));
                        coords[m_uiDim * (k * (eleOrder + 1) * (eleOrder + 1) +
                                          j * (eleOrder + 1) + i) +
                               1] = yFac * (pNodes[ownerID].getY() +
                                            jj_y * (len / (eleOrder)));
                        coords[m_uiDim * (k * (eleOrder + 1) * (eleOrder + 1) +
                                          j * (eleOrder + 1) + i) +
                        ...
                        z         = coords[m_uiDim *
                                       (k * (eleOrder + 1) * (eleOrder + 1) +
                                        j * (eleOrder + 1) + i) +
                                   2];
                        dist[k * (eleOrder + 1) * (eleOrder + 1) +
                             j * (eleOrder + 1) + i] = fx_retain(x, y, z);
                    }
            if (std::all_of(dist.begin(), dist.end(), inside)) {
                ...
                for (unsigned int node = 0; node < nPe; node++) {
                    if (nodeLocalBegin <= indices[node] &&
                        indices[node] < nodeLocalEnd)
        ...
        if (m_uiTotalNodalSz == 0) {
            std::cout << "subDA[Error] on processor " << m_uiRankActive
                      << ": empty subDA of a active DA (use weighted partition "
                         "to avoid this) "
                      << __func__ << std::endl;
        }
        ...
        for (unsigned int e = _mesh->getElementPreGhostBegin();
             e < _mesh->getElementPreGhostEnd(); e++) {
        ...
        for (unsigned int e = _mesh->getElementLocalBegin();
             e < _mesh->getElementLocalEnd(); e++) {
            if (m_ucpSkipList[e] == 0) {
                m_uip_sub2DA_ElemMap[(m_uiNumPreGhostElements) +
                                     m_uiNumLocalElements] = e;
                m_uiNumLocalElements++;
            }
        }

        m_uiNumPostGhostElements = 0;
        for (unsigned int e = _mesh->getElementPostGhostBegin();
             e < _mesh->getElementPostGhostEnd(); e++) {
            if (m_ucpSkipList[e] == 0) {
                m_uip_sub2DA_ElemMap[(m_uiNumPreGhostElements +
                                      m_uiNumLocalElements) +
                                     m_uiNumPostGhostElements] = e;
                m_uiNumPostGhostElements++;
            }
        }

        m_uiNumPreGhostNodes = 0;
        for (unsigned int e = _mesh->getNodePreGhostBegin();
             e < _mesh->getNodePreGhostEnd(); e++) {
        ...
        m_uiNumLocalNodes = 0;
        for (unsigned int e = _mesh->getNodeLocalBegin();
             e < _mesh->getNodeLocalEnd(); e++) {
            if (m_ucpSkipNodeList[e] == 0) {
                m_uip_sub2DA_NodeMap[(m_uiNumPreGhostNodes) +
                                     m_uiNumLocalNodes] = e;
                m_uiNumLocalNodes++;
            }
        }
        ...
        for (unsigned int e = _mesh->getNodePostGhostBegin();
             e < _mesh->getNodePostGhostEnd(); e++) {
            if (m_ucpSkipNodeList[e] == 0) {
                m_uip_sub2DA_NodeMap[(m_uiNumPreGhostNodes +
                                      m_uiNumLocalNodes) +
                                     m_uiNumPostGhostNodes] = e;
        ...
        m_uiElementPostGhostBegin =
            m_uiNumPreGhostElements + m_uiNumLocalElements;
        m_uiElementPostGhostEnd = m_uiNumPreGhostElements +
                                  m_uiNumLocalElements +
                                  m_uiNumPostGhostElements;
        ...
        const std::vector<unsigned int>& da_sendCount =
            _mesh->getNodalSendCounts();
        const std::vector<unsigned int>& da_recvCount =
            _mesh->getNodalRecvCounts();
        const std::vector<unsigned int>& da_sendOffset =
            _mesh->getNodalSendOffsets();
        const std::vector<unsigned int>& da_recvOffset =
            _mesh->getNodalRecvOffsets();
        ...
        for (unsigned int p = 0; p < spList.size(); ++p) {
            unsigned int cnt = 0;
            for (unsigned int w = (da_sendOffset[spList[p]]);
                 w < (da_sendOffset[spList[p]] + da_sendCount[spList[p]]);
                 w++) {
            ...
            if (cnt) m_uiSendProcList.push_back(spList[p]);
        ...
        for (unsigned int p = 0; p < rpList.size(); ++p) {
            unsigned int cnt = 0;
            for (unsigned int w = (da_recvOffset[spList[p]]);
                 w < (da_recvOffset[spList[p]] + da_recvCount[spList[p]]);
                 w++) {
            ...
            if (cnt) m_uiRecvProcList.push_back(rpList[p]);
        ...
        omp_par::scan(&(*(m_uiSendCounts.begin())),
                      &(*(m_uiSendCounts.begin())), npesActive);
        omp_par::scan(&(*(m_uiRecvCounts.begin())),
                      &(*(m_uiRecvOffsets.begin())), npesActive);
    }
}
...
PetscErrorCode ot::subDA::petscCreateVector(Vec& local, bool isElemental,
                                            bool isGhosted,
