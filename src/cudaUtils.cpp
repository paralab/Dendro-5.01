...
cudaDeviceProp* getGPUDeviceInfo(unsigned int device) {
    ...
    cudaMemcpy(__deviceProp, &deviceProp, sizeof(cudaDeviceProp),
...
void computeDendroBlockToGPUMap(const ot::Block* blkList,
    ...
    const unsigned int TOTAL_GPU_BLOCKS =
        dendroBlockToGPUBlockMap[numBlocks - 1] + 1;
    ...
    while (blkCount < numBlocks) {
        // std::cout<<"gpublock ID: "<<gpuBlockId<<" blkCount:
        // "<<blkCount<<std::endl;
        blockMap[2 * gpuBlockId] = blkCount;
        if (blkCount < (numBlocks - 1) &&
            dendroBlockToGPUBlockMap[blkCount] ==
                dendroBlockToGPUBlockMap[blkCount + 1]) {
            blkCount++;
            while (blkCount < (numBlocks - 1) &&
                   dendroBlockToGPUBlockMap[blkCount] ==
                       dendroBlockToGPUBlockMap[blkCount + 1])
                blkCount++;
    ...
    /*for(unsigned int blk=0;blk<numBlocks;++blk)
        std::cout<<"blk: "<<blk<<" gpu bid : "<<dendroBlockToGPUBlockMap[blk]<<"
    block size: "<<blkList[blk].get1DArraySize()<<std::endl;

    for(unsigned int bid=0;bid<TOTAL_GPU_BLOCKS;bid++)
        std::cout<<"block id : "<<bid<<" begin:
    "<<inverseDendroBlockToGPUBlockMap[2*bid]<<" end :
    "<<inverseDendroBlockToGPUBlockMap[2*bid+1]<<std::endl;*/
