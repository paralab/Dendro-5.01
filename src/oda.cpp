 ...
 *@brief The class that manages the octree mesh that support FEM computations.
 ...
 support Dendro-5.0  Currently we use mesh based FEM computations but in future
 we will move towards the mesh - free FEM computation methods. *Distributed
 Array (DA) methods currently uses ot::Mesh class to provide interface to write
 FEM loops.
 ...
 * */
DA::DA(std::vector<ot::TreeNode>& balOct, MPI_Comm comm, unsigned int order,
       unsigned int grainSz, double sfc_tol, SM_TYPE smType) {
...
DA::DA(const Point* pts, unsigned int numPts, Point pt_min, Point pt_max,
       MPI_Comm comm, unsigned int order, unsigned int grainSz, double sfc_tol,
       SM_TYPE smType) {
    if (m_uiDim == 3) {
        ...
        for (unsigned int i = 0; i < numPts; i++) {
            p_int[0] =
                (pts[i].x() - pt_min.x()) * ((1u << m_uiMaxDepth) / gLens[0]);
            p_int[1] =
                (pts[i].y() - pt_min.y()) * ((1u << m_uiMaxDepth) / gLens[1]);
            p_int[2] =
                (pts[i].z() - pt_min.z()) * ((1u << m_uiMaxDepth) / gLens[2]);

            nodes[i] = ot::TreeNode(p_int[0], p_int[1], p_int[2], m_uiMaxDepth,
                                    m_uiDim, m_uiMaxDepth);
        }

        ot::TreeNode root(0, 0, 0, 0, m_uiDim, m_uiMaxDepth);

        SFC::parSort::SFC_treeSort(nodes, tmpNodes, tmpNodes, tmpNodes,
                                   DENDRO_DEFAULT_LB_TOL, m_uiMaxDepth, root,
                                   ROOT_ROTATION, 1, TS_CONSTRUCT_OCTREE,
                                   DENDRO_DEFAULT_SF_K, comm);
        ...
        // generates the mesh.
        m_uiMesh = new ot::Mesh(balOct, 1, order, comm, false, smType, grainSz,
                                sfc_tol);
        balOct.clear();
...
#ifdef DEBUG_ODA
        for (init<DA_FLAGS::INDEPENDENT>();
             curr() < end<DA_FLAGS::INDEPENDENT>();
             next<DA_FLAGS::INDEPENDENT>())
            independent.push_back(allElements[curr()]);

        for (init<DA_FLAGS::W_DEPENDENT>();
             curr() < end<DA_FLAGS::W_DEPENDENT>();
             next<DA_FLAGS::W_DEPENDENT>())
            w_dependent.push_back(allElements[curr()]);
...
void DA::getOctreeBoundaryNodeIndices(std::vector<unsigned int>& bdyIndex,
    ...
    for (init<ot::DA_FLAGS::W_BOUNDARY>();
         curr() < end<ot::DA_FLAGS::W_BOUNDARY>();
         next<ot::DA_FLAGS::W_BOUNDARY>()) {
        ele    = curr();
        tmpOct = allElements[ele];
        // std::cout<<"bdy element : "<<tmpOct<<std::endl;

        if (tmpOct.minX() == octreeMin) {  // x_min boundary
        ...
        if (tmpOct.minY() == octreeMin) {  // y_min boundary
        ...
        if (tmpOct.minZ() == octreeMin) {  // z_min boundary
        ...
        if (tmpOct.maxX() == octreeMax) {  // x_max boundary
        ...
        if (tmpOct.maxY() == octreeMax) {  // y_max boundary
        ...
        if (tmpOct.maxZ() == octreeMax) {  // z_max boundary
    ...
    bdyIndex.erase(std::unique(bdyIndex.begin(), bdyIndex.end()),
                   bdyIndex.end());
    ...
    for (unsigned int i = 0; i < bdyIndex.size(); i++) {
        ...
        // std::cout<<"x: "<<coords[i*m_uiDim+0]<<" y "<<coords[i*m_uiDim+1]<<"
        // z: "<<coords[i*m_uiDim+2]<<std::endl;
    }
...
int DA::getNodeIndices(DendroIntL* nodeIdx, unsigned int ele,
...
ot::DA* DA::remesh(const DA_FLAGS::Refine* flags, unsigned int sz,
                   unsigned int grainSz, double ld_bal, unsigned int sfK,
                   unsigned int (*getWeight)(const ot::TreeNode*)) const {
    ...
    if (m_uiMesh->isActive()) {
        ...
        for (unsigned int i = 0; i < sz; i++) {
            ...
            if (flags[i] == DA_FLAGS::Refine::DA_REFINE) {
                if ((allElements[ele].getLevel() + MAXDEAPTH_LEVEL_DIFF + 1) >=
                    m_uiMaxDepth) {
                ...
                bool isRefine   = false;
                if (((i + NUM_CHILDREN - 1) < sz) &&
                    (allElements[ele].getParent() ==
                     allElements[ele + NUM_CHILDREN - 1].getParent()) &&
                    (allElements[ele].getLevel() > 0)) {
...
ot::DA* DA::repartition(unsigned int grainSz, double ld_bal, unsigned int sfK,
                        unsigned int (*getWeight)(const ot::TreeNode*)) const {
    ...
    ot::Mesh* pMesh =
        new ot::Mesh(pNodes, 1, m_uiMesh->getElementOrder(), commGlobal, false,
                     ot::SM_TYPE::FEM_CG, grainSz, ld_bal, sfK, getWeight);
    ot::DA* newDA = new ot::DA(pMesh);

    return newDA;
}

void DA::computeTreeNodeOwnerProc(const ot::TreeNode* pNodes, unsigned int n,
                                  int* ownerranks) {
    if (m_uiMesh->isActive()) {
...
PetscErrorCode DA::petscCreateVector(Vec& local, bool isElemental,
...
PetscErrorCode DA::createMatrix(Mat& M, MatType mtype, unsigned int dof) const {
    if (m_uiMesh->isActive()) {
        ...
        const unsigned int preAllocFactor =
            dof * (53 * m_uiMesh->getNumNodesPerElement());
        ...
        isSuperLU_Dist =
            PETSC_FALSE;  // PetscStrcmp(mtype,MATSUPERLU_DIST,&isSuperLU_Dist);
        ...
        if (isAij || isAijSeq || isAijPrl || isSuperLU || isSuperLU_Dist) {
            if (npesAll > 1) {
                MatMPIAIJSetPreallocation(M, preAllocFactor, PETSC_NULL,
                                          preAllocFactor, PETSC_NULL);
            } else {
...
PetscErrorCode DA::petscNodalVecToGhostedNodal(const Vec& in, Vec& out,
                                               bool isAllocated,
                                               unsigned int dof) const {
    if (!(m_uiMesh->isActive())) return 0;

    unsigned int status = 0;
    if (!isAllocated) status = petscCreateVector(out, false, true, dof);
    ...
    for (unsigned int var = 0; var < dof; var++)
        std::memcpy(
            (outArry + var * m_uiTotalNodalSz + m_uiMesh->getNodeLocalBegin()),
            (inArry + var * m_uiLocalNodalSz),
            sizeof(PetscScalar) * (m_uiLocalNodalSz));
...
PetscErrorCode DA::petscGhostedNodalToNodalVec(const Vec& gVec, Vec& local,
                                               bool isAllocated,
                                               unsigned int dof) const {
    if (!(m_uiMesh->isActive())) return 0;

    unsigned int status = 0;
    if (!isAllocated) status = petscCreateVector(local, false, false, dof);
    ...
    for (unsigned int var = 0; var < dof; var++)
        std::memcpy(
            (localArry + var * m_uiLocalNodalSz),
            (gVecArry + var * m_uiTotalNodalSz + m_uiMesh->getNodeLocalBegin()),
            sizeof(PetscScalar) * (m_uiLocalNodalSz));
...
void DA::petscVecTopvtu(const Vec& local, const char* fPrefix,
                        char** nodalVarNames, bool isElemental, bool isGhosted,
                        unsigned int dof) {
...
PetscErrorCode DA::petscSetValuesInMatrix(Mat mat,
    ...
    if (!records.empty()) {
        ...
        for (unsigned int i = 0; i < records.size(); i++) {
            ...
            // if(records[i].getRowID()>=nodeLocalBegin &&
            // records[i].getRowID()<nodeLocalEnd)
        ...
        while (currRecord < (records.size() - 1)) {
            ...
            colIndices.push_back(static_cast<PetscInt>(
                (records[currRecord].getColDim()) +
                dof *
                    m_uiLocalToGlobalNodalMap[records[currRecord].getColID()]));

            if ((records[currRecord].getRowID() !=
                 records[currRecord + 1].getRowID()) ||
                (records[currRecord].getRowDim() !=
                 records[currRecord + 1].getRowDim())) {
                PetscInt rowId = static_cast<PetscInt>(
                    (records[currRecord].getRowDim()) +
                    dof * m_uiLocalToGlobalNodalMap[records[currRecord]
                                                        .getRowID()]);
                MatSetValues(mat, 1, &rowId, colIndices.size(),
                             (&(*colIndices.begin())), (&(*values.begin())),
                             mode);
        ...
        PetscInt rowId = static_cast<PetscInt>(
            (records[currRecord].getRowDim()) +
            dof * m_uiLocalToGlobalNodalMap[records[currRecord].getRowID()]);
        if (values.empty()) {
            ...
            values.push_back(records[currRecord].getMatVal());
            colIndices.push_back(static_cast<PetscInt>(static_cast<PetscInt>(
                (records[currRecord].getColDim()) +
                dof * m_uiLocalToGlobalNodalMap[records[currRecord]
                                                    .getColID()])));
            MatSetValues(mat, 1, &rowId, colIndices.size(),
                         (&(*colIndices.begin())), (&(*values.begin())), mode);
            colIndices.clear();
            values.clear();
        }
        records.clear();
...
PetscErrorCode DA::petscChangeVecToMatBased(Vec& v1, bool isElemental,
                                            bool isGhosted,
                                            unsigned int dof) const {
    Vec tmp;
...
PetscErrorCode DA::petscChangeVecToMatFree(Vec& v1, bool isElemental,
                                           bool isGhosted,
                                           unsigned int dof) const {
    Vec tmp;
    petscCreateVector(tmp, isElemental, isGhosted, dof);
