...
int mergeOctrees(std::vector<TreeNode>& inOct1, std::vector<TreeNode>& inOct2,
    ...
    if (!(inOct2.empty())) {
        ...
        // decendant of root.
        // Since inOct2 is also complete and sorted, on the first non-empty
        // processor inOct2[0] will also be a deepest first decendant of root.
        // Now mins[0] can be an ancestor, decendant or equal to inOct2[0]
        // (global first element, so only happens on the first processor). So
        // i=0 is handled separately.
        int minsCnt = 0;
        if (inOct2[0] < mins[0]) {
            sendCnts[0]++;
        } else {
    ...
    par::Mpi_Alltoallv_sparse<ot::TreeNode>(&(*(inOct2.begin())), sendCnts,
                                            sendDisps, &(*(outOct.begin())),
                                            recvCnts, recvDisps, comm);
...
int lineariseList(std::vector<ot::TreeNode>& list, MPI_Comm comm) {
    ...
    if (!list.empty()) {
        ...
        if (new_rank) {
            par::Mpi_Irecv<ot::TreeNode>(&lastOnPrev, 1, new_rank - 1, 1,
                                         new_comm, &recvRequest);
        }
        if (new_rank < (new_size - 1)) {
            par::Mpi_Issend<ot::TreeNode>(&lastOctant, 1, new_rank + 1, 1,
                                          new_comm, &sendRequest);
        }
...
unsigned int getNodeWeight(const TreeNode* t) { return t->getWeight(); }
...
TreeNode getNCA(TreeNode first, TreeNode second) {
    ...
    unsigned int maxDiff =
        (unsigned int)(std::max((std::max((fx ^ sx), (fy ^ sy))), (fz ^ sz)));
...
int readNodesFromFile_binary(char* filename, std::vector<TreeNode>& nodes) {
    ...
    do {
        ...
        if (tmp.getDim() != 3) std::cout << "Dim error:" << tmp << std::endl;
        nodes.push_back(tmp);
...
int writeNodesToFile_binary(char* filename,
...
bool areComparable(TreeNode first, TreeNode second) {
    return (((first.getDim()) == (second.getDim())) &&
            ((first.getMaxDepth()) == (second.getMaxDepth())));
}
    ...
    for (unsigned int i = 0; i < in.size(); i++) {
        ...
        if (in[i].isBoundaryOctant(ot::TreeNode::POSITIVE, &bdyFlags)) {
            ...
            if ((bdyFlags &
                ...
                (ot::TreeNode::X_POS_BDY + ot::TreeNode::Z_POS_BDY)) {
                bdy.push_back(ot::TreeNode((1u << maxDepth), y,
                                           (1u << maxDepth), (d + 1), dim,
                                           maxDepth + 1));
            }
            ...
            if ((bdyFlags &
                ...
                (ot::TreeNode::Y_POS_BDY + ot::TreeNode::Z_POS_BDY)) {
                bdy.push_back(ot::TreeNode(x, (1u << maxDepth),
                                           (1u << maxDepth), (d + 1), dim,
                                           maxDepth + 1));
            }
...
void markBoundaryNodesAtAllLevels(std::vector<ot::TreeNode>& finestOctree,
...
void markHangingNodesAtAllLevels(std::vector<ot::TreeNode>& finestOctree,
                                 unsigned int nlevels,
                                 std::vector<ot::TreeNode>* coarserOctrees,
                                 MPI_Comm* activeComms, unsigned int dim,
                                 unsigned int maxDepth) {
    for (int lev = 0; lev < (nlevels - 1); lev++) {
...
void flagNodesType1(std::vector<ot::TreeNode>& in, MPI_Comm comm) {
    ...
    for (unsigned int i = 0; i < keys.size(); i++) {
        ...
        bool found =
            seq::maxLowerBound<TreeNode>(mins, keys[i], idx, NULL, NULL);
        if (!found) {
            part[i] = rank;
        } else {
            part[i] = idx;
    ...
    if (!rank) {
        for (int i = 0; i < npes; i++) {
            std::cout << "In Flag Nodes:  allNumProcsSend[" << i
                      << "] = " << allNumProcsSend[i] << std::endl;
            std::cout << "In Flag Nodes:  allNumProcsRecv[" << i
                      << "] = " << allNumProcsRecv[i] << std::endl;
            std::cout << "In Flag Nodes:  allKeysSz[" << i
                      << "] = " << allKeysSz[i] << std::endl;
            std::cout << "In Flag Nodes:  allTotalRecv[" << i
                      << "] = " << allTotalRecv[i] << std::endl;
        }  // end for i
    }
    ...
    par::Mpi_Alltoallv_sparse<ot::TreeNode>(sendKptr, numKeysSend, sendOffsets,
                                            recvKptr, numKeysRecv, recvOffsets,
                                            comm);
    ...
    for (unsigned int i = 0; i < totalKeys; i++) {
        ...
        resSend[i] = static_cast<char>(
            seq::BinarySearch<ot::TreeNode>(inPtr, in.size(), recvK[i], &idx));
    }  // end for i
    recvK.clear();
...
void flagNodesType2(std::vector<ot::TreeNode>& in, MPI_Comm comm) {
    ...
    // 1. Generate Keys
    // Only true octants need to generate keys. Pseudo-octants don't generate
    // keys. This is because the positive boundary nodes can only be
    // edge-hanging, they can't be face hanging. And the edge hanging node will
    // be identified by the key generated by the true element instead
    for (int i = 0; i < in.size(); i++) {
        unsigned int myLev = inPtr[i].getLevel();
        if (myLev == maxD) {
            continue;
        ...
        if ((myX + mySz) <= (1u << (maxD - 1))) {
            ...
            TreeNode tmp6x((myX + mySz), (myY + (mySz >> 1)),
                           (myZ + (mySz >> 1)), (myLev + 1), dim, maxD);
            keys.push_back(tmp6x);

            // Add C4 of my +x neighbour (Edge Hanging)
            TreeNode tmp4x((myX + mySz), myY, (myZ + (mySz >> 1)), (myLev + 1),
                           dim, maxD);
            keys.push_back(tmp4x);

            // Add C2 of my +x neighbour (Edge Hanging)
            TreeNode tmp2x((myX + mySz), (myY + (mySz >> 1)), myZ, (myLev + 1),
                           dim, maxD);
            keys.push_back(tmp2x);

            if ((myY + mySz) <= (1u << (maxD - 1))) {
                // Add C4 of my +xy neighbour (Edge Hanging)
                TreeNode tmp4xy((myX + mySz), (myY + mySz), (myZ + (mySz >> 1)),
                                (myLev + 1), dim, maxD);
            ...
            if ((myZ + mySz) <= (1u << (maxD - 1))) {
                ...
                TreeNode tmp2xz((myX + mySz), (myY + (mySz >> 1)), (myZ + mySz),
                                (myLev + 1), dim, maxD);
        ...
        if ((myZ + mySz) <= (1u << (maxD - 1))) {
            ...
            TreeNode tmp3z((myX + (mySz >> 1)), (myY + (mySz >> 1)),
                           (myZ + mySz), (myLev + 1), dim, maxD);
            keys.push_back(tmp3z);

            // Add C1 of my +z neighbour (Edge Hanging)
            TreeNode tmp1z((myX + (mySz >> 1)), myY, (myZ + mySz), (myLev + 1),
                           dim, maxD);
            keys.push_back(tmp1z);

            // Add C2 of my +z neighbour (Edge Hanging)
            TreeNode tmp2z(myX, (myY + (mySz >> 1)), (myZ + mySz), (myLev + 1),
                           dim, maxD);
    ...
    par::Mpi_Gather<unsigned int>(&forwardKeysCount, allForwardKeysCount, 1, 0,
                                  comm);
    if (!rank) {
        for (int i = 0; i < npes; i++) {
            std::cout << "rank = " << i
                      << "keys Before = " << allKeysSzBefore[i]
                      << " After = " << allKeysSzAfter[i]
                      << " Forward = " << allForwardKeysCount[i] << std::endl;
        }  // end for i
    }
    ...
    for (int i = 0; i < rank; i++) {
        if (recvCounts[i]) {
            par::Mpi_Irecv<ot::TreeNode>((&(*((recvNodes[i]).begin()))),
                                         recvCounts[i], i, 1, comm,
                                         (requests + (i << 1)));
        }
    }  // end for i
    ...
    for (int i = 0; i < rank; i++) {
        if (sendCounts[i]) {
            par::Mpi_Issend<ot::TreeNode>((keysPtr + sendOffsets[i]),
                                          sendCounts[i], i, 1, comm,
                                          (requests + ((i << 1) + 1)));
        }
    }  // end for i
    ...
    for (int i = sendOffsets[rank]; i < (sendOffsets[rank] + sendCounts[rank]);
         i++) {
        unsigned int retIdx;
    ...
    for (int i = 0; i < rank; i++) {
        for (int j = 0; j < recvCounts[i]; j++) {
            unsigned int retIdx;
            bool found =
                seq::BinarySearch(inPtr, in.size(), recvNodes[i][j], &retIdx);
            if (found) {
                // hanging
                isHanging[retIdx] = true;
            }
...
void flagNodesType3(std::vector<ot::TreeNode>& in, MPI_Comm comm) {
    ...
    // In my view it is not necessary treeNodesTovtk(keys,rank,"oda_keys");
    ...
    for (int i = 0; i < rank; i++) {
        if (numKeysRecv[i]) {
            par::Mpi_Irecv<ot::TreeNode>((recvK + recvOffsets[i]),
                                         numKeysRecv[i], i, 1, comm,
                                         (requests1 + (i << 1)));
        }
    }  // end for i
    ...
    for (int i = 0; i < rank; i++) {
        if (numKeysSend[i]) {
            par::Mpi_Issend<ot::TreeNode>((sendK + sendOffsets[i]),
                                          numKeysSend[i], i, 1, comm,
                                          (requests1 + ((i << 1) + 1)));
        }
    }  // end for i
    ...
    for (unsigned int i = (recvOffsets[rank] + numKeysRecv[rank]);
         i < totalKeys; i++) {
        unsigned int idx;
        resSend[i] =
            seq::BinarySearch<ot::TreeNode>(inPtr, in.size(), recvK[i], &idx);
    ...
    for (int i = 0; i < rank; i++) {
        if (numKeysSend[i]) {
            par::Mpi_Irecv<bool>((resRecv + sendOffsets[i]), numKeysSend[i], i,
                                 1, comm, (requests2 + (i << 1)));
        }
    }  // end for i
    ...
    for (int i = 0; i < rank; i++) {
        if (numKeysRecv[i]) {
            par::Mpi_Issend<bool>((resSend + recvOffsets[i]), numKeysRecv[i], i,
                                  1, comm, (requests2 + ((i << 1) + 1)));
        }
    }  // end for i
    ...
    for (int i = recvOffsets[rank]; i < (recvOffsets[rank] + numKeysRecv[rank]);
         i++) {
        if (resSend[i]) {
            isHanging[comm_map[sendOffsets[rank] + i - recvOffsets[rank]]] =
                true;
        }
    }  // end for i
