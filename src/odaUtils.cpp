    ...
    for (unsigned int ele = octantLocalBegin; ele < octantLocalEnd; ele++) {
        ...
        for (unsigned int node = 0; node < nPe; node++) {
            ...
            if ((lookUp != LOOK_UP_TABLE_DEFAULT) &&
                !(lookUp >= nodeLocalBegin && lookUp < nodeLocalEnd)) {
        ...
        if ((allElements[ele].minX() == octTreeMin ||
             allElements[ele].minY() == octTreeMin ||
             allElements[ele].minZ() == octTreeMin ||
             allElements[ele].maxX() == octTreeMax ||
             allElements[ele].maxY() == octTreeMax ||
             allElements[ele].maxZ() == octTreeMax)) {
            binOp::setBit(flagList[ele], ODA_W_BOUNDARY_FLAG_BIT);
        }
    }

    const std::vector<unsigned int>& lev1_Octants =
        pMesh->getLevel1GhostElementIndices();
...
void ot::computeLocalToGlobalNodalMap(const ot::Mesh* pMesh,
                                      ...
                                      std::vector<DendroIntL>& nodalOffset) {
    map.clear();
    ...
    if (pMesh->isActive()) {
        ...
        if (npesAll == 1) {
            ...
            for (unsigned int node = nodePreGhostBegin; node < nodePreGhostEnd;
                 node++)
                map[node] = 0;

            for (unsigned int node = nodePostGhostBegin;
                 node < nodePostGhostEnd; node++)
                map[node] = 0;
            ...
            for (unsigned int p = 0; p < activeNpes; p++) nodalCount[p] = 0;
            ...
            globalNodeSz =
                nodalOffset[activeNpes - 1] + nodalCount[activeNpes - 1];
            ...
            const unsigned int* sendNodeOffset =
                &(*(pMesh->getNodalSendOffsets().begin()));
            const unsigned int* sendNodeCounts =
                &(*(pMesh->getNodalSendCounts().begin()));

            const unsigned int* recvNodeOffset =
                &(*(pMesh->getNodalRecvOffsets().begin()));
            const unsigned int* recvNodeCounts =
                &(*(pMesh->getNodalRecvCounts().begin()));

            const unsigned int* sendNodeSM =
                &(*(pMesh->getSendNodeSM().begin()));
            const unsigned int* recvNodeSM =
                &(*(pMesh->getRecvNodeSM().begin()));

            DendroIntL* sendBufferNodes =
                new DendroIntL[sendNodeOffset[activeNpes - 1] +
                               sendNodeCounts[activeNpes - 1]];
            DendroIntL* recvBufferNodes =
                new DendroIntL[recvNodeOffset[activeNpes - 1] +
                               recvNodeCounts[activeNpes - 1]];

            for (unsigned int p = 0; p < activeNpes; p++) {
                for (unsigned int k = sendNodeOffset[p];
                     k < (sendNodeOffset[p] + sendNodeCounts[p]); k++) {
                    sendBufferNodes[k] = map[sendNodeSM[k]];
                }
            }

#ifdef ALLTOALL_SPARSE
            par::Mpi_Alltoallv_sparse(sendBufferNodes, (int*)sendNodeCounts,
                                      (int*)sendNodeOffset, recvBufferNodes,
                                      (int*)recvNodeCounts,
                                      (int*)recvNodeOffset, activeComm);
#else
            par::Mpi_Alltoallv(sendBufferNodes, (int*)sendNodeCounts,
                               (int*)sendNodeOffset, recvBufferNodes,
                               (int*)recvNodeCounts, (int*)recvNodeOffset,
                               activeComm);
#endif

            for (unsigned int p = 0; p < activeNpes; p++) {
                for (unsigned int k = recvNodeOffset[p];
                     k < (recvNodeOffset[p] + recvNodeCounts[p]); k++) {
