...
/**
 * !!! Warning : If you use splitComm functions you should free the allocated
 * commnunicator after use. MPI_Comm Free.
 */
...
unsigned int splitCommBinary(MPI_Comm orig_comm, MPI_Comm* new_comm) {
    ...
    if (static_cast<unsigned int>(rank) < splitterRank) {
        ...
        MPI_Group_incl(orig_group, (npes - splitterRank), ranksDesc,
                       &new_group);
    }
...
int splitCommUsingSplittingRank(int splittingRank, MPI_Comm* new_comm,
    ...
    if (rank < splittingRank) {
        ...
        MPI_Group_incl(orig_group, (size - splittingRank), ranksIdle,
                       &new_group);
    }
