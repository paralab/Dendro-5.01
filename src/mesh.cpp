...
namespace ot {
    ...
    void Mesh::computeSMSpecialPts()
    {
         ...
         //std::cout<<"missing pts dup: "<<m_uiUnzip_3pt.size()<<std::endl;        
         ...
         // No need to increase the maxDepth since the 3rd pt cannot be on an octree boundary. 
         mergeKeys(m_uiUnzip_3pt,m_uiUnzip_3pt_keys);
         assert(seq::test::isUniqueAndSorted(m_uiUnzip_3pt_keys));
         //std::cout<<"missing pts: "<<m_uiUnzip_3pt_keys.size()<<std::endl;        
         ...
         for(unsigned int p=0;p<m_uiActiveNpes;p++)
         {
            ...
            for(unsigned int k=sBegin;k<sEnd;k++)
            {
                ...
                if( (sEleKeys[p] ==  m_uiUnzip_3pt_keys[k]  || sEleKeys[p+1] == m_uiUnzip_3pt_keys[k]) && m_uiUnzip_3pt_keys[k].getOwnerListSize()<1) continue; 
         ...
         for(unsigned int i=0;i<ownerrank.size();i++)
         {
            if( (ownerrank[i] != LOOK_UP_TABLE_DEFAULT ))
            {
                ...
                //std::cout<<"sending key : "<<m_uiUnzip_3pt_keys[i]<<" to proc: "<<ownerrank[i]<<std::endl;
            }
         ...
         for(unsigned int i=0;i<ownerrank.size();i++)
         {
            if( (ownerrank[i] != LOOK_UP_TABLE_DEFAULT ))
            {
                sBuf[m_uiSendOffsetRePt[ownerrank[i]] + m_uiSendCountRePt[ownerrank[i]]] = ot::TreeNode(m_uiUnzip_3pt_keys[i].minX(),m_uiUnzip_3pt_keys[i].minY(),m_uiUnzip_3pt_keys[i].minZ(),m_uiUnzip_3pt_keys[i].getLevel(),m_uiDim,m_uiMaxDepth) ;
                m_uiSendCountRePt[ownerrank[i]]++;
            }
         }
         ...
         //treeNodesTovtk(rBuf,m_uiActiveRank,"rBuf",false);   
         ...
         std::vector<unsigned int> reqEleID;
         for(unsigned int i=0;i<rkey_merged.size();i++)
         {
            ...
            reqEleID.push_back(eleID);
         }
 
         std::sort(reqEleID.begin(),reqEleID.end());
         reqEleID.erase(std::unique(reqEleID.begin(),reqEleID.end()),reqEleID.end());
 
         std::swap(rkey_merged,m_uiUnzip_3pt_recv_keys);
         rkey_merged.clear();
