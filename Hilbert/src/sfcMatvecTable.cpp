...
int main(int argc, char **argv) {
    ...
    if (m_uiDim == 3) {
        ...
        for (unsigned int k = 0; k < m_uiDim; k++)
            for (unsigned int j = 0; j < m_uiDim; j++)
                for (unsigned int i = 0; i < m_uiDim; i++) {
                    ...
                    std::set_intersection(_z[k]->begin(), _z[k]->end(),
                                          _y[j]->begin(), _y[j]->end(),
                                          back_inserter(intersec_zy));
                    std::sort(intersec_zy.begin(), intersec_zy.end());

                    std::set_intersection(
                        intersec_zy.begin(), intersec_zy.end(), _x[i]->begin(),
                        _x[i]->end(), back_inserter(intersec_zyx));
                    std::sort(intersec_zyx.begin(), intersec_zyx.end());

                    for (unsigned int ele = 0; ele < intersec_zyx.size(); ele++)
                        sfcTable[(k * r_z + j * r_y + i) * NUM_CHILDREN + ele] =
                            intersec_zyx[ele];
                }

        std::cout << "/**@brief generated sfc matvec bucking table, do not "
                     "change !!! */ \n\n"
                  << std::endl;
        ...
        std::cout << "static const unsigned char SFC_MATVEC_BUCKET_TABLE[][]={ "
                  << std::endl;
        for (unsigned int r = 0; r < rows; r++) {
            std::cout << " {";
            for (unsigned int child = 0; child < NUM_CHILDREN; child++)
