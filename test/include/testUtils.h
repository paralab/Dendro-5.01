...
/**@brief checks whether the numeric array constains nan values. for block local
 * region */
template <typename T>
...
@fn
@param nodes[in] The vector of nodes that have to be tested.
@return false if any ancestor of any element in the vector is also present in
the vector and true otherwise.
@remark The function uses Binary Search and hence expects the vector of nodes to
be sorted.
*/
 ...
 * @brief for a given sorted 2:1 balanced octree and a block list this procedure
 * checks that for each block, number of octants at level l (reg grid level)
 * cross all blocks should equal to the level l octants across the octree. This
 * ensures that blocks have captured all the octants corresponding to regular
 * grid level l .
 *
...
bool isBlockListValid(const std::vector<T>& pNodes, std::vector<T>& blockList,
