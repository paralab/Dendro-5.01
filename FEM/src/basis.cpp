 ...
 * Gauss-Lobatto quadrature. reference: Mangll implementation.
 * */
#include "basis.h"
...
void jacobip(double alpha, double beta, unsigned int N, double *x, double *p,
    ...
    apb    = alpha + beta;
    gamma0 = pow(2.0, (apb + 1.0)) / (apb + 1.0) * std::tgamma(alpha + 1.0) *
             std::tgamma(beta + 1.0) / std::tgamma(apb + 1.0);
    ...
    if (N > 0) {
        for (unsigned int k = 0; k < xn; k++) {
            pl[xn + k] =
                ((alpha + beta + 2) * (x[k]) / 2.0 + (alpha - beta) / 2.0) /
                sqrt(gamma1);
        }
    }
...
void gradjacobip(double alpha, double beta, int N, double *x, double *dp,
    ...
    assert(N >= 0 && alpha > -1.0 && beta > -1.0);
    if (N == 0) {
        for (unsigned int k = 0; k < np; k++) dp[k] = 0;
    } else {
...
void jacobigq(double alpha, double beta, int N, double *x, double *w) {
    ...
    for (unsigned int k = 0; k < (N + 1); k++) h1[k] = 2 * k + alpha + beta;
    ...
    for (unsigned int i = 0; i < (N + 1); i++)
        for (unsigned int j = 0; j < (N + 1); j++) {
            if (i != j)
                ...
                ji[i * (N + 1) + j] =
                    (-0.5 * (alpha * alpha - beta * beta) / (h1[j] + 2)) /
                    h1[j];
        }

    for (unsigned int j = 0; j < N;
         j++) {  // added to the diagonal shifted by 1.
        ji[j * (N + 1) + j + 1] =
            2.0 / (h1[j] + 2.0) *
            sqrt((j + 1) * (j + 1 + alpha + beta) * (j + 1 + alpha) *
                 (j + 1 + beta) / (h1[j] + 1.0) / (h1[j] + 3.0));
    }
...
void jacobiglq(double alpha, double beta, int N, double *x, double *w) {
    ...
    if (N >= 2) {
        ...
        for (unsigned int k = 1; k <= (N - 1); k++) x[k] = xint[k - 1];
        ...
        double adgammaN =
            (2.0 * N + alpha + beta + 1.0) / (N * (alpha + beta + N + 1.0));
...
void jacobisf(double alpha, double beta, unsigned int N, double *x, double *p,
    ...
    double py_fac = 0;
    for (unsigned int i = 0; i < (N + 1); i++) py_fac += py[i] * py[i];

    for (unsigned int i = 0; i < (N + 1); i++) py[i] = py[i] / py_fac;

    // for(unsigned int i=0; i< (N+1); i++)
    // {
    //   std::cout<<"i: "<<i<<"root of order :"<<N+1<<" jac :
    //   "<<py[i]<<std::endl;
    // }
