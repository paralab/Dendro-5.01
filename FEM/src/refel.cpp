...
RefElement::RefElement(unsigned int dim, unsigned int order) {
    ...
    im_vec1.resize(
        m_uiNp);  // resize intermidiate values for number of points in 1D case.
    im_vec2.resize(
        m_uiNp);  // resize intermidiate values for number of points in 1D case.
    ...
    for (unsigned int i = 0; i < m_uiNrp; i++) {
        basis::jacobip(alpha, beta, i, (&(*(u_0.begin()))),
                       &(*(Vu_0.begin() + i * m_uiNrp)), m_uiNrp);
        basis::jacobip(alpha, beta, i, (&(*(u_1.begin()))),
                       &(*(Vu_1.begin() + i * m_uiNrp)), m_uiNrp);

        basis::jacobip(alpha, beta, i, (&(*(u.begin()))),
                       &(*(Vu.begin() + i * m_uiNrp)), m_uiNrp);
        basis::gradjacobip(alpha, beta, i, (&(*(u.begin()))),
                           &(*(gradVu.begin() + i * m_uiNrp)), m_uiNrp);

        basis::jacobip(alpha, beta, i, (&(*(r.begin()))),
                       &(*(Vr.begin() + i * m_uiNrp)), m_uiNrp);
        basis::gradjacobip(alpha, beta, i, (&(*(r.begin()))),
                           &(*(gradVr.begin() + i * m_uiNrp)), m_uiNrp);

        basis::jacobip(alpha, beta, i, (&(*(g.begin()))),
                       &(*(Vg.begin() + i * m_uiNrp)), m_uiNrp);
        basis::gradjacobip(alpha, beta, i, (&(*(g.begin()))),
                           &(*(gradVg.begin() + i * m_uiNrp)), m_uiNrp);

        // std::cout<<i<<" r eval : ";printArray_1D(&(*(Vr.begin() +
        // i*m_uiNrp)),m_uiNrp); std::cout<<i<<" r0 eval :
        // ";printArray_1D(&(*(xCh_0.begin())),m_uiNrp); std::cout<<i<<" r1:eval
        // ";printArray_1D(&(*(xCh_1.begin())),m_uiNrp);
    }
    ...
    lapack::lapack_DGESV(m_uiNrp, m_uiNrp, &(*(Vu.begin())), m_uiNrp,
                         &(*(gradVr.begin())), &(*(Dr.begin())), m_uiNrp, info);
    lapack::lapack_DGESV(m_uiNrp, m_uiNrp, &(*(Vu.begin())), m_uiNrp,
                         &(*(gradVg.begin())), &(*(Dg.begin())), m_uiNrp, info);

    lapack::lapack_DGESV(m_uiNrp, m_uiNrp, &(*(Vu.begin())), m_uiNrp,
                         &(*(Vu_0.begin())), &(*(ip_1D_0.begin())), m_uiNrp,
                         info);
    lapack::lapack_DGESV(m_uiNrp, m_uiNrp, &(*(Vu.begin())), m_uiNrp,
                         &(*(Vu_1.begin())), &(*(ip_1D_1.begin())), m_uiNrp,
                         info);

    lapack::lapack_DGESV(m_uiNrp, m_uiNrp, &(*(Vu.begin())), m_uiNrp,
                         &(*(Vg.begin())), &(*(quad_1D.begin())), m_uiNrp,
                         info);
    ...
    if (m_uiDimension == 3 && m_uiOrder == 1) {
        ...
        std::cout << "RefEl: Error invalid dimension and order specified"
                  << std::endl;
    }
    ...
    kron(c2p.data(), c2p_2d.data(), c2p_3d.data(), m_uiNrp, npb2,
         m_uiNrp * m_uiNrp, npb2 * npb2);
...
void RefElement::generateHeaderFile(char* fName) {
    ...
    if (myfile.is_open()) {
        myfile << " /**\n * @author Milinda Fernando\n * @breif This file "
                  "contains the precomputed interpolation matrices for a given "
                  "order, computed based on Hari's HOMG refele.m code. \n * "
                  "@Note: For wavelet based finite differencing we use uniform "
                  "points instead of gll points.\n */";
        myfile << "// This is a machine generated code to initialize "
                  "interpolation matrices for reference element specified. \n";

        myfile << "static double IP_1D_Order_" << m_uiOrder << "_0 [] ={";
        for (unsigned int i = 0; i < m_uiNrp; i++)
            for (unsigned int j = 0; j < m_uiNrp; j++)
                (i != (m_uiNrp - 1) || j != (m_uiNrp - 1))
...
#else
    std::cout << "GenerateHeader file should be run with BLAS enabled. "
              << std::endl;
#endif
}

void RefElement::I3D_Children2ParentInjection(const double* in,
                                              double* out) const {
    const unsigned int npb2 = 2 * m_uiOrder + 1;
        ...
        for (unsigned int j = 0; j < npb2; j++)
            for (unsigned int i = 0; i < npb2; i++)
                if (k % 2 == 0 && j % 2 == 0 && i % 2 == 0)
                    out[(k >> 1u) * m_uiNrp * m_uiNrp + (j >> 1u) * m_uiNrp +
                        (i >> 1u)] = in[k * npb2 * npb2 + j * npb2 + i];
}
