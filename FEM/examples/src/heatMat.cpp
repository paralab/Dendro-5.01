...
void HeatEq::HeatMat::elementalMatVec(const VECType* in, VECType* out,
                                      double* coords, double scale) {
    ...
    Point eleMin(coords[0 * m_uiDim + 0], coords[0 * m_uiDim + 1],
                 coords[0 * m_uiDim + 2]);
    Point eleMax(coords[(nPe - 1) * m_uiDim + 0],
                 coords[(nPe - 1) * m_uiDim + 1],
                 coords[(nPe - 1) * m_uiDim + 2]);
...
bool HeatEq::HeatMat::preMatVec(const VECType* in, VECType* out, double scale) {
    ...
    for (unsigned int i = 0; i < bdyIndex.size(); i++) out[bdyIndex[i]] = 0.0;

    return true;
}

bool HeatEq::HeatMat::postMatVec(const VECType* in, VECType* out,
                                 double scale) {
    // apply boundary conditions.
    std::vector<unsigned int> bdyIndex;
    std::vector<double> bdyCoords;
...
int HeatEq::HeatMat::cgSolve(double* x, double* b, int max_iter, double& tol,
                             unsigned int var) {
    double resid, alpha, beta, rho, rho_1;
    int status = 1;  // 0 indicates it has solved the system within the
                     // specified max_iter, 1 otherwise.
    ...
    if (m_uiOctDA->isActive()) {
        ...
        if (!activeRank) std::cout << "normb = " << normb << std::endl;
...
*/
        ...
        if (status != 0) {
            for (unsigned int i = 1; i <= max_iter; i++) {
                ...
                alpha = (dot(r0, r0, local_dof, activeComm) /
                         dot(p, Ap, local_dof, activeComm));
                par::Mpi_Bcast(&alpha, 1, 0, activeComm);

                // if(!activeRank) std::cout<<"rank: " <<activeRank<<" alpha:
                // "<<alpha<<std::endl;
                ...
                if ((!activeRank) && (i % 10 == 0))
                    std::cout << " iteration : " << i << " residual : " << resid
                              << std::endl;
                ...
                beta = (dot(r1, r1, local_dof, activeComm) /
                        dot(r0, r0, local_dof, activeComm));
                par::Mpi_Bcast(&beta, 1, 0, activeComm);

                // if(!activeRank) std::cout<<"<r_1,r_1> :
                // "<<dot(r1+nodeLocalBegin,r1+nodeLocalBegin,local_dof,activeComm)<<"
                // <r_0,r_0>:
                // "<<dot(r0+nodeLocalBegin,r0+nodeLocalBegin,local_dof,activeComm)<<"
                // beta "<<beta<<std::endl;
