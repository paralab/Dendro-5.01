 ...
 * This tries to interpolate a sin(\theta)+cos(\theta) using legendre
 * polynomials.
 *
 * */
...
int main(int argc, char **argv) {
    if (argc < 5) {
        std::cout << "Usage: " << argv[0] << " dim "
                  << "order "
                  << "x_min "
                  << "x_max "
                  << " freq[enter k for k*pi]" << std::endl;
        return 0;
    }
    ...
    for (unsigned int k = 0; k < max_order; k++) {
        ...
        std::cout << "child 0 : order: " << (k + 1) << " I0: " << std::endl;
        printArray_2D(I0, (k + 2), (k + 2));
        std::cout << "child 1 : order: " << (k + 1) << " I1: " << std::endl;
        printArray_2D(I1, (k + 2), (k + 2));
        ...
        for (unsigned int w = 0; w < (k + 2); w++) {
            ...
            f[k][w] =
                std::sin(k * M_PI * r[k][w]) + std::cos(k * M_PI * r[k][w]);

            r0[k][w] = x_min + w * (0.5 * (x_max + x_min) - x_min) / (k + 1);
            f0[k][w] =
                std::sin(k * M_PI * r0[k][w]) + std::cos(k * M_PI * r0[k][w]);

            r1[k][w] = 0.5 * (x_max + x_min) +
                       w * (x_max - 0.5 * (x_max + x_min)) / (k + 1);
            f1[k][w] =
                std::sin(k * M_PI * r1[k][w]) + std::cos(k * M_PI * r1[k][w]);
        ...
        std::cout << "order: " << (k + 1) << " r: ";
        printArray_1D(r[k], (k + 2));
        std::cout << "order: " << (k + 1) << " f: ";
        printArray_1D(f[k], (k + 2));

        std::cout << "order: " << (k + 1) << " r0: ";
        printArray_1D(r0[k], (k + 2));
        std::cout << "order: " << (k + 1) << " f0: ";
        printArray_1D(f0[k], (k + 2));
        std::cout << "order: " << (k + 1) << " fI0: ";
        printArray_1D(fI0[k], (k + 2));

        std::cout << "order: " << (k + 1) << " r1: ";
        printArray_1D(r1[k], (k + 2));
        std::cout << "order: " << (k + 1) << " f1: ";
        printArray_1D(f1[k], (k + 2));
        std::cout << "order: " << (k + 1) << " fI1: ";
        printArray_1D(fI1[k], (k + 2));

        std::cout << "child 0 : order: " << (k + 1)
                  << " l2 norm(actual- interpolated): "
                  << l2Norm(f0[k], fI0[k], (k + 2)) << std::endl;
        std::cout << "child 1 : order: " << (k + 1)
                  << " l2 norm(actual- interpolated): "
                  << l2Norm(f1[k], fI1[k], (k + 2)) << std::endl;
    }
