 ...
 *@brief This file contains FEM discretization operators for well-known PDEs,
 *this is just to test if the FEM solver (i.e. matvec) works correctly. Later we
 *need to write a SymPy interface which convert a given PDE to it's FEM
 *discretization.
 *
 *
 */
 ...
 * */
template <typename T>
void elementMassMatvec(const T* f_rhs, const ot::TreeNode& elem,
                       const RefElement* refEl, T* out) {
    ...
    // std::cout<<"Mass:  elem: "<<elem<<" ele Sz:
    ...
    for (unsigned int k = 0; k < (eleOrder + 1); k++)
        for (unsigned int j = 0; j < (eleOrder + 1); j++)
            for (unsigned int i = 0; i < (eleOrder + 1); i++)
                out[k * (eleOrder + 1) * (eleOrder + 1) + j * (eleOrder + 1) +
                    i] *= (Jx * Jy * Jz * W1d[i] * W1d[j] * W1d[k]);
...
/**
 * @brief computes the stiffness elemental matvec (i.e. LHS ) of CG
 * discretization
 *
 * */
template <typename T>
void elementStiffnessMatvec(const T* in, const ot::TreeNode& elem,
                            const RefElement* refEl, T* out) {
#ifdef MATVEC_PROFILE
    dendro::timer::sfcmatvec::t_elemMvec.start();
#endif
    ...
    const unsigned int sz =
        1u << (m_uiMaxDepth - elem.getLevel());  // curent element size
    const double refElSz = refEl->getElementSz();
    // x derivative
    ...
    for (unsigned int k = 0; k < (eleOrder + 1); k++)
        for (unsigned int j = 0; j < (eleOrder + 1); j++)
            for (unsigned int i = 0; i < (eleOrder + 1); i++) {
                Qx[k * (eleOrder + 1) * (eleOrder + 1) + j * (eleOrder + 1) +
                   i] *= (((Jy * Jz) / Jx) * W1d[i] * W1d[j] * W1d[k]);
                Qy[k * (eleOrder + 1) * (eleOrder + 1) + j * (eleOrder + 1) +
                   i] *= (((Jx * Jz) / Jy) * W1d[i] * W1d[j] * W1d[k]);
                Qz[k * (eleOrder + 1) * (eleOrder + 1) + j * (eleOrder + 1) +
                   i] *= (((Jx * Jy) / Jz) * W1d[i] * W1d[j] * W1d[k]);
            }
    ...
    for (unsigned int i = 0; i < nPe; i++) out[i] = Qx[i] + Qy[i] + Qz[i];
...
void applyDirichletBoundary(const ot::Mesh* mesh, T* in, T bdyValue) {
    ...
    for (unsigned int ele = mesh->getElementLocalBegin();
        ...
        if (x == d_min) {
            for (unsigned int k = 0; k < (eleOrder + 1); k++)
                for (unsigned int j = 0; j < (eleOrder + 1); j++) {
                    if ((!mesh->isNodeHanging(ele, 0, j, k)) &&
                        (mesh->isNodeLocal(ele, 0, j, k)))
        ...
        if (y == d_min) {
            for (unsigned int k = 0; k < (eleOrder + 1); k++)
                for (unsigned int i = 0; i < (eleOrder + 1); i++) {
                    if ((!mesh->isNodeHanging(ele, i, 0, k)) &&
                        (mesh->isNodeLocal(ele, i, 0, k)))
        ...
        if (z == d_min) {
            for (unsigned int j = 0; j < (eleOrder + 1); j++)
                for (unsigned int i = 0; i < (eleOrder + 1); i++) {
                    if ((!mesh->isNodeHanging(ele, i, j, 0)) &&
                        (mesh->isNodeLocal(ele, i, j, 0)))
        ...
        if (x == d_max) {
            for (unsigned int k = 0; k < (eleOrder + 1); k++)
                for (unsigned int j = 0; j < (eleOrder + 1); j++) {
                    if ((!mesh->isNodeHanging(ele, eleOrder, j, k)) &&
                        (mesh->isNodeLocal(ele, eleOrder, j, k)))
                        in[e2n[ele * nPe + k * ny + j * nx + eleOrder]] =
                            bdy_value;
                }
        }
        ...
        if (y == d_max) {
            for (unsigned int k = 0; k < (eleOrder + 1); k++)
                for (unsigned int i = 0; i < (eleOrder + 1); i++) {
                    if ((!mesh->isNodeHanging(ele, i, eleOrder, k)) &&
                        (mesh->isNodeLocal(ele, i, eleOrder, k)))
                        in[e2n[ele * nPe + k * ny + eleOrder * nx + i]] =
                            bdy_value;
                }
        }
        ...
        if (z == d_max) {
            for (unsigned int j = 0; j < (eleOrder + 1); j++)
                for (unsigned int i = 0; i < (eleOrder + 1); i++) {
                    if ((!mesh->isNodeHanging(ele, i, j, eleOrder)) &&
                        (mesh->isNodeLocal(ele, i, j, eleOrder)))
                        in[e2n[ele * nPe + eleOrder * ny + j * nx + i]] =
                            bdy_value;
                }
        }
    }
...
void matvec(ot::Mesh* mesh, T* in, T* out, bool applyBoundary = true) {
    if (mesh->isActive()) {
        const std::vector<unsigned int> level1Ghost =
            mesh->getLevel1GhostElementIndices();
        ...
        // iterate over ghost elements . (later we can use this to potentially
        ...
        for (unsigned int gIndex = 0; gIndex < level1Ghost.size(); gIndex++) {
            ...
            operators::poisson::elementStiffnessMatvec(
                elementalVec, pNodes[ele], refEl, elementalMVec);
        ...
        for (unsigned int ele = mesh->getElementLocalBegin();
            ...
            /*  if(ele==0)
                  for(unsigned int node=0;node<125;node++)
                      std::cout<<" node value in:
               "<<elementalVec[node]<<std::endl;*/
            ...
            /* if(ele==0)
                 for(unsigned int node=0;node<125;node++)
                     std::cout<<" node value out:
               "<<elementalMVec[node]<<std::endl;*/
...
void computeRHS(ot::Mesh* mesh, T* in, T* out) {
    if (mesh->isActive()) {
        ...
        for (unsigned int gIndex = 0; gIndex < level1Ghost.size(); gIndex++) {
            ...
            operators::poisson::elementMassMatvec(elementalVec, pNodes[ele],
                                                  refEl, elementalMVec);
