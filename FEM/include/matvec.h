 ...
 *@brief This file contains the functionality to perform SFC- based traversal
 *for performing FEM matvec for mesh free and well structured memory access to
 *the array.
 *
...
static const unsigned char
    SFC_MATVEC_BUCKET_TABLE[m_uiDim * m_uiDim * m_uiDim][NUM_CHILDREN] = {
...
// Note: if possible remove this sequenction namespace once the FEM mesh free
// matvec implemented for parallel case as well.
namespace seq {
 ...
 * */
template <typename T>
inline unsigned int computeIJK(const T& pt, const ot::TreeNode elem,
                               const unsigned int dx, const unsigned int logDx,
                               unsigned int* ijk);

template <typename T>
inline bool isInternal(const T& pt, const ot::TreeNode elem,
                       const unsigned int dx, const unsigned int logDx);

/**@brief computes the FEM matrix vector multiplication in the SFC induced
 * traversal ordering.
...
inline void matvec(const T* pt_coords, const da* in, unsigned int pBegin,
                   unsigned int pEnd, const unsigned int lev,
                   const unsigned int maxDepth, const unsigned int order,
                   const ot::TreeNode& pOctant, const RefElement* refEl,
                   da* out, unsigned int mVecType);
...
template <typename T>
inline unsigned int fem::seq::computeIJK(const T& pt, const ot::TreeNode elem,
    ...
    // if((((pt.xint() | ~elem.maxX()) & ((pt.xint() ^ elem.maxX()) |
    // (~elem.maxX() + pt.xint())))>>31u) && (((elem.minX() | ~pt.xint()) &
    // ((elem.minX() ^ pt.xint()) | (~pt.xint() + elem.minX())))>>31u)  &&
    // (((pt.yint() | ~elem.maxY()) & ((pt.yint() ^ elem.maxY()) | (~elem.maxY()
    // + pt.yint())))>>31u) && (((elem.minY() | ~pt.yint()) & ((elem.minY() ^
    // pt.yint()) | (~pt.yint() + elem.minY())))>>31u) && (((pt.zint() |
    // ~elem.maxZ()) & ((pt.zint() ^ elem.maxZ()) | (~elem.maxZ() + pt.zint())))
    // && ((elem.minZ() | ~pt.zint()) & ((elem.minZ() ^ pt.zint()) | (~pt.zint()
    // + elem.minZ())))>>31u) )
    if (elem.minX() <= pt.xint() && elem.minY() <= pt.yint() &&
        elem.minZ() <= pt.zint() && pt.xint() <= elem.maxX() &&
        pt.yint() <= elem.maxY() && pt.zint() <= elem.maxZ()) {
        // check if diff %dx ==0
        if (!((pt.xint() - elem.minX()) & ((1u << logDx) - 1)) &&
            !((pt.yint() - elem.minY()) & ((1u << logDx) - 1)) &&
            !((pt.zint() - elem.minZ()) & ((1u << (logDx)) - 1))) {
...
template <typename T>
inline bool fem::seq::isInternal(const T& pt, const ot::TreeNode elem,
                                 const unsigned int dx,
                                 const unsigned int logDx) {
    return (elem.minX() <= pt.xint() && elem.minY() <= pt.yint() &&
            elem.minZ() <= pt.zint() && pt.xint() <= elem.maxX() &&
            pt.yint() <= elem.maxY() && pt.zint() <= elem.maxZ())
               ? true
               : false;
...
template <typename T, typename da>
inline void fem::seq::matvec(
    const T* pt_coords, const da* in, unsigned int pBegin, unsigned int pEnd,
    const unsigned int lev, const unsigned int maxDepth,
    const unsigned int order, const ot::TreeNode& pOctant,
    const RefElement* refEl, da* out, unsigned int mVecType) {
    ...
    unsigned int cnum;
    const unsigned int pMin[] = {pOctant.minX(), pOctant.minY(),
                                 pOctant.minZ()};
    const unsigned int pMax[] = {pOctant.maxX(), pOctant.maxY(),
                                 pOctant.maxZ()};
    const unsigned int pMid[] = {(pMax[0] + pMin[0]) >> 1u,
                                 (pMax[1] + pMin[1]) >> 1u,
                                 (pMax[2] + pMin[2]) >> 1u};
    ...
    childElem[0] = ot::TreeNode(pOctant.minX(), pOctant.minY(), pOctant.minZ(),
                                pOctant.getLevel() + 1, m_uiDim, maxDepth);
    childElem[1] =
        ot::TreeNode(pOctant.minX() + szb2, pOctant.minY(), pOctant.minZ(),
                     pOctant.getLevel() + 1, m_uiDim, maxDepth);
    childElem[2] =
        ot::TreeNode(pOctant.minX(), pOctant.minY() + szb2, pOctant.minZ(),
                     pOctant.getLevel() + 1, m_uiDim, maxDepth);
    childElem[3] =
        ot::TreeNode(pOctant.minX() + szb2, pOctant.minY() + szb2,
                     pOctant.minZ(), pOctant.getLevel() + 1, m_uiDim, maxDepth);

    childElem[4] =
        ot::TreeNode(pOctant.minX(), pOctant.minY(), pOctant.minZ() + szb2,
                     pOctant.getLevel() + 1, m_uiDim, maxDepth);
    childElem[5] = ot::TreeNode(pOctant.minX() + szb2, pOctant.minY(),
                                pOctant.minZ() + szb2, pOctant.getLevel() + 1,
                                m_uiDim, maxDepth);
    childElem[6] = ot::TreeNode(pOctant.minX(), pOctant.minY() + szb2,
                                pOctant.minZ() + szb2, pOctant.getLevel() + 1,
                                m_uiDim, maxDepth);
    childElem[7] = ot::TreeNode(pOctant.minX() + szb2, pOctant.minY() + szb2,
                                pOctant.minZ() + szb2, pOctant.getLevel() + 1,
                                m_uiDim, maxDepth);
    ...
    for (unsigned int index = 0; index < duplicateCoordIndex.size(); index++) {
        ...
        for (unsigned int child = 0; child < NUM_CHILDREN; child++) {
            cnum = SFC_MATVEC_BUCKET_TABLE[(ijk[2] * SFC_MVEC_TABLE_Rz +
                                            ijk[1] * SFC_MVEC_TABLE_Ry +
                                            ijk[0])][child];
            if (cnum == UCHAR_MAX) break;

            // std::cout<<"pt : "<<pt_coords[p].xint()<<"
            // "<<pt_coords[p].yint()<<" "<<pt_coords[p].zint()<<" ijk:
            // "<<ijk[0]<<" "<<ijk[1]<<" "<<ijk[2]<<" child: "<<child <<" cnum
            // "<<cnum<<" pMid: "<<pMid[2]<<std::endl;
            //   std::cout<<"pt : "<<pt_coords[p].xint()<<"
            //   "<<pt_coords[p].yint()<<" "<<pt_coords[p].zint()<<" ijk:
            //   "<<ijk[0]<<" "<<ijk[1]<<" "<<ijk[2]<<" cnum "<<cnum<<" pMid:
            //   "<<pMid[2]<<"table 0 :
            //   "<<(int)SFC_MATVEC_BUCKET_TABLE[(ijk[2]*SFC_MVEC_TABLE_Rz+ijk[1]*SFC_MVEC_TABLE_Ry+ijk[0])][0]<<"
            //   table 1:
            //   "<<(int)SFC_MATVEC_BUCKET_TABLE[(ijk[2]*SFC_MVEC_TABLE_Rz+ijk[1]*SFC_MVEC_TABLE_Ry+ijk[0])][1]<<std::endl;
            bCounts[cnum]++;
        }
    }
    ...
    for (unsigned int child = 0; child < NUM_CHILDREN; child++) {
        ...
        if (bCounts[child] != 0) {
            u_internal[child] =
                new da[binOp::getNextHighestPowerOfTwo(bCounts[child])];
            v_internal[child] =
                new da[binOp::getNextHighestPowerOfTwo(bCounts[child])];
            pt_internal[child] =
                new T[binOp::getNextHighestPowerOfTwo(bCounts[child])];
        }
    }
    ...
    for (unsigned int index = 0; index < duplicateCoordIndex.size(); index++) {
        ...
        for (unsigned int child = 0; child < NUM_CHILDREN; child++) {
            ...
            pt_internal[cnum][counts[cnum]] =
                pt_coords[duplicateCoordIndex[index]];
            counts[cnum]++;
        }
    }
    ...
    if (computeParent) {
        ...
        for (unsigned int p = pBegin; p < pEnd; p++) {
            if (fem::seq::computeIJK(pt_coords[p], pOctant, dx, logDx, ijk) ==
                PT_INTERNAL_DG_NODE) {
    ...
    for (unsigned int child = 0; child < NUM_CHILDREN; child++) {
        ...
        if (bCounts[child] > nPe) {  // this is a non-leaf node. hence recurse.
            fem::seq::matvec(pt_internal[child], u_internal[child], 0,
                             bCounts[child], lev + 1, maxDepth, order,
                             childElem[child], refEl, v_internal[child],
                             mVecType);
        } else {
            ...
            for (unsigned int i = 0; i < nPe; i++) unzipStatus[i] = false;
            ...
            for (unsigned int p = 0; p < bCounts[child]; p++) {
                ...
                pt_status =
                    fem::seq::computeIJK(pt_internal[child][p],
                                         childElem[child], dxb2, logDxb2, ijk);
            ...
            if (bCounts[child] != nPe) {
                // we need to figure out the missing nodes and perform
                // interpolations.
                ...
                // OCT_DIR_RIGT
                if (!unzipStatus[IDX(order, intepCheckIndex,
                                     intepCheckIndex)]) {
                ...
                // OCT_DIR_UP
                if (!unzipStatus[IDX(intepCheckIndex, order,
                                     intepCheckIndex)]) {
                ...
                // OCT_DIR_FRONT
                if (!unzipStatus[IDX(intepCheckIndex, intepCheckIndex,
                                     order)]) {
                ...
                if (interpReq[OCT_DIR_LEFT]) {
                    ...
                    cnum = (binOp::getBit(child, 2) << 1) |
                           (binOp::getBit(child, 1));
                    // std::cout<<"cnumL: "<<cnum<<" binOp::getBit(child,2)
                    // "<<binOp::getBit(child,2)<<"
                    // binOp::getBit(child,1)"<<binOp::getBit(child,1)<<std::endl;
                ...
                if (interpReq[OCT_DIR_RIGHT]) {
                    ...
                    for (unsigned int k = 0; k < (order + 1); k++)
                        for (unsigned int j = 0; j < (order + 1); j++)
                            interp2D_in[IDX2D(j, k)] =
                                parentVal[IDX(order, j, k)];

                    refEl->I2D_Parent2Child(interp2D_in, interp2D_out, cnum);

                    for (unsigned int k = 0; k < (order + 1); k++)
                        for (unsigned int j = 0; j < (order + 1); j++)
                            u_unzip[IDX(order, j, k)] =
                                interp2D_out[IDX2D(j, k)];
                ...
                if (interpReq[OCT_DIR_UP]) {
                    ...
                    for (unsigned int k = 0; k < (order + 1); k++)
                        for (unsigned int i = 0; i < (order + 1); i++)
                            interp2D_in[IDX2D(i, k)] =
                                parentVal[IDX(i, order, k)];

                    refEl->I2D_Parent2Child(interp2D_in, interp2D_out, cnum);

                    for (unsigned int k = 0; k < (order + 1); k++)
                        for (unsigned int i = 0; i < (order + 1); i++)
                            u_unzip[IDX(i, order, k)] =
                                interp2D_out[IDX2D(i, k)];
                ...
                if (interpReq[OCT_DIR_FRONT]) {
                    ...
                    for (unsigned int j = 0; j < (order + 1); j++)
                        for (unsigned int i = 0; i < (order + 1); i++)
                            interp2D_in[IDX2D(i, j)] =
                                parentVal[IDX(i, j, order)];

                    refEl->I2D_Parent2Child(interp2D_in, interp2D_out, cnum);

                    for (unsigned int j = 0; j < (order + 1); j++)
                        for (unsigned int i = 0; i < (order + 1); i++)
                            u_unzip[IDX(i, j, order)] =
                                interp2D_out[IDX2D(i, j)];
                ...
                if (interpReq[OCT_DIR_LEFT_DOWN] &&
                    (!interpCpy[OCT_DIR_LEFT_DOWN])) {
                    assert(binOp::getBit(child, 0) == 0 &&
                           binOp::getBit(child, 1) == 0);
                ...
                if (interpReq[OCT_DIR_LEFT_UP] &&
                    (!interpCpy[OCT_DIR_LEFT_UP])) {
                    assert(binOp::getBit(child, 0) == 0 &&
                           binOp::getBit(child, 1) == 1);
                    cnum = binOp::getBit(child, 2);
                ...
                if (interpReq[OCT_DIR_LEFT_BACK] &&
                    (!interpCpy[OCT_DIR_LEFT_BACK])) {
                    assert(binOp::getBit(child, 0) == 0 &&
                           binOp::getBit(child, 2) == 0);
                ...
                if (interpReq[OCT_DIR_LEFT_FRONT] &&
                    (!interpCpy[OCT_DIR_LEFT_FRONT])) {
                    assert(binOp::getBit(child, 0) == 0 &&
                           binOp::getBit(child, 2) == 1);
                    cnum = binOp::getBit(child, 1);
            ...
            if (mVecType == 0)  // mass matvec
                fem::operators::poisson::elementMassMatvec(
                    u_unzip, childElem[child], refEl, v_unzip);
            else if (mVecType == 1)  // stifness matvec
                fem::operators::poisson::elementStiffnessMatvec(
                    u_unzip, childElem[child], refEl, v_unzip);
            ...
            if (bCounts[child] != nPe) {
                ...
                if (interpReq[OCT_DIR_RIGHT_UP]) {
                    ...
                    if (!vertexAcc[7]) {
                        parentVal[IDX(order, order, order)] =
                            interp1D_out[order];
...
#endif
                for (unsigned int p = pBegin; p < pEnd; p++) {
                    if (fem::seq::computeIJK(pt_coords[p], pOctant, dx, logDx,
                                             ijk) == PT_INTERNAL_DG_NODE) {
            ...
            for (unsigned int p = 0; p < bCounts[child]; p++) {
                pt_status =
                    fem::seq::computeIJK(pt_internal[child][p],
                                         childElem[child], dxb2, logDxb2, ijk);
                if (pt_status == PT_INTERNAL_DG_NODE &&
                    !accumStatus[IDX(ijk[0], ijk[1], ijk[2])])
