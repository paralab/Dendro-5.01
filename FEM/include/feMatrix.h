...
template <typename T>
void fem_eMatTogMat(ot::MatRecord *eMat, const T *Imat_p2c, unsigned int pOrder,
                    unsigned int dof = 1) {
    ...
    // TODO: need to fix this properly, with efficient matrix matrix
    // multiplication.
    T *IT_Ke_I              = new T[N * N];
            ...
            for (unsigned int i = 0; i < nPe; i++)
                for (unsigned int j = 0; j < nPe; j++) {
                    ...
                    for (unsigned int k = 0; k < nPe; k++) {
                        val += (eMat[offset + i * nPe + k].getMatVal() *
                                Imat_p2c[k * nPe + j]);
                    }
    ...
    for (unsigned int di = 0; di < dof; di++)
        for (unsigned int dj = 0; dj < dof; dj++) {
            ...
            for (unsigned int i = 0; i < nPe; i++)
                for (unsigned int j = 0; j < nPe; j++) {
                    eMat[offset + i * nPe + j].setMatValue(
                        IT_Ke_I[offset + i * nPe + j]);
                }
        }
        ...
        for (unsigned int dj = 0; dj < dof; dj++) {
            ...
            for (unsigned int i = 0; i < nPe; i++)
                for (unsigned int j = 0; j < nPe; j++) {
                    T val = (T)0;
                    for (unsigned int k = 0; k < nPe; k++) {
                        val += (Imat_p2c[k * nPe + i] *
                                eMat[offset + k * nPe + j].getMatVal());
                    }
                    IT_Ke_I[offset + i * nPe + j] = val;
                }
        }
...
class feMatrix : public feMat {
   ...
   public:
    ...
    /**@brief Computes the LHS of the weak formulation, normally the stifness
     ...
     * @param[out] diag : computed diagonal (allocation happens inside the
     * function) for multiple dof values the diagonal is ordered by the dof.
     ...
     *
     * @param blkDiag : list of mat records containing the block diagonal Jacobi
     * preconditioner.
     * @param scale : scale value (default is 1.0).
     */
    virtual void getMatBlockDiagonal(std::vector<ot::MatRecord> &blkDiag,
                                     double scale = 1.0);
     ...
     * @param [in] default parameter scale vector by scale*Ku
     **/
    virtual void elementalMatVec(const VECType *in, VECType *out,
                                 double *coords = NULL, double scale = 1.0) = 0;
    ...
    bool preMat() { return asLeaf().preMat(); }

    /**@brief executed after the matrix assembly */
    bool postMat() { return asLeaf().postMat(); }
     ...
     *
     * */
    void getElementalMatrix(unsigned int eleID,
...
void feMatrix<T>::matVec(const VECType *in, VECType *out, double scale) {
    ...
    for (unsigned int var = 0; var < m_uiDof; var++) val[var] = (VECType)0;
    ...
    for (m_uiOctDA->init<ot::DA_FLAGS::INDEPENDENT>();
         m_uiOctDA->curr() < m_uiOctDA->end<ot::DA_FLAGS::INDEPENDENT>();
         m_uiOctDA->next<ot::DA_FLAGS::INDEPENDENT>()) {
        m_uiOctDA->getElementNodalValues(_in, m_uiEleVecIn, m_uiOctDA->curr(),
                                         m_uiDof);
        ...
        for (unsigned int dof = 0; dof < m_uiDof; dof++) {
            for (unsigned int i = 0; i < nPe; i++) {
                ...
                _out[dof * totalNodalSize + e2n_cg[currentId * nPe + i]] +=
                    (VECType)ss;
            }
        }
    }
    ...
    for (m_uiOctDA->init<ot::DA_FLAGS::W_DEPENDENT>();
         m_uiOctDA->curr() < m_uiOctDA->end<ot::DA_FLAGS::W_DEPENDENT>();
         m_uiOctDA->next<ot::DA_FLAGS::W_DEPENDENT>()) {
        // temporary fix to skip ghost writable.
        if (m_uiOctDA->curr() < eleLocalBegin ||
            m_uiOctDA->curr() >= eleLocalEnd)
            continue;
    ...
    m_uiOctDA->writeToGhostsEnd(_out, ot::DA_FLAGS::WriteMode::ADD_VALUES,
                                m_uiDof);
...
void feMatrix<T>::getMatDiagonal(VECType *&diag, double scale) {
    ...
    for (m_uiOctDA->init<ot::DA_FLAGS::LOCAL_ELEMENTS>();
         m_uiOctDA->curr() < m_uiOctDA->end<ot::DA_FLAGS::LOCAL_ELEMENTS>();
         m_uiOctDA->next<ot::DA_FLAGS::LOCAL_ELEMENTS>()) {
        std::vector<ot::MatRecord> records;
        const unsigned int currentId = m_uiOctDA->curr();
        ...
        for (unsigned int i = 0; i < records.size(); i++) {
            if ((records[i].getRowID() == records[i].getColID()) &&
                (records[i].getRowDim() == records[i].getColDim())) {
                _diag[records[i].getRowDim() * totalNodes +
                      records[i].getRowID()] +=
                    (scale * records[i].getMatVal());
            }
        }
    }
    postMat();
    m_uiOctDA->writeToGhostsBegin(_diag, m_uiDof);
    m_uiOctDA->writeToGhostsEnd(_diag, ot::DA_FLAGS::WriteMode::ADD_VALUES,
                                m_uiDof);
    delete[] coords;
    delete[] p2cEleMat;
...
template <typename T>
void feMatrix<T>::getMatBlockDiagonal(std::vector<ot::MatRecord> &blkDiag,
                                      double scale) {
    blkDiag.clear();
    if (!(m_uiOctDA->isActive())) return;
    ...
    for (unsigned int i = pMesh->getNodeLocalBegin();
         i < pMesh->getNodeLocalEnd(); i++)
    ...
    for (unsigned int i = 0; i < activeNpes; i++) rSendCount[i] = 0;
    ...
    for (m_uiOctDA->init<ot::DA_FLAGS::LOCAL_ELEMENTS>();
        ...
        for (unsigned int i = 0; i < nPe; i++)
            for (unsigned int j = 0; j < nPe; j++) {
                if (_n2p[e2n_cg[currentId * nPe + i]] ==
                    _n2p[e2n_cg[currentId * nPe + j]])
                    rSendCount[_n2p[e2n_cg[currentId * nPe + i]]]++;
            }
    }

    for (unsigned int i = 0; i < activeNpes; i++) rSendCount[i] *= m_uiDof;
    ...
    par::Mpi_Alltoallv(sendBuf.data(), rSendCount, rSendOffset, recvBuf.data(),
                       rRecvCount, rRecvOffset, m_uiOctDA->getCommActive());
    ...
    for (unsigned int i = 0; i < recvBuf.size(); i++) {
        unsigned int curr = i;
        while (((i + 1) < recvBuf.size()) &&
               (recvBuf[i].getRowID() == recvBuf[i + 1].getRowID()) &&
               (recvBuf[i].getColID() == recvBuf[i + 1].getColID()) &&
               (recvBuf[i].getRowDim() == recvBuf[i + 1].getRowDim()) &&
               (recvBuf[i].getColDim() == recvBuf[i + 1].getColDim())) {
            recvBuf[i].setMatValue(recvBuf[i].getMatVal() +
                                   recvBuf[i + 1].getMatVal());
            i = i + 1;
        }
...
bool feMatrix<T>::getAssembledMatrix(Mat *J, MatType mtype) {
    if (m_uiOctDA->isActive()) {
        ...
        for (m_uiOctDA->init<ot::DA_FLAGS::LOCAL_ELEMENTS>();
            ...
            fem_eMatTogMat(&(*(records.begin() + nCount)), p2cEleMat, eleOrder,
                           m_uiDof);
            nCount += (m_uiDof * nPe * nPe * m_uiDof);
            if (records.size() > 500) {
                m_uiOctDA->petscSetValuesInMatrix(*J, records, m_uiDof,
                                                  ADD_VALUES);
                nCount = 0;
            }
