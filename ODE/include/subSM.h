...
class SubScatterMap {
   ...
   private:
    inline void print_error(const char* const m) {
        std::cout << "[subSM Error] in " << __FILE__
                  << " function: " << __func__ << " at line " << __LINE__
                  << " message " << m << std::endl;
    }
   ...
   public:
     ...
     */
    SubScatterMap(const ot::Mesh* pMesh,
                  std::bitset<ot::TreeNode::OctantFlagType::MAX_LEVEL>* stag,
                  std::bitset<ot::TreeNode::OctantFlagType::MAX_LEVEL>* rtag);
...
template <typename T>
void SubScatterMap::readFromGhostBegin(T* vec, unsigned int l,
                                       unsigned int dof) {
    const bool isActive = m_uiMesh->isActive();

    if (m_uiMesh->getMPICommSizeGlobal() == 1 || (!isActive)) return;
    ...
    if (m_uiMesh->isActive()) {
        ...
        if (recvBSz) {
            ...
            // active recv procs
            for (unsigned int recv_p = 0; recv_p < m_uiL2RecvProcList[l].size();
                 recv_p++) {
                const unsigned int proc_id = m_uiL2RecvProcList[l][recv_p];

                MPI_Request* req           = new MPI_Request();
                par::Mpi_Irecv((recvB + dof * m_uiRecvOffset[l][proc_id]),
                               dof * m_uiRecvCount[l][proc_id], proc_id,
                               m_uiAsyncTag, commActive, req);
        ...
        if (sendBSz) {
            ...
            for (unsigned int send_p = 0; send_p < m_uiL2SendProcList[l].size();
                 send_p++) {
                const unsigned int proc_id = m_uiL2SendProcList[l][send_p];
                for (unsigned int var = 0; var < dof; var++) {
                    for (unsigned int k = m_uiSendOffset[l][proc_id];
                         k < (m_uiSendOffset[l][proc_id] +
                              m_uiSendCount[l][proc_id]);
                         k++)
                        sendB[dof * (m_uiSendOffset[l][proc_id]) +
                              (var * m_uiSendCount[l][proc_id]) +
                              (k - m_uiSendOffset[l][proc_id])] =
                            (vec + var * total_sz_zip)[m_uiL2SendSM[l][k]];
                }
            }
            ...
            for (unsigned int send_p = 0; send_p < m_uiL2SendProcList[l].size();
                ...
                const unsigned int proc_id = m_uiL2SendProcList[l][send_p];
                MPI_Request* req           = new MPI_Request();
                par::Mpi_Isend(sendB + dof * m_uiSendOffset[l][proc_id],
                               dof * m_uiSendCount[l][proc_id], proc_id,
                               m_uiAsyncTag, commActive, req);
                ctx.getRequestList().push_back(req);
            }
        }
...
void SubScatterMap::readFromGhostEnd(T* vec, unsigned int l, unsigned int dof) {
    ...
    if (m_uiMesh->isActive()) {
        ...
        if (ctxIndex < 0) {
            std::cout << " Rank: " << m_uiMesh->getMPIRank()
                      << " [SubSM Error]: " << __LINE__
                      << " readFromGhostEnd async context not foound:  "
                      << std::endl;
        ...
        MPI_Status status;
        // std::cout<<" ctx.getRequestList().size():
        // "<<ctx.getRequestList().size()<<std::endl;
        ...
        if (recvBSz) {
            ...
            for (unsigned int recv_p = 0; recv_p < m_uiL2RecvProcList[l].size();
                ...
                const unsigned int proc_id = m_uiL2RecvProcList[l][recv_p];
                for (unsigned int var = 0; var < dof; var++) {
                    for (unsigned int k = m_uiRecvOffset[l][proc_id];
                         k < (m_uiRecvOffset[l][proc_id] +
                              m_uiRecvCount[l][proc_id]);
                         k++) {
                        (vec + var * total_sz_zip)[m_uiL2RecvSM[l][k]] =
                            recvB[dof * (m_uiRecvOffset[l][proc_id]) +
                                  (var * m_uiRecvCount[l][proc_id]) +
                                  (k - m_uiRecvOffset[l][proc_id])];
                    }
                }
            }
