...
class SDC {
   ...
   public:
     ...
     * @return int
     */
    int picard_solve(DVec* in, DVec* out, double tn, double dt,
    ...
    inline DendroScalar get_residual_tolerance() const {
        return m_uiResidualTolerance;
    }

    /**@brief set the residual tolerance*/
    inline int set_residual_tolerance(DendroScalar re) {
        m_uiResidualTolerance = re;
...
int SDC<T, Ctx>::allocate_internal_vars() {
    ...
    for (unsigned int s = 0; s < num_q; s++) {
        m_uiU0[s].create_vector(pMesh, m_uiEVar.get_type(), m_uiEVar.get_loc(),
                                m_uiEVar.get_dof(),
                                m_uiEVar.is_ghost_allocated());
        m_uiUk[s].create_vector(pMesh, m_uiEVar.get_type(), m_uiEVar.get_loc(),
                                m_uiEVar.get_dof(),
                                m_uiEVar.is_ghost_allocated());
        m_uiLUk[s].create_vector(pMesh, m_uiEVar.get_type(), m_uiEVar.get_loc(),
                                 m_uiEVar.get_dof(),
                                 m_uiEVar.is_ghost_allocated());
...
int SDC<T, Ctx>::set_time_integration_order(unsigned int tOrder) {
    ...
    // QMat - First row is zero, since we use Gauss-Labatto integration,
    // \int{t0}^{to} will give us zero. QMat - mth row should be scaled by
    // (tm-t0)/2 which is = dt*(q[m]-q[0])/4

    for (unsigned int si = 1; si < num_q_pts; si++)
        for (unsigned int sj = 0; sj < num_q_pts; sj++)
            m_uiQMat[si * num_q_pts + sj] =
                ((qgl[si] - qgl[0]) / 4.0) * wgl[sj];
...
int SDC<T, Ctx>::evolve() {
    ...
    const int status = this->picard_solve(&m_uiEVar, &m_uiEVar, current_t, dt,
                                          m_uiResidualTolerance);
...
template <typename T, typename Ctx>
int SDC<T, Ctx>::picard_solve(DVec* in, DVec* out, double tn, double dt,
                              double residual_tol) {
    const ot::Mesh* pMesh = m_uiAppCtx->get_mesh();
    if (pMesh->isActive()) {
        ...
        // {
        //     T min = vecMin((T*)&in_ptr[v* sz +
        //     nodeLocalBegin],numLocalNodes,acomm); T max =
        //     vecMax((T*)&in_ptr[v* sz + nodeLocalBegin],numLocalNodes,acomm);

        //     if(!arank)
        //         std::cout<<"VAR : "<<v<<" min : "<<min<<" max:
        //         "<<max<<std::endl;
        // }
        ...
        do {
            ...
            for (unsigned int s = 0; s < num_q; s++) m_uiVk[s].copy_data(*in);

            for (unsigned int si = 1; si < num_q; si++)
                for (unsigned int sj = 0; sj < num_q; sj++)
                    DVec::axpy(m_uiAppCtx->get_mesh(),
                               (dt)*m_uiQMat[si * num_q + sj], m_uiLUk[sj],
                               m_uiVk[si]);

            for (unsigned int s = 0; s < num_q; s++)
                for (unsigned int v = 0; v < dof; v++)
                    error_norms[s * dof + v] = normL2(
                        &m_uiVk[s].get_vec_ptr()[v * sz + nodeLocalBegin],
                        &m_uiUk[s].get_vec_ptr()[v * sz + nodeLocalBegin],
                        numLocalNodes);

            par::Mpi_Allreduce(error_norms, error_norms_g, num_q * dof, MPI_SUM,
            ...
            iter_count++;
                 ...
                 (residual_global > residual_tol));
        if (!arank)
            printf(
                "[sdc] converged at iter : %d \t residual: %.10E tol = %.10E "
                "\n",
                iter_count, residual_global, residual_tol);
        std::memcpy(out->get_vec_ptr(), m_uiUk[m_uiOrder].get_vec_ptr(),
                    sizeof(T) * (sz * dof));
        ...
        // {
        //     if(!arank)
        //         std::cout<<RED<<"[sdc]: picard iteration diverged. killing
        //         the solver program"<<NRM<<std::endl;
