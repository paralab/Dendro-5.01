...
class RK45Transport : public RK {
   private:
    ...
    /**load imbalance tolerance used for flexible partioning (defaule value set
     * to 0.1) use setLDImbParameters function to change this*/
   ...
   public:
     ...
     * @param [in] constant vector b as given in transport equation. This should
     * be an array size of 3.
     * @param [in] g: initial value when t=pTBegin.
     * */
    void setParameters(double* b,
     ...
     * */
    int restoreRK45Solver(const char* fNamePrefix, const unsigned int step,
                          const MPI_Comm comm);
};
...
void RK45Transport::applyBoundaryConditions(const double time, T* vec) {
    ...
    for (unsigned int ele = m_uiMesh->getElementLocalBegin();
        ...
        if ((pNodes[ele].minX() == grid_min) ||
            (pNodes[ele].minY() == grid_min) ||
            ((pNodes[ele].minZ() ==
              grid_min))) {  // current element is a boundary element.
                        ...
                        if ((m_uiMesh->isNodeLocal(ele, i, j, k))) {
                            ...
                            if (((x == grid_min) || (y == grid_min) ||
                                ...
                                assert(!m_uiMesh->isNodeHanging(owner, ii_x,
                                                                jj_y, kk_z));
                                x1 = ((x - grid_min) / (grid_max - grid_min)) *
                                         (d_max - d_min) +
                                     d_min;
                                y1 = ((y - grid_min) / (grid_max - grid_min)) *
                                         (d_max - d_min) +
                                     d_min;
                                z1 = ((z - grid_min) / (grid_max - grid_min)) *
                                         (d_max - d_min) +
                                     d_min;

                                /*((x1-m_uiCoef_b[0]*m_uiCurrentTime)<d_min)?
                                value=value*(-1)*sin(2*d_min-(x1-m_uiCoef_b[0]*m_uiCurrentTime))
                                :value=value*sin((x1-m_uiCoef_b[0]*m_uiCurrentTime));
                                ((y1-m_uiCoef_b[1]*m_uiCurrentTime)<d_min)?
                                value=value*(-1)*sin(2*d_min-(y1-m_uiCoef_b[1]*m_uiCurrentTime))
                                :value=value*sin((y1-m_uiCoef_b[1]*m_uiCurrentTime));
                                ((z1-m_uiCoef_b[2]*m_uiCurrentTime)<d_min)?
                                value=value*(-1)*sin(2*d_min-(z1-m_uiCoef_b[2]*m_uiCurrentTime))
                                :value=value*sin((z1-m_uiCoef_b[2]*m_uiCurrentTime));*/

                                value = sin(x1 - m_uiCoef_b[0] * time) *
                                        sin(y1 - m_uiCoef_b[1] * time) *
                                        sin(z1 - m_uiCoef_b[2] * time);
                                // if(!m_uiMesh->getMPIRank() && (fabs(value-
                                // m_uiU[e2n[ele*nPe+k*(eleOrder+1)*(eleOrder+1)+j*(eleOrder+1)+i]])>1e-2))
                                // std::cout<<" rank:
                                // "<<m_uiMesh->getMPIRank()<<"bdy value:
                                // "<<value<<" computed value: "<<
                                // m_uiU[e2n[ele*nPe+k*(eleOrder+1)*(eleOrder+1)+j*(eleOrder+1)+i]]<<"
                                // i,j,k: ("<<i<<" , "<<j<<" ,"<<k<<" )
                                // "<<std::endl;
                                ...
                                vec[e2n[ele * nPe +
                                        k * (eleOrder + 1) * (eleOrder + 1) +
                                        j * (eleOrder + 1) + i]] =
                                    value;  // m_uiFunc_g((x-m_uiCoef_b[0]*m_uiCurrentTime),(y-m_uiCoef_b[1]*m_uiCurrentTime),(z-m_uiCoef_b[2]*m_uiCurrentTime));
                            }
                        }
                    }
        }
