...
void ot::SubScatterMap::compute_L2SM() {
    ...
    const unsigned int LMAX =
        m_uiLMax + 1;  // ot::TreeNode::OctantFlagType::MAX_LEVEL;
    ...
    for (unsigned int p = 0; p < sProcList.size(); p++) {
        ...
        for (unsigned int k = sOffset[proc_id];
             k < (sOffset[proc_id] + sCounts[proc_id]); k++) {
            for (unsigned int l = m_uiLMin; l < m_uiLMax + 1; l++) {
                if (m_uiSendTag[k][l] == 1) m_uiSendCount[l][proc_id]++;
            }
        }
    }
    ...
    for (unsigned int p = 0; p < rProcList.size(); p++) {
        ...
        for (unsigned int k = rOffset[proc_id];
             k < (rOffset[proc_id] + rCounts[proc_id]); k++) {
            for (unsigned int l = m_uiLMin; l < m_uiLMax + 1; l++) {
                if (m_uiRecvTag[k][l] == 1) m_uiRecvCount[l][proc_id]++;
            }
        }
    }
        ...
        for (unsigned int p = 0; p < activeNpes; p++)
            if (rCts[p] != m_uiRecvCount[l][p]) {
                std::cout << " rank : " << m_uiMesh->getMPIRank()
                          << " p : " << p
                          << " recvCounts: " << m_uiRecvCount[l][p] << " rCnt "
                          << rCts[p] << "\n";
                std::ostringstream oss;
                oss << __func__ << " inconsistent send recv counts for level "
                    << l << " encountered. ";
    ...
    rCts.clear();
    for (unsigned int l = m_uiLMin; l < m_uiLMax + 1; l++) {
        io::dump_array(m_uiSendCount[l], activeNpes, "sendCnt",
                       m_uiMesh->getMPICommunicator());
        io::dump_array(m_uiRecvCount[l], activeNpes, "recvCnt",
                       m_uiMesh->getMPICommunicator());
    }
    ...
    for (unsigned int l = m_uiLMin; l < m_uiLMax + 1; l++) {
        ...
        for (unsigned int p = 0; p < activeNpes; p++) {
            if (m_uiSendCount[l][p] > 0) m_uiL2SendProcList[l].push_back(p);

            if (m_uiRecvCount[l][p] > 0) m_uiL2RecvProcList[l].push_back(p);
        }

        const unsigned int sendBufSz = (m_uiSendOffset[l][activeNpes - 1] +
                                        m_uiSendCount[l][activeNpes - 1]);
        const unsigned int recvBufSz = (m_uiRecvOffset[l][activeNpes - 1] +
