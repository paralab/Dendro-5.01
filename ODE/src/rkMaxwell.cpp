 ...
 * @brief Constains the implementation of Maxwell equations, with the stability
 * properties in numerical relativity.
 *
 * Reference : https://arxiv.org/abs/gr-qc/0201051
 *
 * With the numerical stability properties we can rewrite the Maxwell's
...
RK45Maxwell::RK45Maxwell(ot::Mesh *pMesh, double pTBegin, double pTEnd,
                         double pTh)
...
void RK45Maxwell::performSingleInteration() {
    ...
    for (m_uiMesh->init<ot::WaveletDA::INDEPENDENT>();
         m_uiMesh->nextAvailable<ot::WaveletDA::INDEPENDENT>();
         m_uiMesh->next<ot::WaveletDA::INDEPENDENT>()) {
