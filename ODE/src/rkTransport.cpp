 ...
 *@brief This file contains specific example of solving 3D transport equation
 *using RK45 method
 */
//
...
RK45Transport::RK45Transport(ot::Mesh* pMesh, double pTBegin, double pTEnd,
                             double pTh)
...
void RK45Transport::setParameters(
    double* b, std::function<double(double, double, double)> g,
    std::function<double(double, double, double, double)> f,
    const char* fprefix, double wltTol) {
    m_uiWaveletTol = wltTol;
    m_uiCoef_b[0]  = b[0];
    m_uiCoef_b[1]  = b[1];
    m_uiCoef_b[2]  = b[2];

    // std::cout<<"vec b: "<<m_uiCoef_b[0]<<" ,"<<m_uiCoef_b[1]<<" ,
    // "<<m_uiCoef_b[2]<<std::endl;
...
void RK45Transport::performSingleIteration() {
    ...
    do {
        ...
        if (m_uiMesh->isActive()) {
            ...
            m_uiMesh->unzip(&(*(m_uiPrevU.begin())),
                            &(*(m_uiUZipVecIn.begin())));

            grad(m_uiMesh, 0, &(*(m_uiUZipVecIn.begin())),
                 &(*(m_uiUZipVecOut.begin())));
            m_uiMesh->zip(&(*(m_uiUZipVecOut.begin())), &(*(m_uiDxU.begin())));

            grad(m_uiMesh, 1, &(*(m_uiUZipVecIn.begin())),
                 &(*(m_uiUZipVecOut.begin())));
            m_uiMesh->zip(&(*(m_uiUZipVecOut.begin())), &(*(m_uiDyU.begin())));

            grad(m_uiMesh, 2, &(*(m_uiUZipVecIn.begin())),
                 &(*(m_uiUZipVecOut.begin())));
            m_uiMesh->zip(&(*(m_uiUZipVecOut.begin())), &(*(m_uiDzU.begin())));

            m_uiFunc_f1 = [current_t](const double x, const double y,
                                      const double z) { return 0; };
            ...
            for (unsigned int k = nLocalBegin; k < nLocalEnd; k++) {
                m_uiStage_1[k] =
                    m_uiT_h *
                    (m_uiRHS[k] - m_uiCoef_b[0] * m_uiDxU[k] -
                     m_uiCoef_b[1] * m_uiDyU[k] - m_uiCoef_b[2] * m_uiDzU[k]);
                m_uiVecIm[k] = m_uiPrevU[k] + RK45_STAGE_2_U * m_uiStage_1[k];
            }

            current_t_adv = current_t + RK45_STAGE_2_T * m_uiT_h;
            // m_uiFunc_f1=[current_t_adv](const double x,const double y,const
            // double z){return m_uiFunc_f(x,y,z,current_t_adv);};
            m_uiMesh->createVector(m_uiRHS, m_uiFunc_f1);

            applyBoundaryConditions(current_t_adv, &(*(m_uiVecIm.begin())));
            m_uiMesh->performGhostExchange(m_uiVecIm);

            m_uiMesh->unzip(&(*(m_uiVecIm.begin())),
                            &(*(m_uiUZipVecIn.begin())));
            ...
            for (unsigned int k = nLocalBegin; k < nLocalEnd; k++) {
                m_uiStage_2[k] =
                    ...
                    (m_uiRHS[k] - m_uiCoef_b[0] * m_uiDxU[k] -
                     m_uiCoef_b[1] * m_uiDyU[k] - m_uiCoef_b[2] * m_uiDzU[k]);
                m_uiVecIm[k] = m_uiPrevU[k] + (RK45_STAGE_3_U1)*m_uiStage_1[k] +
                               (RK45_STAGE_3_U2)*m_uiStage_2[k];
            }
            ...
            for (unsigned int k = nLocalBegin; k < nLocalEnd; k++) {
                m_uiStage_3[k] =
                    ...
                    (m_uiRHS[k] - m_uiCoef_b[0] * m_uiDxU[k] -
                     m_uiCoef_b[1] * m_uiDyU[k] - m_uiCoef_b[2] * m_uiDzU[k]);
                m_uiVecIm[k] = m_uiPrevU[k] + (RK45_STAGE_4_U1)*m_uiStage_1[k] +
                               (RK45_STAGE_4_U2)*m_uiStage_2[k] +
                               (RK45_STAGE_4_U3)*m_uiStage_3[k];
            }
            ...
            for (unsigned int k = nLocalBegin; k < nLocalEnd; k++) {
                m_uiStage_4[k] =
                    ...
                    (m_uiRHS[k] - m_uiCoef_b[0] * m_uiDxU[k] -
                     m_uiCoef_b[1] * m_uiDyU[k] - m_uiCoef_b[2] * m_uiDzU[k]);
                m_uiVecIm[k] = m_uiPrevU[k] + (RK45_STAGE_5_U1)*m_uiStage_1[k] +
                               (RK45_STAGE_5_U2)*m_uiStage_2[k] +
                               (RK45_STAGE_5_U3)*m_uiStage_3[k] +
                               (RK45_STAGE_5_U4)*m_uiStage_4[k];
            }
            ...
            for (unsigned int k = nLocalBegin; k < nLocalEnd; k++) {
                m_uiStage_5[k] =
                    ...
                    (m_uiRHS[k] - m_uiCoef_b[0] * m_uiDxU[k] -
                     m_uiCoef_b[1] * m_uiDyU[k] - m_uiCoef_b[2] * m_uiDzU[k]);
                m_uiVecIm[k] = m_uiPrevU[k] + (RK45_STAGE_6_U1)*m_uiStage_1[k] +
                               (RK45_STAGE_6_U2)*m_uiStage_2[k] +
                               (RK45_STAGE_6_U3)*m_uiStage_3[k] +
                               (RK45_STAGE_6_U4)*m_uiStage_4[k] +
                               (RK45_STAGE_6_U5)*m_uiStage_5[k];
            }
            ...
            for (unsigned int k = nLocalBegin; k < nLocalEnd; k++) {
                m_uiStage_6[k] =
                    ...
                    (m_uiRHS[k] - m_uiCoef_b[0] * m_uiDxU[k] -
                     m_uiCoef_b[1] * m_uiDyU[k] - m_uiCoef_b[2] * m_uiDzU[k]);
                m_uiU[k] = m_uiPrevU[k] + RK45_STAGE_1_COEF * m_uiStage_1[k] +
                           RK45_STAGE_3_COEF * m_uiStage_3[k] +
                           RK45_STAGE_4_COEF * m_uiStage_4[k] +
                           RK45_STAGE_5_COEF * m_uiStage_5[k] +
                           RK45_STAGE_6_COEF * m_uiStage_6[k];
                m_uiVecIm[k] = m_uiPrevU[k] + (25.0 / 216.0) * m_uiStage_1[k] +
                               (1408.0 / 2565.0) * m_uiStage_3[k] +
                               (2197.0 / 4101.0) * m_uiStage_4[k] +
                               (-1.0 / 5.0) * m_uiStage_5[k];
            }

            applyBoundaryConditions(m_uiCurrentTime + m_uiT_h,
                                    &(*(m_uiU.begin())));
            applyBoundaryConditions(m_uiCurrentTime + m_uiT_h,
                                    &(*(m_uiVecIm.begin())));

            rk45_linf = normLInfty(&(*(m_uiU.begin() + nLocalBegin)),
                                   (&*(m_uiVecIm.begin() + nLocalBegin)),
...
void RK45Transport::rkSolve() {
    ...
    std::function<double(double, double, double, double* hx)> waveletTolFunc =
        [wTol](double x, double y, double z, double* hx) { return wTol; };
    ...
    for (double t = m_uiCurrentTime; t < m_uiTimeEnd; t = t + m_uiT_h) {
        if ((m_uiCurrentStep % IO_OUTPUT_FREQ) == 0) {
            ...
            if (m_uiCurrentStep % IO_OUTPUT_FREQ ==
                0) /*if((m_uiCurrentStep==0) ||
                      fabs((m_uiCurrentTime)-m_uiLastIOTimeValue)>IO_OUTPUT_GAP)*/
            {
        ...
        if (m_uiCurrentStep % REMESH_TEST_FREQ == 0) {
            ...
            isRefine = m_uiMesh->isReMeshUnzip((const double**)unzipVarPtr,
                                               refineVarIds, refineNumVars,
                                               waveletTolFunc);
            ...
            if (isRefine) {
                ...
                std::cout<<"Interation: "<<m_uiCurrentStep<<" rank:
                "<<m_uiMesh->getMPIRank()<<" isRefine: "<<isRefine<<" # split
                elements: "<<numRefinedElements<<" # coarsen elements:
                "<<numCoarsenElements<<" # not changed:
                "<<numNotChangedElements<<std::endl;*/

                ot::Mesh* newMesh =
                    m_uiMesh->ReMesh(m_uiLoadImbTol, m_uiSplitterFix);
...
int RK45Transport::storeCheckPoint(const char* fNamePrefix) {
    ...
    sprintf(fName, "%s_octree_%d_%d_%d.oct", fNamePrefix, m_uiCurrentStep, rank,
            npes);
    ...
    for (unsigned int i = 0; i < numVars; i++) {
        sprintf(fName, "%s_%s_%d_%d_%d.var", fNamePrefix, varNames[i],
...
int RK45Transport::restoreRK45Solver(const char* fNamePrefix,
