...
namespace ode
{
    namespace solver
    {
        ...
        RK4_QUADGRAV::RK4_QUADGRAV(ot::Mesh *pMesh, double pTBegin, double pTEnd,double pTh): RK(pMesh,pTBegin,pTEnd,pTh)
        {

            // allocate memory for the variables.
            m_uiVar=new double*[quadgrav::QUADGRAV_NUM_VARS];
            for(unsigned int index=0;index<quadgrav::QUADGRAV_NUM_VARS;index++)
                m_uiVar[index]=m_uiMesh->createVector<double>();

            m_uiPrevVar=new double*[quadgrav::QUADGRAV_NUM_VARS];
            for(unsigned int index=0;index<quadgrav::QUADGRAV_NUM_VARS;index++)
                m_uiPrevVar[index]=m_uiMesh->createVector<double>();

            m_uiVarIm=new double*[quadgrav::QUADGRAV_NUM_VARS];
            for(unsigned int index=0;index<quadgrav::QUADGRAV_NUM_VARS;index++)
                m_uiVarIm[index]=m_uiMesh->createVector<double>();


            m_uiStage=new double**[quadgrav::QUADGRAV_RK4_STAGES];
            for(unsigned int stage=0;stage<quadgrav::QUADGRAV_RK4_STAGES;stage++)
            {
                m_uiStage[stage]=new double*[quadgrav::QUADGRAV_NUM_VARS];
                for(unsigned int index=0;index<quadgrav::QUADGRAV_NUM_VARS;index++)
                    m_uiStage[stage][index]=m_uiMesh->createVector<double>();
            }

            m_uiUnzipVar=new double*[quadgrav::QUADGRAV_NUM_VARS];
            for(unsigned int index=0;index<quadgrav::QUADGRAV_NUM_VARS;index++)
                m_uiUnzipVar[index]=m_uiMesh->createUnZippedVector<double>();


            m_uiUnzipVarRHS=new double*[quadgrav::QUADGRAV_NUM_VARS];
            for(unsigned int index=0;index<quadgrav::QUADGRAV_NUM_VARS;index++)
            ...
            // mpi communication
            m_uiSendNodeBuf=new double*[quadgrav::QUADGRAV_ASYNC_COMM_K];
            m_uiRecvNodeBuf=new double*[quadgrav::QUADGRAV_ASYNC_COMM_K];
        ...
        void RK4_QUADGRAV::applyInitialConditions(double** zipIn)
        {
                    ...
                    for(unsigned int j=0;j<(eleOrder+1);j++ )
                        for(unsigned int i=0;i<(eleOrder+1);i++)
                        {
                            ...
                            if(nodeLookUp_CG>=nodeLocalBegin && nodeLookUp_CG<nodeLocalEnd)
                            {
                                ...
                                quadgrav::initData((double)x,(double)y,(double)z,var);
        ...
        void RK4_QUADGRAV::initialGridConverge()
        {
            ...
            std::function<double(double,double,double)> waveletTolFunc =[wTol](double x,double y, double z){ return quadgrav::computeWTol(x,y,z,wTol);};
            ...
            do
            {
                ...
                if(isRefine)
                {
                    ...
                    if(m_uiMesh->isActive())
                    {
                        double l_min=vecMin(m_uiPrevVar[quadgrav::VAR::U_CHI]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                        double l_max=vecMax(m_uiPrevVar[quadgrav::VAR::U_CHI]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
        ...
        void RK4_QUADGRAV::reallocateMPIResources()
        {
            for(unsigned int index=0;index<quadgrav::QUADGRAV_ASYNC_COMM_K;index++)
            {
                delete [] m_uiSendNodeBuf[index];
        ...
        void RK4_QUADGRAV::writeToVTU(double **evolZipVarIn, double ** constrZipVarIn, unsigned int numEvolVars,unsigned int numConstVars,const unsigned int * evolVarIndices, const unsigned int * constVarIndices)
        {
            ...
            double *pData[(numConstVars+numEvolVars+2)];
                ...
                #ifdef QUADGRAV_COMPARE_WITH_ANALYTICAL_SOL
                                    ...
                                    for(unsigned int j=0;j<(eleOrder+1);j++ )
                                        for(unsigned int i=0;i<(eleOrder+1);i++)
                                        {
                                            ...
                                            if(nodeLookUp_CG>=nodeLocalBegin && nodeLookUp_CG<nodeLocalEnd)
                                            {
                                                ...
                                                diffVec[nodeLookUp_CG]=var[quadgrav::VAR::U_CHI]-evolZipVarIn[quadgrav::VAR::U_CHI][nodeLookUp_CG];
                                                chiAnalytical[nodeLookUp_CG]=var[quadgrav::VAR::U_CHI];
                            ...
                            if(!m_uiMesh->getMPIRank()) {
                                ...
                                std::ofstream fileGW;
                                char fName[256];
                                sprintf(fName,"%s_error.dat",quadgrav::QUADGRAV_PROFILE_FILE_PREFIX.c_str());
        ...
        void RK4_QUADGRAV::performGhostExchangeVars(double** zipIn)
        {
        ...
        void RK4_QUADGRAV::intergridTransferVars(double**& zipIn, const ot::Mesh* pnewMesh)
        {
        ...
        void RK4_QUADGRAV::unzipVars(double** zipIn , double** uzipOut)
        {
        ...
        void RK4_QUADGRAV::unzipVars_async(double ** zipIn , double **uzipOut)
        {
        ...
        void RK4_QUADGRAV::zipVars(double** uzipIn , double** zipOut)
        {
        ...
        void RK4_QUADGRAV::applyBoundaryConditions()
        {

        }

        void RK4_QUADGRAV::performSingleIteration()
        {
            ...
            do
            {
                ...
                if(m_uiMesh->isActive())
                {
                    ...
                    /*for(unsigned int index=0;index<quadgrav::QUADGRAV_NUM_VARS;index++)
                        for(unsigned int node=nodeLocalBegin;node<nodeLocalEnd;node++)
                            m_uiStage[(quadgrav::QUADGRAV_RK4_STAGES-1)][index][node]*=m_uiT_h;*/
        ...
        void RK4_QUADGRAV::rkSolve()
        {
            ...
            for(double t=m_uiCurrentTime;t<m_uiTimeEnd;t=t+m_uiT_h)
            {
                ...
                if((m_uiMesh->isActive()) && (m_uiCurrentStep%quadgrav::QUADGRAV_TIME_STEP_OUTPUT_FREQ)==0)
                {
                    ...
                    l_min=vecMin(m_uiPrevVar[quadgrav::VAR::U_CHI]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    l_max=vecMax(m_uiPrevVar[quadgrav::VAR::U_CHI]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    if(!m_uiMesh->getMPIRank()) {
                        std::cout << "executing step: " << m_uiCurrentStep << " dt: " << m_uiT_h << " rk_time : "<< m_uiCurrentTime << std::endl;
                ...
                if((m_uiCurrentStep%quadgrav::QUADGRAV_IO_OUTPUT_FREQ)==0)
                {
                    ...
                    writeToVTU(m_uiPrevVar,m_uiConstraintVars,quadgrav::QUADGRAV_NUM_EVOL_VARS_VTU_OUTPUT,0,quadgrav::QUADGRAV_VTU_OUTPUT_EVOL_INDICES,NULL);
                    //#endif
        ...
        void RK4_QUADGRAV::storeCheckPoint(const char * fNamePrefix)
        {
        ...
        void RK4_QUADGRAV::restoreCheckPoint(const char * fNamePrefix,MPI_Comm comm)
        {
